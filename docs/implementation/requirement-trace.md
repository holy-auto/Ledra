...
| Role / Permission | §16（部分） | Role5段+Permission55種+RLS240は稼働中。正準動詞7種・店舗スコープ判定関数の型基盤を追加。API側の権限強制は`API_ROUTE_PERMISSIONS`+構造テストで固定（2026-09-01、103ルート）。**変更系ルート317本中、認可チェックを持たないものは39本まで減った（うち15本は自己完結型で現状維持が正しい。残り24本の方針はOPEN_QUESTIONS.md）。動詞が語彙に無い資源（在庫・発注・部品・工程テンプレート・受注更新）はロール下限で暫定的に守っている。`operationRisk()`/`storeScope.ts` は依然として本番の認可経路から呼ばれておらず、Assignment軸・Risk軸の権限強制・「承認を依頼」ワークフローは未実装** | IMP-013, 045 | ⚠️ 部分（critical操作の経路間不一致は解消。網羅的強制は未着手） |
