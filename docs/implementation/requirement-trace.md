...
| Role / Permission | §16（部分） | Role5段+Permission55種+RLS240は稼働中。正準動詞7種・店舗スコープ判定関数の型基盤を追加。API側の権限強制は`API_ROUTE_PERMISSIONS`+構造テストで一部固定（2026-08-31、17ルート）。**ただしテナント認証を通す変更系ルート316本中125本が認可チェックを持たない（自己完結型を含むため全部がバグではない。切り分けは判断待ち）。`operationRisk()`/`storeScope.ts` は依然として本番の認可経路から呼ばれておらず、Assignment軸・Risk軸の権限強制・「承認を依頼」ワークフローは未実装** | IMP-013, 045 | ⚠️ 部分（critical操作の経路間不一致は解消。網羅的強制は未着手） |
...
- **IMP-013**（一部本番配線、2026-08-31）: 証明書無効化(critical)の3経路で認可が食い違い、`/api/certificates/void`はテナント所属だけで通っていた（viewerでも無効化可能）ため修正し、`API_ROUTE_PERMISSIONS`+構造テストで固定した。ただし網羅的な強制は未着手（変更系316本中125本が認可チェックなし。修正前は128本）で、正準権限動詞・店舗スコープ判定関数は依然として本番の認可強制経路に接続されていない（多店舗テナントが本番に0件のため店舗スコープは意図的に見送り）。
