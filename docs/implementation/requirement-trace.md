...
| §17 | Localization & Terminology | 自前 i18n 基盤(`src/lib/i18n/`)+`messages/ja.json`,`en.json`(errors 8キーのみ)。AI 翻訳 `translateContent` は別系統 | 部分 | 6言語(ja/en/vi/id/fil/hi)未対応。画面適用ゼロ。自動車用語集なし。original_text/translated 分離なし | IMP-011 |
| §18 | Privacy / Data Access / Retention | データ保持 cron(`data-retention`)+顧客削除リクエスト(`customer_deletion_requests`)+PII 開示制御(保険 `is_pii_disclosed()`)+`docs/data-retention.md`+`src/lib/privacy/`(4段階データ分類+4段階可視性+レンディションマスキング+エクスポート監査) | 実装済み | 型基盤完了。既存 PII 遮断・保持 cron・削除リクエストに加え、分類/可視性/マスキング/監査の構造化フレームワークを追加。API/UI 統合は下流タスクで | IMP-050 |
...
| 12 | 初期6言語 ja/en/vi/id/fil/hi | ja/en の2ロケール(errors 8キーのみ)、画面適用ゼロ | 部分 | IMP-011 |
...
| IMP-046 | 4 / P1 | §21, ANALYTICS_STORE(経営分析) | **実装済み**(2026-08-20): 運用KPI計算器6本(`operationalKpi.ts` — VERIFIED到達率/証跡充足率/レビュー待ち時間/サイクルタイム/SLA遵守率/日次スループット、一括算出`computeOperationalKPIs()`)+キャパシティ分析(`capacityAnalytics.ts` — `decomposeTimeBands()`でcapacity>1ブースの時間帯別占有分解(IMP-041 L330/L347委譲実装)、`computeFleetUtilization()`で全ブースフリート稼働率サマリー、`computeStaffCapacity()`でスタッフ負荷率・効率・過負荷/遊休識別)。既存財務KPI(ManagementClient)変更なし。DB/API/UI変更なし(型基盤先行)。テスト40件 | 014, 029, 041, 045 |
| IMP-050 | 5 / P0 | §18, SECURITY_PRIVACY(プライバシー・保持・Export) | **実装済み**(2026-08-20): 4モジュール純関数で型基盤を整備。(1) `classification.ts` — 4段階データ分類(restricted/pii/confidential/public、ISO 27001 A.5.12準拠)、`FIELD_CLASSIFICATIONS`レジストリ19エントリ(customers/vehicles/invoices/tenant_secrets)、`getFieldClassification()`ルックアップ、`maxClassification()`最厳分類、`findClassificationViolations()`閾値超過検出。(2) `visibility.ts` — 4段階可視性(owner_only/tenant_internal/partner_shared/public)、`ViewerContext`(role/isDataSubject/hasPartnerConsent)→有効レベル解決`resolveVisibility()`、`DEFAULT_REQUIRED_VISIBILITY`分類→可視性マッピング、`findHiddenFields()`。(3) `rendition.ts` — ADR-0003一般化、4戦略(nullify/redact/truncate/hash)、`createRendition()`非破壊マスキング、定義済みルール3セット(CERTIFICATE/VEHICLE/PASSPORT_PUBLIC_RULES — 既存certificates_publicビュー・VEHICLE_TABLE_PII_COLUMNSパターンの型安全化)。(4) `exportAudit.ts` — 4スコープ(admin/customer/agent/insurer)統一監査フォーマット、`createExportAuditEntry()`、`detectAbnormalExportFrequency()`。既存PII遮断(customerRelation.ts)・保持cron・削除リクエストは変更なし。DB/API/UI変更なし。テスト64件 | 013, 025, 028 |
