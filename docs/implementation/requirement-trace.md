...
| §12 | Certificate / Integrity / Correction | 写真ゲート+void+編集履歴(`certificate_edit_histories`)+content hash(`certificate_versions` Phase 1)+アンカー2層+Certificate Gate評価器(`gateEvaluator.ts`、10条件一括判定) | 部分 | Gate評価器は純関数で実装済み(3条件実装+7スタブ)。活性化ルートへの統合は後続。READY→ISSUING→VERIFYING→VERIFIEDの自動遷移なし(§1.4)。SUPERSEDED版遷移・旧QR誘導なし。REVOKED相当のvoid公開表示はあり | IMP-028, 030 |
...
| IMP-031 | 3 / P0 | §19.1 例外(cancel/no-show/pause/追加作業) | cancelled のみ。PAUSED/NO_SHOW/PARTIALLY_COMPLETED なし。追加作業の版管理なし(部分) | 015, 022, 027, 030 |
