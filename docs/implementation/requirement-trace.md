...
| Role / Permission | §16（部分） | Role5段+Permission55種+RLS240は稼働中。正準動詞7種・店舗スコープ判定関数の型基盤を追加。API側の権限強制は`API_ROUTE_PERMISSIONS`+構造テストで一部固定（2026-08-31、17ルート）。**ただしテナント認証を通す変更系ルート316本中125〜164本が認可チェックを持たない（数は判定条件による。詳細と条件はOPEN_QUESTIONS.md。自己完結型を含むため全部がバグではなく、切り分けは判断待ち）。`operationRisk()`/`storeScope.ts` は依然として本番の認可経路から呼ばれておらず、Assignment軸・Risk軸の権限強制・「承認を依頼」ワークフローは未実装** | IMP-013, 045 | ⚠️ 部分（critical操作の経路間不一致は解消。網羅的強制は未着手） |
