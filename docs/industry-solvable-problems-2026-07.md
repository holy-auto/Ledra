...
### ① 施工の「証明」— 主張 → 外部検証可能な事実 〔実装済〕
- **Before**: 紙の保証書・作業記録。発行後の差し替え・紛失・後出しが可能。ビッグモーター事件では損保4社が約6.5万件を不正認定(≈¥39,000/台)、国交省は作業前中後の写真保存を指針化。
- **After**: 証明書＋写真を SHA-256 → Merkle → Polygon にアンカリング(`src/lib/anchoring/`)。発行には写真が最低1枚必須でサーバ強制(`src/lib/certificates/photoRequirement.ts`)、コーティング/PPF は Before/After 必須。顧客・保険会社は URL/QR で真正性を検証可能。
- **言語化**: 証明が「作った側の主張(信じるしかない)」から「**第三者が検証できる事実**」に変わる。発行後に内容・写真を差し替えても検証で露見する。
...
- **After**: AI 自動入力(`src/lib/ai/`、40+タスク)。同社試算で証明書1件20分×50件/月＝月17時間削減。
- **言語化**: 同じ人員で回せる工数が増える。ただし**物理的な整備そのものは代替不可**(ここは変わらない)。

### ④ 中古車履歴 — 断絶 → VIN 単位で連続 〔実装済〕
- **Before**: メーター・整備履歴が輸出/オークション間で断絶しやすい。
- **After**: Vehicle Passport(`src/app/passport/`、`passport_ownership_transfers`、検証 API)で VIN 単位に履歴・所有権移転を連結。
- **言語化**: 履歴が「点」から「**連続した線**」になり、流通時の情報非対称が縮む。

### ⑤ 点検整備記録簿 — 紙の二重管理 → 電子(封緘は近接ロードマップ) 〔一部実装／要転用〕
- **Before**: 2025-07-08 に電子記録簿が解禁されたが、多くは紙とデジタルの二重管理。
- **After**: 電子保存には対応。ただし**改ざん不能な封緘**は、TSA/アンカリング基盤(`src/lib/parts/tsa.ts`)は保有するものの現状は部品装着確認(`part_installations.tsa_*`)に結線され、`inspection_records` は今のところ可変レコード。→ 記録簿封緘は「**基盤あり・要転用**」の近接ロードマップ。
- **言語化**: 二重管理の解消は「今」、改ざん不能な記録簿は「基盤転用ですぐ近く」。

### 総括（一文）
**Ledra があると、自動車アフターマーケットの取引が「主張ベース(信じるしかない)」から
「検証ベース(事実で確かめられる)」に移る。** 最も強く効くのは ①施工証明の改ざん耐性と
②保険即時照会(いずれも実装済み)。③省力化・④中古履歴が続き、⑤記録簿封緘は基盤転用で近接。
EV/ADAS の物理スキルは対象外。
