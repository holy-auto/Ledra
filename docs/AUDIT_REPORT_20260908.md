...
| PR | 範囲 | 状態 |
|----|------|------|
| PR-1 | Critical/High セキュリティ修正（Web + DB） | **実装済み・レビュー対応済み**（PR #1054） |
| PR-2 | 課金・予約・cron の整合性（High/Medium バグ） | 未着手 |
| PR-3 | モバイル修正（Medium/Low） | 未着手 |
| PR-4 | AI/クローラ露出と衛生 | 未着手 |
| PR-5 | 重複圧縮 | 未着手 |

各 PR は実装後にプラン記載内容との照合チェックを行い、確認完了後に次の
PR へ進む逐次フローで実施する。
...
して補正済み。PR-2〜PR-5 の是正状況は各 PR マージ時に本ファイルへ
追記する。*
