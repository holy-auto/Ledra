...
| `inbound_message.auto_reply_knowledge`    | LINE の一般質問 (営業時間・駐車場 等) 受信時に**店舗ナレッジ + 全店舗共有ナレッジのみを根拠に自動返信** (ナレッジ外は返信せずスタッフ対応に残す) | OFF  | ✅ LINE webhook (inboundAuto→knowledgeReplyAuto)     |
...
> (CRUD: `/api/admin/settings/line-knowledge`、編集は owner/admin のみ / 上限 = 注入上限の
> 50 件、`KNOWLEDGE_LIMIT` を単一定義として共有)。加えて **全テナント共有ナレッジ**
> (`global_line_knowledge`、運営が `/admin/platform/line-knowledge` で管理・上限 30 件) も
> 共通の回答ソースとして注入され、**矛盾時は店舗ナレッジが優先** (プロンプトで強制)。
> LINE 受信時は `maybeAutoProcessInboundMessage` の中で `maybeAutoReplyKnowledge`
...
>
> ゲート: 抽出 (`inbound_message.auto_extract`) を opt-in していなくても、自動返信系
> (`auto_reply_knowledge` / `auto_reply_rough_estimate`) のいずれかが ON なら抽出は
> **内部依存として実行** される (その場合 `ai_extracted` の保存はしない)。
>
> 安全ガード: opt-in (既定 OFF) + `ai_inbound_extract` プラン / AI が
> 「ナレッジのみで回答可能」(can_answer) と判断し confidence ≥ 閾値の場合のみ送信 /
> ナレッジ未登録なら何もしない / intent は許可リスト (inquiry_only・new_reservation・other)
> のみ — cancel・change_reservation は返信しない (予約操作はスタッフが行うため) /
> **ナレッジ返信は概算見積りより先に判定** され、返信した場合は概算見積り
> (`quote.auto_reply_rough_estimate`) をスキップする (二重返信防止 — 返り値 boolean で判定。
> 「駐車場の料金は？」のような価格語を含む一般質問を、見積りの不足情報聞き返しが
> 先取りしないための順序)。
> 監査ログ (`logAutoActionExecuted`, actionKey=`inbound_message.auto_reply_knowledge`) に残す。
> テスト: `knowledgeReplyAuto.test.ts` / `inboundAutoReplyGate.test.ts` /
> `src/lib/ai/__tests__/knowledgeReply.test.ts`。
...
> **一次判定は保存済みシグナルの集約**で行う (追加ダウンロード・AI 呼び出しなし)。重複ハッシュ /
> ディープフェイク likely_fake / 撮影日時の未来を suspicious、撮影メタ欠落を inconclusive とする。
> **二次判定 (Vision エスカレーション)**: 一次で判定不能 (inconclusive) なグレーゾーン画像だけを
> Opus Vision (`inspectImageTamperingVision`) で内容審査する (画素ベースなので EXIF 除去後でも有効)。
> コスト最小化のため画像ごとの Vision 結果を `meta.tampering_check.vision` にキャッシュし、再
> アップロード時は **新規グレー画像のみ** Opus を呼ぶ (1 回最大 4 枚)。`source_policies.photos=false`
