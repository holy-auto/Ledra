...
> 実際に突き合わせた結果: 本番にだけ **`ft_*` 12 テーブル**（全件 0 行・アプリ参照 0・
> 外部 FK 0。用途不明のため削除は代表判断待ち）、列は**本番だけ 15 件 / マイグレーション
> だけ 13 件**、ポリシーは**本番 626 / 再生 607**。
...
> `20260918000000` で列を足した。**本番反映は `db-migrate` 待ち。**
