...
- WebAuthn 操作署名・鍵の運営者非保持（パスキー登録/失効 + 証明書 finalize/void への
  暗号署名。秘密鍵は端末側、サーバは公開鍵のみ保持。操作署名の強制は
  `WEBAUTHN_OPERATION_SIGNING` で段階導入、既定 off。`src/lib/webauthn/`、PR #787/#800）
- Google カレンダー双方向同期（`gcal-sync` cron が 15 分毎に予約↔GCal を同期。複数
  読み取りカレンダー + 「予定あり(非公開)」モード対応。PR #793/#810）
- 指名 BtoB 請求・仮押さえ予約（取引先指名の請求書払い/支払サイクル自動、枠の仮押さえ→
  承認で本予約・オーバーセル防止。PR #783/#785/#794）
...
- API: 617 Route Handlers（37 トップレベルグループ、2026-07-24 実測）
- Cron: 31 本（vercel.json）／ページ: 297（App Router）
