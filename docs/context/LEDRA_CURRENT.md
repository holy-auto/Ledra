...
> 「デモ保険会社 → デモ施工店」の1件だけ。
> **未解決**: マイグレーションを通さず本番スキーマへ入った経路と、同じ経路で入った他の
> オブジェクトの有無（OPEN_QUESTIONS「マイグレーション外で本番スキーマが変更された経路が不明」）。
> 詳細は DECISION_LOG 2026-09-03 / 2026-09-04、RELEASE_LOG 2026-09-04。

> 2026-09-06 追記: **Academy の事例公開を2段階にした。** 「内容を確認」で AI 要約を
