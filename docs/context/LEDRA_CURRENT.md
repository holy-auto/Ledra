...
> 2026-09-05 追記: **決済・帳票送付・アカデミーAI の7箇所に認可を入れ、残っていた
> 判断待ちを解消した。** Stripe 連携（接続・解除）は owner のみ、顧客への決済リンク発行は
> staff、備品購入は admin、帳票の顧客送付は staff。
> あわせて**「未強制24本」が数え間違いだった**ことが分かった。1本ずつ読んだら18本は
> 別の形で守られていた（著者判定・インラインのロール判定・受講系の自己スコープ）。
> 構造テストの検出器が決め打ちの関数名しか認可と認識せず、**「認識できない」を
> 「認可が無い」と読み替えていた**のが原因。検出器の一覧の意味を実態に合わせ、
> 28件すべてに分類コメントを付けた。**説明のつかないハンドラはゼロ。**
