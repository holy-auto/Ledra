...
- **IMP-052（§23 E2E_SUITE — 必須 E2E テストスイート）完了**:
  ...
  作業詳細→証明書→車両→顧客→請求書）、例外フロー9テスト（API 5: cancel/void/approve/
  status/photo-gate + UI 4: settings/404/POS/search）、顧客確認4テスト、WCAG AA 9テスト
  （公開4+管理4+全違反レポート1）。CI E2E ジョブ復元（secrets ゲート付き）。
  全テスト環境変数ゲートで secrets 未設定時は自動 skip。
