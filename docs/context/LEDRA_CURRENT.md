...
> TypeScript の書き手は共有ヘルパー `recordInsurerAccessLog` を通る（`action` は union 型なので
> 語彙外は tsc が落ちる）。直 insert **10 箇所**をこのヘルパーに寄せ、どこも `error` を捨てなくなった。
> 型の一覧と再生検査の一覧のずれは `npm run check:audit-actions`（CI 9 本目）が落とす。
>
> **#1135 で直っていなかった故障を1件発見**: `caseSummaryAuto` / `caseAssignAuto` / `fraudScoreAuto`
> の3本は `insurer_user_id`（NOT NULL・既定なし）を渡しておらず、本番で毎回 **23502** で落ちていた
> （実測）。`.then(() => {})` で戻り値を捨てていたため無言。代表判断で、保険会社ごとに
> `insurer_users` のシステム行（`is_system = true` / `user_id IS NULL`）を持たせ、その id を使う形に。
> RLS のポリシーはすべて `user_id = auth.uid()` の等値比較なので、この行は**誰にも権限を与えない**
> （本番でポリシー7本すべてがこの形であることを確認済み）。人の一覧（利用者管理・担当候補・
> プラットフォーム集計）からは除外し、監査ログの絞り込みには表示名として出す。
>
> 検証: `ci-parallel-checks.sh` 9/9・再生 506/506・振る舞いの検査 6 件。陰性対照4件を実測。
> **未決**: SQL 関数の**中の** `action` リテラルは依然として縛れていない（書き手の関数集合が
> 増えたら落とす、までしか出来ていない）。**本番未適用**（PR 作成時点）。

> 2026-09-24 追記: **管理画面と課金 API が「選択中の会社」で動くよう統一し、課金操作をオーナー限定にした**（#1147・`f3d2457c`）。
> 複数テナント所属のユーザーが別テナントを選んでいても、管理画面 18 ファイル（証明書・顧客・案件・車両・設定・監査ログ・
> 膜厚レポートの各画面と、証明書/帳票/請求書の PDF・CSV 出力）と課金 API 5 本が最初の所属テナントで動いていた。
> すべて `active_tenant_id` を優先する解決に揃え、プラン購入・請求ポータル・支払い再開はオーナー以上のみ（admin 以下は 403、
> 課金画面でもボタンを出さない）。**未確認**: 複数テナント所属アカウントでの実ブラウザ確認、
> 本番で過去に別テナントの契約を誤操作した例の有無（Stripe 顧客とテナントの対応の照合が必要）。
...
> 2026-09-24: 発注書の写真に TC コードが書かれていれば OCR で読み、工賃計算の TC 欄に入れる（#1148・`953c083`）。
> 実際の発注書に TC コードが載っているかは未確認【要確認】。
> 2026-09-24 決着（DECISION_LOG 同日）: 工賃の1円未満は**切り上げ**（全店共通、`calcLaborPrice`）。TC 付きでの再収集と d-Happy 運営者への問い合わせは**しない**。
...
> `my_insurer_ids()` に同じ停止判定を入れ、**ポリシーは1本も触っていない**
> （14本すべてがこの関数を通り、他の呼び出し元は 0 本と実測）。
> **同じファイルで、本番にしか無かった SELECT ポリシー3本を書き起こした** ——
> `insurers_select_own` / `insurers_select_linked_user` / `insurer_users_select_self`。
> この3本が「停止中でも自社の1行は読める」を支えており、**無い状態でゲートだけ入れると
> 本番は無事でプレビュー分岐と新環境だけが壊れる**。順序が意味を持つので分割していない。
> 停止中の見え方（再生 DB で `authenticated` に降りて実測）: 自社 1 行・自分のメンバーシップ 1 行は残り、
> 閲覧許可テナント・案件・メッセージは 0 行。active では現行と一切変わらない。
> 検出器は3通りの壊し方で検証済み。
> **残件**: アプリ側の読み取り経路（RLS かサービスロールか）の全数調査【要確認】、
> `current_insurer_id()` の棚卸し【要確認】、実アカウントでの画面確認。
...
> 1本に集約**し、顧客データを返す5本を全部そこ経由にした（規則・並び順・**判定の順序**ともルート層と同一。
> 「1件選んでから停止判定」で、別のメンバーシップへは落ちない）。
> **本番データで新旧を突き合わせ、現行4ユーザは全員そのまま通り、アクセスを失う人は0人。**
> 増えたのは拒否経路だけ。
> あわせて `scripts/replay/checks/*.sql` を再生後の DB に流す段を足し、
> 陽性対照3件・陰性対照6件で振る舞いを毎回確かめるようにした
> （条件をわざと落とすと落ちることも実測済み）。
> **本番へは手で当てていない** —— `db-migrate` が復旧済みなので通常経路で流した。
> **2026-09-20 15:32 UTC に本番適用済み**（PR #1101 マージ `cd04b934` → `db-migrate` #79 成功）。
> 本番で `schema_migrations` の `20260920151600`・`current_insurer_access` の存在・
> 5本の RPC が呼んでいること（5/5）を実測した。
...
> 保留していた `audit_logs` の8列も、代表判断で `20260920154100` により落とした
> （2026-08-23 の `audit_logs_reconcile` が予告していた後始末）。
> **これで列のドリフトは両方向とも 0** —— `public` の実表の列を全部並べた md5 が
> 再生 DB と本番で `09fe8d69088f885e0e754a8ef3ee271d`・3549列で一致する（2026-09-20 実測）。
> 索引も 2026-09-21 に名前で突き合わせた。**差は7本ではなく、本番にだけ 44 本 /
> 再生にだけ 36 本**だった（7 は総数の引き算で、両方向を数えていなかった。
> MISTAKE_LEDGER `M-20260921-reported-a-subtraction-as-a-measurement`）。
> うち**一意制約 10 件は `20260921093300`〜`05` で両方向とも解消**。
> この中に `idx_payments_idempotency` があり、**2026-09-18 以降、本番は同じ冪等キーの
> 決済行を2つ受け入れる状態だった**。残る一意でない索引 70 本は性能の話として保留
> （OPEN_QUESTIONS）。制約・ポリシーの差も未着手。
...
> （`active, void, draft`）なのに、関数は `'expired'` と比べていて **22P02**、
> さらに返り値の `latest_cert_status`（`text`）に enum を返していて **42804**。
> `status::text` にして本番へ適用済み（版 `20260919132119`）。
> **`db-migrate` が止まっているので手で当てた** —— ファイル名を本番の記録版に
...
> **Codex が P1 を4件返し、2件を同じ PR で直した**（`20260919150043`）——
> (1) **下書き証明書が保険会社に漏れていた**。`insurer_search_vehicles` の最新証明書
> サブクエリ3本に status の条件が無く、その `public_id` を `insurer_get_certificate` に
> 渡すと本文まで返っていた（本番に draft は1件）。両方に条件を足した。
> (2) **6引数オーバーロードを落とした**。5引数と両方に一致して PostgREST が
> RPC を解決できない恐れがあったため。呼び出し元0件・呼ばれれば必ず 42883 で、残す理由が無い。
> **これで本番の全 plpgsql 関数で `plpgsql_check` の error は0件になった。**
> 残り2件（停止中の保険会社が RPC を直叩きできる／複数所属ユーザで保険会社の文脈が落ちる）は
...
> 誤検出（登録ルート144件）と**盲点**（未登録ハンドラの走査が withCaller 包みを素通り）が同時に起きていた。
> 構文木でラッパのオプションを読む共通ヘルパーを入れて両方を解消し、**本当に剥がれていた
> AI レート制限 13 本**をテナント/ユーザー単位で復元した。陰性対照も追加済み。
> `vitest` は **5,782 件緑 + 1 件 skip**。**ラッパの `rateLimit` は IP 単位**という性質は残っている【要確認】。

> 2026-09-18 追記: **支給部品を伴う外注施工履歴（発注元 ⇄ 施工事業者）の MVP を実装した。**
> 状態語彙は正準 9 軸目 `OUTSOURCED_WORK_STATES`（通常 11 + 例外・中間 14 = 25 値、
> `src/lib/domain/states.ts`）、遷移表は TR-001〜047 を `OUTSOURCED_WORK_TRANSITIONS` に
> 写し、実行主体（PER-001〜027）と復帰先制御（PER-029）は `src/lib/outsourcedWork/rules.ts`。
> DB は `20260918160000` の4表（作業依頼・支給部品・受領試行・追記専用イベント）。
> **完了後は状態を動かせない**（TR-048。アプリ層とトリガの二重）、**受領は試行ごとに新しい行**
> （拒否前を上書きしない）、**例外承認後の復帰先は発生工程で絞る**（TR-026 の施工中直行禁止を含む）。
> 施工事業者の候補は既存の「外注職人のテナント連携」（`staff_members.linked_tenant_id`）から取る。
> 画面は `/admin/outsourced-work`。**東日本三菱での実運用確認（AC-030）は未実施**【要確認】。
> 写真アップロード UI・店舗単位の閲覧絞り（PER-023）・配送追跡等の対象外機能（AC-031）は入れていない。

> 2026-09-18 追記: **本番スキーマのドリフト検出器が、入れてから一度も動いていなかった。**
> `#1045` で入れた `check-schema-drift.mjs` はシークレット未設定のとき黙って skip する
> 作りで、週次ジョブの該当ステップは 2026-09-14 の実行で **0 秒**（再生だけで数分かかる）。
> `REQUIRE_SCHEMA_DRIFT=1` で落とすようにし、**検出対象に列と RLS ポリシーを足した**。
> 実際に突き合わせた結果: 本番にだけ **`ft_*` 12 テーブル**、列は**本番だけ 15 件 /
> マイグレーションだけ 13 件**、ポリシーは**本番 626 / 再生 607**。
> **`ft_*` はドリフトではなかった** —— 未マージの PR #1093（メーカー実証テスト）の
> schema が本番へ先に入っていたもので、消してはいけない。
> **より重い問題が同時に出た**: 本番の台帳に **main に無い版が7つ**ある
> （#1093 の4版＋`20260917000400`・`20260917100000`・`20260918142610`）。
> **次に main へマージした時点で `db-migrate` が失敗する。**
...
> 4回目は「一括置換は危険」と自分で書いた手順を読んだうえで同じ操作を実行している。
> 既存 104 件を新形式にし、**旧番号は見出し末尾に `旧 M-NNN` として残した**ので、
> 参照 450 箇所は1件も書き換えていない。重複していた10組は対応表で引ける。
> 再発防止は `npm run check:ledger-ids`（CI + pre-commit）。PR #1089。
> **手順書ではなく、衝突した状態をマージさせない検査に寄せた。**
> 2026-09-15 追記: **ダッシュボードヘッダーのモバイルレイアウトを改善した**（PR #1084、
> squash マージ済み）。PageBar の actions ラッパーに `basis-full` / `sm:basis-auto` +
> `sm:ml-auto` のモバイルファースト responsive クラスを適用し、モバイルでは
> コントロール群（スコープ切替・表示モード）が左寄せ全幅配置になるよう修正。
> ダッシュボード固有の `justify-end` も `sm:justify-end` に変更。
> PageBar は全 admin ページ共通のため、全ページのモバイル表示が改善される。

> 2026-09-15 追記: **管理画面全体のモバイルレスポンシブ崩れを修正した**（PR #1076、
> squash マージ済み）。帳票管理を起点に admin 全体（約100ファイル）を調査し、
> 22ファイル・20箇所の崩れを修正。崩れのパターンは5つに集約された:
> (1) `flex-wrap` 欠落で横溢れ (2) モーダル overlay の横余白なし
> (3) 固定列数グリッドが viewport を超える (4) layout と page の二重 `px-4`
> (5) `min-w-0` / `break-words` なしのテキスト溢れ。
> `admin/layout.tsx` の main 要素に `min-w-0` を追加する根本修正で、
> flex 子要素がコンテンツ幅以下に縮小可能になった（全 admin ページに効く）。
> line items グリッド（10列・固定幅合計 736px）のモバイルカード化は
> 設計課題として別 PR に分離（OPEN_QUESTIONS に起票済み）。
...
> ターゲット仮説10個・国内競合4層比較・AITURBO戦略・保険提案ポジショニングを集約。
> 現在地の公式認識: **コード Lv.2.8 / 外から見える Lv.2**。独自軸は改ざん検知・
> 収益還元・保険連携の3つに絞り、汎用的な「施工管理」「AI」「写真」は独自性として
> 主張しない方針。Lv.3 への昇格条件は本番有効化3つ（C2PA証明書・収益送金・保険接続）
> のいずれか1つ。

> 2026-09-12 追記: **システム構成図を正準文書として立てた**
> （`docs/diagrams/system-architecture.md`、#1068）。Mermaid 6図（概要 / モジュール /
> 主系統フロー / 非同期フロー / テナント分離 / ハードウェア配置）とインタフェース一覧、
> および図に書ききれない前提8件。件数はこの日に測り直した —— 画面 301 / API 649 /
> migration 464 / RLS 246 / cron 35 / テスト 562。
> **RLS の 246 は数え方を直した結果**で、既存の監査文書が使っていた未正規化の
> コマンドだと 251 になる（同じテーブルが `public.` 付き・無しの2表記で書かれた
> migration が5件あり、二重計上されていた。MISTAKE_LEDGER M-085）。
> 併せて `docs/implementation/current-architecture.md` §3 の
> 「`middleware.ts` は存在しない / エッジでの一括認証ゲートはない」に追記した ——
> 現在は `src/proxy.ts` がほぼ全パスを `matcher` で拾い、CSRF 検査・CSP nonce・
> レート制限・セッション更新・未ログインリダイレクト・AAL2 ステップアップを担う。
> 監査文書の本文は書き換えていない（日付付きの実測記録として残す。DECISION_LOG 2026-09-12）。

> 2026-09-11 追記: **下の 2026-09-10 追記で「まだ本番は壊れたまま」と書いた
> `insurer_get_certificate` は、本番へ入った。** PR #1051 をマージ（`27e99a2`）、
> `DB migrate` run #69 success。本番の最大版は `20260910010100` で、
> `insurer_get_certificate` の `search_path` が `""` になり、衝突していた参照に
> `ita.` の別名が付いた定義が入っていることを実測確認した。
> ~~**保険会社ポータルの証明書詳細は、これで初めて動く状態になった。**~~
> **【2026-09-18 訂正】この一文は誤り。証明書詳細は今も動いていない。**
> 42702 は消えたが、その1段下に **42703（`column c.certificate_no does not exist`）**が
> 在った。`certificates.certificate_no` は最初期の `20260313020000_core_tables.sql` から
> 全マイグレーションに在るのに、**本番の `certificates` には一度も存在しない**（列レベルの
> ドリフト）。本番で `select c.certificate_no from public.certificates c limit 0` を
> 実行して 42703 を再現済み。読んでいるのは `insurer_get_certificate` と
> `insurer_get_vehicle_certificates` の2関数で、**証明書詳細・車両ごとの証明書一覧・
> CSV エクスポートが揃って落ちる**。修正は `20260918000000`（列を1本足す）。
> なぜ誤ったかは MISTAKE_LEDGER `M-20260918-replay-db-lacked-prod-column`。
> ただし**実際に保険会社ユーザーとして画面を開いて 200 を確認してはいない**
> （そのセッションを用意していない。呼べば `insurer_access_logs` に行が増えるので、
> 本番で試すなら実利用者の操作で確かめる）。同一の定義に対して CI の再生 DB 上で
> `plpgsql_check` が通っていることまでが、いま言える範囲。
> **【2026-09-19 追記】この 42703 は解消した。** `20260918150000` を手で本番へ当てて
> `certificate_no` 列を足し、その下から出てきた 42804 も `20260919134412` で直した。
> 本番の `insurer_get_certificate` / `insurer_get_vehicle_certificates` は、
> `plpgsql_check` で指摘なしになっている（上の 2026-09-19 追記）。
> **画面を開いての 200 確認は、この日も行っていない**【要確認】——
> 上の「いま言える範囲」という線引きはそのまま生きている。
> あわせて `agent_rankings` の**認可チェックが本番で無事であること**も確認済み
> （取り下げ判断どおり上書きは起きていない）。

> 2026-09-11 追記: **typegen の専用トークン対応をマージした**（#1056、`b38a7445`）。
> `db-typegen.yml` が `TYPEGEN_TOKEN` を使えるようになり、**シークレットが登録されれば
> 2箇所の穴（PR が作れない／PR を人が作っても CI が走らない）が両方とも解ける。**
...
> Claude 側から実行できないため代表の操作待ち。** 完了後に `list_branches` で確認する。
> あわせて、モバイルで潰れる固定列グリッドを直した（グリッド36箇所・col-span 4箇所、計22ファイル。#924 の作り直し）。

> 2026-09-13 追記: 滞留していたドラフト PR 3件（#981 / #913 / #965）をマージした。
> いずれもドキュメントのみ。**#981 が記録した「本番の台帳に書く経路が2つある」問題は
> 今も生きている**（Supabase の既定ブランチ `main` が本番プロジェクトに直結したまま）。
> 恒久対策＝Supabase 側の本番自動適用を切るかは**代表判断待ち**。プレビューブランチの
> 詰まりは解消したので、切ったときに失うものは以前より小さい。

> 2026-09-11 追記: **車両履歴の外部公開を許可リストに反転した。** 顧客ポータルに
> 閲覧監査の IP / uid が出ていたのを塞いだ後（本番で14行）、`/code-review` が
> 「除外しているのは23種別中5種別だけ」を指摘。`member_added` にメールアドレスを含む行が
> 本番に1件あり、外に出ていなかったのは `vehicle_id` が NULL だったという偶然だった。
> `Record<AuditEventType, boolean>` で分類を強制し、外へ出すのは発行・編集・無効化の
> 3種別のみにした。**本番で外へ出る行は発行28件・無効化2件で、顧客が見る情報は減っていない。**
> 既存行の IP / uid / メールは DB に残ったまま（扱いは未判断）。

> 2026-09-10 追記【訂正】: **2026-09-08 に書いた「PII 開示判定が本番で動く」は誤りだった。**
> 直したのは関数1本（`is_pii_disclosed`、#1016）で、それを使う**証明書詳細
> `insurer_get_certificate` は別の理由で壊れたまま**だった —— `RETURNS TABLE` の出力列
> `tenant_id` と `insurer_tenant_access.tenant_id` が同名で 42702、`is_pii_disclosed` を
> **呼ぶ手前で落ちる**。本番の `insurer_access_logs` は `action='view'` が**0件**で、
> 保険会社ポータルの証明書詳細は**一度も成功していない**（MISTAKE_LEDGER M-074）。
> **2026-09-10 時点でまだ本番は壊れたまま**（本番の `insurer_get_certificate` の
> `search_path` は `public, extensions` のままで、誰も再作成していないことを実測確認）。
> 修正は PR #1051（版 `20260910010000`）で、マージ後に `db-migrate` が適用する。
...
> **同じ検査で見つかった `agent_rankings`（`date >= text` で 42883、呼び出し元が
> `rpc()` の `error` を捨てているため 200 で空のランキングが返っていた）は、
> 別経路で先に本番へ入った** —— `20260908131725` が型不一致を、`20260909003200` が
> JOIN のファンアウトを直し、あわせて「有効な代理店ユーザーであること」の
> 認可チェックが付いた。本番で認可チェックの存在を実測確認済み。
> このため PR #1051 からは `agent_rankings` の再作成を**取り下げた**（版番号の付け替えで
> 最後に適用されると、認可チェック付きの本番定義を上書きしてしまうため）。
...
> 無関係な既存の未解決事項も多数あり、「全部で4件」ではない）：
> (1) モバイル signup のPKCE Cookie衝突（B-H3是正とTap to Payアプリ内
> オンボーディング要件の緊張関係、2026-09-08起票）、(2) モバイル_layout.tsx の
> Stack.Protected設計変更（2026-09-09起票）、(3) F-4 importer 0 モジュールの
> 削除可否（2026-09-08起票。件数は監査時点の推定値「25本」で、このセッションでは
> 確認できた10本以外は未確定 — OPEN_QUESTIONS.md本文が既にこの数字を採用しない
> よう明記している）、(4) レート制限2系統の完全統合要否（2026-09-08起票）。
>
> なお round 1/round 2 のCodexレビュー対応（追記5・6、計13件の指摘）は
> 「PR-1相当の内容だけ」を扱っていたわけではない。実際には Stripe webhook・
> 予約・AIコストガード等 PR-2 相当の領域、モバイルのサインアウト・レジ・
> pushトークン・ディープリンク等 PR-3 相当の領域にも及んでいた
> （PR #1054 が既に PR-1〜PR-5 全段階を含んでいたため）。13件のうち1件
> （モバイルsignupのPKCE Cookie衝突）は製品判断待ちとして意図的に未修正
> のまま残しており、「全件修正」ではなく**12件修正・1件はfounder判断待ちで
> 保留**が正確な内訳（MISTAKE_LEDGER M-071参照。当初M-070として書いたが
> 並行PR#1055との番号衝突でM-071へ改番済み）。
>
...
> 【状態は追記(8)で更新済み・本エントリの「PRは引き続きレビュー中」は
> 当時の状態でありその後mainへマージされた】
>
...
> のまま維持。【状態は追記(8)で更新済み・本エントリの「PRは現在レビュー中」は
> 当時の状態でありその後mainへマージされた】

> 2026-09-08 追記(4): **重複圧縮（PR-5）も完了。全体セキュリティ監査の
> 是正シリーズ（PR-1〜PR-5）が完結。** 未使用依存削除、二重実装の
> `resolveCallerFull` 廃止（副産物で template-options 系6ルートの認可漏れを
> 発見・修正）、Redisクライアント4本の共有シングルトン統合、紛らわしい
> 重複名ディレクトリ/ファイルの整理、CSV export 4ルートの formula
> injection 対策漏れの統合修正、JSTオフセットのマジックナンバー統合、
> `sendResendEmail` 直呼び10箇所の `sendEmail()`（Resend→SendGrid
> フォールバック付き）への統一、新規ルート用の `withCaller` ヘルパー新設。
> レート制限2系統の完全統合・日付フォーマッタの完全統合・未接続モジュール
> 群の削除可否は、セキュリティ影響とリスクを理由に見送り OPEN_QUESTIONS に
> 起票（無理な一括統合は行わない判断）。
>
> 2026-09-08 追記(3): **AI/クローラ露出と衛生（PR-4）も完了。**
> `/agent`, `/manufacturer`, `/my` 等が robots.txt に漏れていた索引可否の
> 既定を反転（マーケティングページのみ明示的に索引可）、`/c/[public_id]`
> の索引拒否を二重固定、CSPのSupabaseワイルドカードを自プロジェクトに
> 限定、Sentryのauthorization/cookieヘッダースクラブ追加、gitleaksをCIに
> 導入（全履歴を実走査し偽陽性38件を確認したうえでallowlist設定）。
> **未完了: PR-5（重複圧縮）のみ。**
>
> 2026-09-08 追記(2): **モバイル修正（PR-3）も完了。** レジ機能が
> `stores.id`/`registers.id` の取り違えで常に動かなかった不具合、車検証
> OCR が cookie 専用ルートを叩いて常に401だった不具合、「店舗なしで続行」
> 時の空文字 store_id で予約・証明書作成が失敗する不具合、アプリロックの
> 生体認証2回失敗で出る無認証バイパス、未認証ディープリンクのガード欠落等
> 11項目を是正。**未完了: PR-4（AI/クローラ露出と衛生）〜PR-5（重複圧縮）。**
>
> 2026-09-08 追記: **全体セキュリティ監査（Web + モバイル）の是正を実施中。**
> PR #1054（ブランチ `claude/ledra-security-audit-zeu9rp`）で Critical 1件・
> High 11件の是正（PR-1）と、課金・予約・cron の整合性是正8項目（PR-2）が完了。
> 主な是正: 組織メンバー追加で任意テナントの顧客PIIを横断閲覧できた穴（Critical）、
> 代理店運営APIが任意テナント管理者に開放されていた穴、保険会社登録OTPの迂回、
> IPレート制限の偽装ヘッダ迂回（本番はVercel直配信を前提、Cloudflare前段構成向けの
> opt-inフラグの優先順位バグも `/code-review` で発覚し是正済み）、パスワード登録が
> メール所有確認なしで即ログインできた穴、支払停止テナントの課金猶予が計算されて
> いなかった不具合、モバイル6箇所の監査ログがRLSで黙って弾かれ1件も残っていな
> かった不具合。**未完了: PR-3（モバイル）〜PR-5（重複圧縮）。**
> `/c/[public_id]` は索引させない方針（PR-4で `robots: noindex` を明示予定）。
> 詳細は DECISION_LOG 2026-09-08 各エントリ・RELEASE_LOG 2026-09-08。

> 2026-09-08 追記: **本番へのマイグレーション自動適用は復旧済み。out-of-order 検査の
> 見逃しも塞いだ**（PR #1044 / `2868e397`）。検査は「base（main）の最新 >= 本番の最新」を
> 前提に base とだけ比べていたが、`apply_migration` で本番へ直接当てた版は main を
> 通らないためこの前提は偽で、**#1020 の4本を見逃して19時間の本番停止を起こした**。
> しきい値を `supabase/migrations.production-ledger`（本番台帳の要約）と base の
> **大きい方**にし、本番が適用済みの版だけを**中身の sha256 ごと**免除する形に変えた。
> `db-migrate.yml` に台帳の古さを検出するステップも追加（`db push` の後に置き、本番への
> 適用そのものは止めない）。詳細は DECISION_LOG 2026-09-07、MISTAKE_LEDGER M-064 / M-065。
>
> **型の再生成（`src/types/db.generated.ts`）も main に入った**（PR #1049 / `37dbe4f6`）。
...
> **~~未解決（別項）~~ → 2026-09-10 解消**: 本番と migrations で**列の型が違う**件。
> `tenants.plan_tier` は本番が enum（5値）、migrations は `text check`（3値）で、
> **本番 24 テナント中 20 件が check に弾かれる**状態だった。
> **PR #1052（版 `20260910000000`、マージ `662e46f`）で check を enum と同じ5値へ広げ、
> 本番適用も成功**（`DB migrate` run #68）。本番は元から check を持たないので no-op で、
> 効くのは空 DB からの再生。**型名**の食い違い（text か enum か）は意図的に残した。
...
> （DECISION_LOG 2026-09-07、MISTAKE_LEDGER M-064）。
...
> 詳細は RELEASE_LOG 2026-09-06、MISTAKE_LEDGER M-045 / M-065。
