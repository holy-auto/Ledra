...
> 2026-09-06 追記: **本番へのマイグレーション自動適用が止まっている。** #1020 のマージ
> （`6bc745f7`、12:57 UTC）から `DB migrate (apply to production)` が赤。#1042
> （`b82e2a05`）で out-of-order（不変条件2）は直したが、**不変条件1 が残っている** ——
> `20260906094512` / `20260906094735` が本番の `schema_migrations` にあるのに、
> ファイルが main に無い。**#966 が両方を持っているので、#966 のマージでしか解けない。**
> ここで同じファイルを足すと版番号が重複するため触らない。
> **止まっている間、以降のスキーマ変更は本番に届かない。** 同じ形で 2026-08-02〜08-15 に
> 13日間止まり、証明書発行が全件停止している。
