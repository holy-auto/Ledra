...
> 2026-09-03 追記(7): **職人の実績を店舗を跨いでまとめられるようにした（未マージ、PR #1020）。**
> 制約は代表判断の「**他社に自分の稼働先が見えないように**」。束ねられるのは両方の
> リンクのトークンを持つ**本人だけ**で、束ねの情報はテナントから読めないテーブル
> （RLS `USING (false)`）に置いた。`staff_portfolio_links` には何も足していない —
> 印を1つでも置くと「非 NULL であること自体」が漏洩になるため。各テナントの見え方は
> 束ねても変わらない。運用ルールではなく構造で守る形にしてある。

> 2026-09-03 追記(6): **職人が自分の施工実績を確認できるようにした（未マージ、PR #1020）。**
> 外注職人はログインを持たない設計なので、テナントが発行する読み取り専用リンク
> `/w/[token]` を追加した。失効はロスターの在籍状態に相乗りさせてあり、「休止中」に
> するとリンクが自動的に死ぬ（離職時に別作業が要らない）。顧客 PII は出さない。
> **残る本丸**: 複数の元請けに登録された同じ職人の通算実績は依然として出せない
> （`staff_members` が元請けごとに別行）。名寄せには「他社に自分の稼働先が見えてよいか」
> の同意設計が先に要る。
