...
> 2026-09-01 追記(2): **業務データの変更系47ルートにサーバ側の認可を入れた。**
> 認可チェックの無い変更系ルートは 86本 → 39本。証明書・車両・顧客・予約・マーケット・
> 受注は権限マトリクスの動詞で、在庫・発注・部品・工程テンプレート・ショップ受注は
> 動詞が語彙に無いのでロール下限（staff 以上）で守る。**本番で書き込みを失うユーザーは
> いない**（owner 23 / staff 1 / super_admin 1、viewer・admin は 0 名）。
> 証明書の発行だけは Server Action `createCertAction` の中にガードを置いた —— Web の
> 発行画面と `/api/admin/certificates` の共通の入口がそこだから。
> 残り39本のうち、アカデミー14・決済3・設定2・その他3の計22本は方針が未決。
> 在庫・発注・部品・受注更新に専用の Permission を作るかも未決（OPEN_QUESTIONS.md）。
