...
- **レポート収益の施工店還元（2026-07-30, PR #848 + 後続）**: 有料の車両全履歴レポート売上を、記録を残した施工店へ**記録件数に比例して按分**し蓄積台帳（`vehicle_report_revenue_shares`）に計上。還元率は設定値 `merchant_share_bps`（既定 70%）。施工店ポータル `/admin/report-revenue` で「あなたの記録が生んだ収益」として可視化し**「技術が、資産になる。」を実体化**。後続で**実送金（Stripe Connect 精算）と返金巻き戻し**も実装: 人手承認（platform-admin）した `approved` の share を cron `/api/cron/vehicle-report-payout` が送金し、connect-webhook `transfer.paid/reversed`・メイン webhook `charge.refunded` で確定/巻き戻す（代理店コミッション精算と同型）。**還元率70%は【要確認】で、承認しない限り送金は走らない**安全弁付き。詳細は DECISION_LOG / RELEASE_LOG 2026-07-30。
