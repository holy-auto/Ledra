...
> 保険会社ポータルの証明書詳細は**一度も成功していない**（MISTAKE_LEDGER M-064）。
...
> 1本。**適用後は本番で `false` を返すことを実測済み**（適用前は 42P01）。
> **この壊れ方は `CREATE` では作れず、`ALTER FUNCTION ... SET search_path` でだけ入る**
> （ALTER は本体を再検証しない）ため、マイグレーションも型検査も CI も素通りしていた。
> 名指しの修正で取りこぼしが2回続いたので、**`scripts/replay-migrations.mjs` が空 DB への
> 再生後に `search_path=""` 付き SECURITY DEFINER 関数の定義を1本ずつ流し直す**検査を常設
> （判定は Postgres の `check_function_bodies` にさせる）。**未検査の領域**: plpgsql 本体の
> 名前解決・動的 SQL・ビュー／トリガは同じ検査では見えない（OPEN_QUESTIONS）。
