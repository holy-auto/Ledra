...
**マージ前の検証**: `ci-parallel-checks.sh` 9/9・`check:migrations` 再生 506/506・
振る舞いの検査 **6 件**。あわせて陰性対照を4つ実測（語彙外の値で tsc が落ちる／
一覧のずれを検出する／CHECK の値集合の不一致を検出する／担当候補のガードを外すと
テストが落ちる）。**これらはすべて手元と CI で、本番には触れていない。**

**本番適用 実測（2026-09-24 15:50 UTC）**: `db-migrate` run 90 成功
（15:50:09→15:50:34 UTC）。本番で `insurer_users.is_system` 列・システム行 2社×1行
（`user_id` を持つものは0）・形の CHECK `convalidated: true`・部分一意索引・挿入トリガ・
適用台帳4版を確認。**定義を読むだけでなく実際に書き込んで**確かめ、2社 × 3種の
自動処理 action が **6件すべて成功**、`insurer_user_id` を渡さない古い形は今も **23502**
