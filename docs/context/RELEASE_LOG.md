...
- コード修正3件:
  ...
  - `createApplication`: `UNIQUE(recruitment_id, tenant_id)` 違反（23505）を型付きエラー `FT_DUPLICATE_APPLICATION` にし、admin/mobile 両ルートで明確な 4xx に変換（不透明な 500 を解消）。
- 検証: `lint:migrations` OK、`check:migrations` 再生 493/493・RLS 打ち消しなし、`tsc --noEmit` エラー0、`fieldTest` テスト14件パス（createApplication 重複マッピングの回帰2件を追加）、変更ファイル eslint エラー0。
- 未対応（本PR外・要別対応）: evidence_submitted 通知の宛先（現状は提出元テナント自身。レビューするメーカー側に届けるにはメーカー通知チャネルが必要で、既存の tenant-keyed `notifications` では表現できない＝別設計）。issue #1117 に残す。
