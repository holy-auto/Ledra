...
**3. 新規2本（どちらも既存のライブデータから生成、本部の差し替え不要）。**
    ...
    運用スタートガイド  ← OPERATION_GUIDE_GROUPS（HelpDrawer / /guide と同じ 22 項目）
    自動車施工・記録の用語集 ← GLOSSARY（4カテゴリ 20 語）

全資料 ZIP・代理店ポータルの「常に最新の商品資料」欄・リード自動返信は
すべてレジストリ駆動なので、追加のみで自動的に反映される（ZIP は 8 本で 4.7 秒 / 1.0MB）。

**4. デザイン。** DESIGN_SYSTEM.md のダークトークンに揃えた
（`--bg-surface-solid #0d1525` をカード地に、アクセントを `#60a5fa` → `--accent-blue #4d9fff`、
章扉の罫とタグラインに `--accent-gold #b08d3f`）。章扉の罫は全幅の暗いトラックの左 64pt だけを
アクセント色にした2色帯。描画オペレータを実際に読んで意図どおり出ていることを確認済み。
