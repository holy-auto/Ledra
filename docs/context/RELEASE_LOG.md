...
**接頭辞なしの固定列を全件数えてから、1件ずつ分類した。** 対象範囲で 55 箇所あり、
**35 箇所 / 19 ファイル**を修正、**20 箇所は固定が正解**として残した。

| 変更 | 対象 |
|---|---|
| `grid-cols-1 sm:grid-cols-2` | `Field` + `input` / `select` を並べたフォーム 26 箇所 |
| `grid-cols-1 sm:grid-cols-3` | 3列のフォーム 4 箇所・統計カード行 3 箇所 |
| `grid-cols-2 sm:grid-cols-4` | 4列の実績タイル 2 箇所 |
...
ダッシュボード）と固定レイアウトのスライドなので、対象範囲から外した。

**`PageBar`**: 外側の行には別経路で `flex-wrap` が入っていたが、**アクション群の内部は
折り返さない**（`shrink-0` かつ `flex-wrap` なし）ままだった。ボタンが複数あると
横にはみ出すので `flex-wrap` を足した。
...
**回帰検査** `src/lib/__tests__/responsiveGrids.test.ts`。接頭辞なしの固定列を
**全件数えて、意図的なものの一覧と突き合わせる**。フォームに素の固定列を足すと
落ちる。変異2通り（修正を戻す／新しい素の固定列を足す）で赤を確認済み。
