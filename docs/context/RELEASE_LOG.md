...
- **#1020 をマージしたら `DB migrate (apply to production)` が赤くなり、本番への
  ...
  - **不変条件1 — 未解決**。`20260906094512` / `20260906094735` のファイルが main に
    無い（`Remote migration versions not found in local migrations directory.`）。
    **#966 が両方を持っているので、そちらのマージでしか解けない。**
    ここで同じファイルを足すと版番号が重複するため触らない。
...
- **残ること**: #966 のマージ、その後 `npm run db:typegen`（#1020 の4本が本番へ
  適用された後）。**それまで本番の適用は止まったまま**で、以降のスキーマ変更は届かない。
