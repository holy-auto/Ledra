...
## 2026-08-30 IMP-026（#941）を main へ取り込み。実在しない列名の参照を修正、resurrection バグを6度目の再削除

- 内容: IMP-026（顧客懸念提起フロー、branch impl/IMP-026-customer-concern）を main へ取り込む際、
  マージ後の標準検証（`npm run check:schema`）で本PR自身のバグを発見・修正。
  (1) `src/app/api/customer/concerns/route.ts` が `part_confirmation_signatures` テーブルの
  存在しない列 `part_installation_id` を SELECT していた（実列名は `installation_id`） —
  本番デプロイ後は `parts_confirmation` 経由の懸念提起が PostgREST の 400 で失敗する状態
  だった。列名を修正。
  (2) 本PR自身が新設した `customer_concerns` テーブルが `scripts/schema.snapshot.json` に
  未登録で、テーブル参照全件が「存在しない」として検出されていた — マイグレーション DDL
  から列名を書き起こして登録。
  加えて、IMP-024/025 と同じ squash 履歴の断絶で `src/lib/sync/`・`WorkScopeProvider.tsx` が
  6度目の復活をしていたため再削除。
- 対象: `src/app/api/customer/concerns/route.ts`、`scripts/schema.snapshot.json`。詳細は
  DECISION_LOG「IMP-026（#941）を main へ取り込み。check:schema が実在しない列名を検出」参照。
