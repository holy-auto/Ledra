...
- 実装: 記事ドラフト通知（`notify-article-pr.yml`）が既に使っている `SLACK_WEBHOOK_URL` を流用し、
  ...
  通知は `continue-on-error: true` にして、Slack 側の障害でジョブの結論を緑に見せないようにした。
...
- 検証: ワークフローYAMLから当該ステップの `run` ブロックを実際に取り出し、`curl` をスタブ化して実行。
  (1) 正常系＝有効なJSONを生成し、複数行コミットメッセージから1行目だけを抜くことを確認。
  (2) `SLACK_WEBHOOK_URL` 未設定＝curl を呼ばず exit 0 になることを確認。
