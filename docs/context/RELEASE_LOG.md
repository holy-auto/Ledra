...
- `posCheckoutSchema` に `stripe_payment_intent_id` を追加。`qr-status` と
  `checkout-session` の状態取得が PaymentIntent を返すようにし、記録時に渡す。
- モバイルは PaymentIntent を ref に保持し、**やり直しでも同じ値を送る**
  （送り直すたびに変わると重複防止が効かない）。
