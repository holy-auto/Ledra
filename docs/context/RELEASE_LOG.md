...
`insurer_access_logs` に `insurer_user_id` を渡しておらず（NOT NULL・既定なし）、
**毎回 23502 で落ちていた**。本番で実測。`.then(() => {})` で戻り値を捨てていたので無言。
#1135（CHECK を 20 値へ）はこの3本を直していない —— 語彙には入ったが NOT NULL で先に落ちる。

**語彙の単一定義源**

- `src/lib/insurer/auditActions.ts`: 正準語彙 20 種 + 共有ヘルパー `recordInsurerAccessLog`。
  `action` は union 型なので、語彙外の値は **tsc が落とす**（陰性対照で実測）
- TypeScript の直 insert **10 箇所**をこのヘルパーに寄せた。**どこも `error` を捨てなくなった**
- `npm run check:audit-actions`: 型の一覧と再生検査の一覧のずれを落とす（CI 9 本目）
- 再生検査を強化: CHECK の値集合との**完全一致**（広げ忘れ・広げすぎの両方を見る）と、
  この表に insert する SQL 関数が既知の6本のままであること

鎖は「型 → check:audit-actions → 再生検査の一覧 → 本物の Postgres の CHECK」で1本に繋がった。

**自動処理のシステム行**（代表判断）

保険会社ごとに `insurer_users` へ1行（`is_system = true` / `user_id IS NULL`）。
`user_id` を NULL にできるのはこの行だけ（CHECK で縛る）。RLS のポリシーはすべて
`user_id = auth.uid()` の等値比較なので、**この行は誰にも権限を与えない**。
`insurers` への挿入トリガで自動的に作られる。人の一覧（利用者管理・担当候補・
プラットフォーム集計）からは除外した。監査ログの絞り込みには表示名として出る。

- `20260924133000` 列（`user_id` を NULL 可・`is_system`・形の CHECK / NOT VALID）
- `20260924133100` 部分一意索引（CONCURRENTLY・1文1ファイル）
- `20260924133200` 既存2社の埋め戻しと挿入トリガ
- `20260924133300` VALIDATE

検証: `ci-parallel-checks.sh` 9/9・`check:migrations` 再生 506/506・振る舞いの検査 **6 件**。

**本番適用 実測（2026-09-24 15:50 UTC）**: `db-migrate` run 90 成功。本番で
`insurer_users.is_system` 列・システム行 2社×1行（`user_id` を持つものは0）・形の CHECK
`convalidated: true`・部分一意索引・挿入トリガ・適用台帳4版を確認。
**定義を読むだけでなく実際に書き込んで**確かめ、2社 × 3種の自動処理 action が
**6件すべて成功**、`insurer_user_id` を渡さない古い形は今も **23502** で落ちる（陰性対照）。
`RAISE EXCEPTION` で全件ロールバックし、表の行数は2件・最新 2026-09-03 のまま。
陰性対照を4つ実測（語彙外の値で tsc が落ちる／一覧のずれを検出する／CHECK の値集合の不一致を
検出する／担当候補のガードを外すとテストが落ちる）。


## 2026-09-24 工賃の1円未満を切り上げに変更

工賃（工数 × 時間単価）の1円未満を、四捨五入から切り上げに変えた（`calcLaborPrice`、代表判断）。帳票の「工賃を計算」と、
品目マスタの提供価格（レバーレート保存時の再計算・品目の保存）の両方に効く。小数の誤差で1円多くならないよう、
...
データを表示・出力していた。`tenant_memberships` を `limit(1)` で直接引いていた 18 ファイル
（ページ 11: 証明書一覧・顧客詳細・案件詳細・車両一覧・設定・ブランド設定・監査ログ・部品真正性・膜厚レポート一覧/詳細・車両の膜厚詳細、
ルート 7: 証明書 CSV/PDF 出力 5 本・帳票 PDF・請求書 PDF）と `adminFeatureGate` を、
選択中テナント（`active_tenant_id`）を尊重する `resolveCallerWithRole` に統一した。
`src/app/admin` が `tenant_memberships` を直接引いたら落ちるテストを追加（修正前のコードで落ちることを確認）。
...
発注書の写真の読み取り（OCR）で TC コードも読むようにした。書かれていれば、帳票画面の工賃計算の TC 欄と、
備考の「車両:」行に入る。TC 欄は帳票フォームが持つ値になり、手で直すこともできる。書かれていなければ空のままで、
これまでどおり型式だけで計算する。
...
列・トリガでずれていた。`20260920120500` で寄せた。**本番では中身が変わらない**（落とす対象は
既に無く、足す対象は既に在る）。形が変わるのは再生 DB と新しいプレビュー分岐だけ。
ただし**「1文も実行されない」わけではない** —— ④ のビュー置き直し（`CREATE OR REPLACE VIEW`）は
本番でも実際に走る。だから `WITH (security_invoker = on)` の有無が本番に効く（下記）。
...
**本番適用（2026-09-20 13:15 UTC）**: `db-migrate` run **#78 成功**（push / `db219e3`）。
適用後に本番へ問い合わせて確認した実測値 ——
台帳の最大版 `20260920120500`・総数 **483**、`invoices` の `reloptions` は
**`{security_invoker=on}`**（剥がれていない）、`invoices` の列 **25**、
`audit_logs` の列 **12**（対象外にした8列は本番でも足していない）、
`tenants.updated_at` は **0 件**（DROP が予告どおり no-op）。

## 2026-09-19〜20 本番へのスキーマ適用を復旧させた（改名を戻し、実在する版は免除欄で追認）

`db-migrate` は #1094 のマージから **4回連続で失敗**していた（run #72・#73・#74・#76）。
止めていたのは2種類だけで、どちらも「本番の台帳にあって main に無い版」。

- **#1098**: #1093 のマージで7版（`ft_*` 5版 + `workshop_capability_profiles` + `remote_schema`）が
  `20260919000000`〜`20260919000600` へ**改名された状態**で main に入っていた。本番に実在する版の
  改名は `db push` を止める。元の版番号へ戻した。
  改名のままだと (1) `lint:migrations` が7本とも赤、(2) 本番の台帳に残る版のファイルが main から
  消えたままで復旧しない、(3) 再適用が走れば `DROP ... IF EXISTS` の無い `CREATE POLICY` /
  `CREATE TRIGGER` **41文**が本番の既存オブジェクトと衝突して 42710、の3つが起きる。
- **#1097**: 保険会社 RPC の3版（`20260919132119` / `20260919134412` / `20260919150043`）を
  `supabase/migrations.production-ledger` の免除欄へ sha256 つきで追認。競合していた
  `docs/context` 6ファイルは両方の記録を残す形で解消した。
- **`workshop_capability_profiles` が本番に無かった**。台帳は `20260917000400` を適用済みと
  記録しているのに表が無く（`pg_class` を全スキーマで確認）、`db push` は適用済み版を
  再実行しないので通常の経路では永久に作られない。`20260917000400` は書き換えず、
...
`M-20260919-wrote-a-replay-count-i-never-read`。
...
シークレット未設定のとき `exit 0` で skip する作りだった。週次ジョブ
`Supabase Advisors` の「Check schema drift」ステップは **2026-09-14 の実行で 0 秒**
（再生だけで数分かかるので、走っていれば 0 秒にはならない）。**入れてから一度も
本番と比べていない。** plpgsql 検査には `REQUIRE_PLPGSQL_CHECK` で同じ穴を塞いで
おきながら、こちらは塞いでいなかった。

- **`REQUIRE_SCHEMA_DRIFT=1` を週次ジョブに渡し、シークレットが無ければ落とすようにした。**
  手元とフォークは従来どおり skip する。
- **検出対象に「列」と「RLS ポリシー」を足した。** どちらも `表名.名前` で突き合わせる。
  表ごと無いときは列・ポリシー側を黙らせ、件数だけ出す（`ft_*` 12 表で 150 行以上の
  重複になり、本当に見るべき差が埋もれるため）。
- **pg_dump から列を拾うパーサを、再生 DB の `pg_attribute` と突き合わせて検証した。**
  最初の版は `GENERATED ALWAYS AS (CASE WHEN … ELSE … END)` の折り返し行を列と誤認し、
  4件多く拾っていた（`service_reminders.when` など）。括弧の深さを追う形に直し、
  **列 3309 件・ポリシー 607 件で差分ゼロ**になることを確認してから本体へ入れた。

### 出てきたもの

| 種類 | 中身 |
|---|---|
| テーブル | `ft_*` 12 本が本番にだけ在る（全件 0 行）。**【訂正】ドリフトではなく、未マージの PR #1093 の schema が本番へ先に入っていたもの**。消してはいけない |
| 列 | **本番にだけ在る 15 件 / マイグレーションにだけ在る 13 件**（数え直し済み） |
| ポリシー | 本番 626 / 再生 607。`ft_*` の 12 本のほか、`certificates` に 3・`insurer_access_logs` に 2 など |

### そのうちの1件が、本番で動いていない機能だった

**`certificates.certificate_no` が本番に存在しない。** この列は最初期の
`20260313020000_core_tables.sql` から全マイグレーションに在る。本番で
`select c.certificate_no from public.certificates c limit 0` を実行して **42703** を再現。

読んでいるのは `insurer_get_certificate` と `insurer_get_vehicle_certificates` の2関数で、
**保険会社ポータルの証明書詳細・車両ごとの証明書一覧・CSV エクスポートが揃って落ちる。**
9/11 に「証明書詳細が初めて動く状態になった」と書いたのは誤りで、42702 を直した1段下に
これが在った（MISTAKE_LEDGER `M-20260918-replay-db-lacked-prod-column`）。

`20260918150000` で列を1本足す（nullable・既定値なし＝本番は表の書き換え無し。再生側は
既に在るので no-op）。再生 DB でわざと列を落として **2関数とも 42703 を再現**し、
このマイグレーションを当てると **2関数とも通る**ことを確認済み。

### さらに、本番の台帳に main に無い版が7つあった

`20260917000000`〜`20260917000300`（未マージの PR #1093）、`20260917000400`、
`20260917100000`、`20260918142610`（今日）。**次に main へマージした時点で
`db-migrate` が「ローカルに無い版が本番にある」で失敗する。** 2026-09-07 に19時間
止まったのと同じ不変条件1の再発で、規模が大きい。
あわせて `migrations.production-ledger` が 2026-09-06 で止まっており
...
（`DB migrate` run #69）。本番で確認した状態:

- 最大版 **`20260910010100`**（`20260910010000` / `20260910010100` の2本とも適用済み）
- `insurer_get_certificate` の `search_path` が **`""`** になり、衝突していた参照に
  `ita.` の別名が付いた定義が本番に入っている（~~＝**保険会社ポータルの証明書詳細が
  初めて動く状態になった**~~ →
  **【2026-09-18 訂正】動いていない。42702 の1段下に 42703 が在った。詳細は
  2026-09-18 のエントリ**）
- `agent_rankings` の**認可チェックは無事**（`agent_users` の判定が定義に残っている）。
  このPRから取り下げた判断どおり、上書きは起きていない
- `vehicle_histories.updated_at` は元から在り、`20260910010100` は予告どおり no-op

**未確認**: 実際に保険会社ユーザーとして証明書詳細を開いて 200 が返ることまでは
確かめていない（そのセッションを用意していない。呼べば `insurer_access_logs` に
行が増えるので、本番で試すなら実利用者の操作で確かめる）。静的には、同一の定義に
対して CI の再生 DB 上で `plpgsql_check` が通っている。
...
- **修正**（**`20260910010000`**。起票時は `20260908005952`。本番の最大版が
  ...
  まま残っており、リポジトリの lint にも違反していた）。**2026-09-11 に本番適用済み**
  —— 2026-09-07 に決めたとおり PR をマージして `db-migrate` に任せた（run #69 success）。
...
- **書き起こし**（`20260907010100`）関数 18・トリガ 3・ビュー 1・enum 型 5・
