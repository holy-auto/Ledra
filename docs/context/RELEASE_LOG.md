...
## 2026-09-15 MISTAKE_LEDGER の ID を日付＋スラッグ方式に変更し、重複検査を CI に追加

- MISTAKE_LEDGER の見出し ID を連番 `M-NNN` から `M-<YYYYMMDD>-<スラッグ>` に変更。
  既存 102 件を全件書き換えた（例: `M-060` → `M-20260908-null-made-guard-fail-open`）。
- **旧番号は見出し末尾に `旧 M-NNN` として残した。** 本文・他ファイル・コード内コメントからの
  参照 438 箇所（台帳の外 227 / 台帳本文 211）は**1件も書き換えていない**。
...
- `scripts/check-ledger-ids.mjs` を新設し、`npm run check:ledger-ids` で実行。
  CI の並列チェック（8本目）と pre-commit フックに登録。落ちる条件は4つ:
  書式から外れた `## M-` 見出し／新 ID の重複／旧番号の重複が10組を超える／
  エントリ数が下限（102件）を割る。
- **検査自体のテストを置いた**（`scripts/__tests__/check-ledger-ids.test.ts`、10件）。
  陽性対照3件に加え、**実物の台帳に1箇所ずつ壊れを入れる陰性対照を7件**:
  重複 ID・旧形式の残存・大文字スラッグ・表題の付け忘れ・旧番号の重複11組・
  見出し書式の変更・エントリ3件削除。「無傷で OK が出る」だけだと、
