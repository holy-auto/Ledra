...
- 演出: 2.0秒のロゴスティング動画（Lマーク → LEDRA のロックアップ）を `expo-video` で1回だけ再生する。
  ...
  - `src/app/_layout.tsx:88` にあった `if (!isReady) return null;`（起動処理中に何も描かない空白）を
    ...
    先に呼ぶとデコード待ちの黒画面が挟まる。
  - ネイティブスプラッシュ画像は**動画のフレーム0から生成**しているので、静止画→再生の継ぎ目が出ない。
- アセット生成 (`scripts/build-mobile-intro.sh` 新規、ffmpeg のみで完結):
  ...
  - スプラッシュ画像はフレーム0から `delogo` で L マークの芽（bbox 60x76）を消して無地にした。
    ネイティブスプラッシュは JS バンドルのロード中ずっと出たままなので、極小の芽が数百ms固まって
    見えると描画バグに見えるため。消したのは 10260画素中15画素で、ビネットは動画と一致したまま。
  - 成果物: `ledra-intro.mp4`（1080x1920 / 2.00秒 / 60フレーム / 音声なし / 267KB）、
    `splash-icon.png`、`icon.png`（iOSはアルファ不可なので白でフラット化）、
    Androidアダプティブアイコンのフォアグラウンド／モノクロ。`android-icon-background.png` は削除。
