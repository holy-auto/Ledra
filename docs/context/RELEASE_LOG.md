...
## 2026-09-03 職人の実績を店舗を跨いでまとめられるようにした（本人だけが見える束ね）

- 背景: 同じ日に入れた実績リンクは1テナント分しか出せなかった。同じ職人が複数の元請けに
  登録されると `staff_members` は元請けごとに別行になるため、通算の実績が出せない。
  複数の元請けの下で動く職人ほど自分の実績が散らばる構造だった。
- 制約（代表判断）: **他社に自分の稼働先が見えないこと。** A社の元請けが
  「この職人はB社でも働いている」と分かってはいけない。これが設計を決めた。
- 内容:
  - `staff_portfolio_identities` / `staff_portfolio_identity_members` を追加
    （`20260903000001`）。**両方ともテナントからは読めない**（RLS `USING (false)`、
    `customer_sessions` と同じサービスロール専用）。
  - **束ねを `staff_portfolio_links` の行に持たせない。** `identity_id` 列を生やすと
    「非 NULL であること自体」が他所でも働いている事実の漏洩になる（テナントは自分の行を
    `members:manage` で読める）。だから別テーブルに出した。
  - 束ねられるのは**両方のトークンを持っている本人だけ**。`/w/[token]` の画面で他店の
    リンクを貼る（`POST /api/staff-portfolio`）。店舗側には操作も表示も無い。
  - 外すこともできる（一度束ねたら戻せない仕組みは本人にとって取り返しがつかない）。
  - 閲覧記録は**開いたリンクにだけ**付ける。束ねた他店の `last_viewed_at` を動かすと
    「本人が別経路で見た」ことがその店に伝わってしまう。
- 結果として各テナントから見えるものは前日の実装から**何も変わらない**。増えたのは
  「本人が自分の画面で複数店舗の実績を並べて見られる」ことだけ。
- 番人（`portfolioLink.test.ts`、いずれもわざと壊して落ちることを確認）:
  許可リストとの完全一致 / craftsman とテナントの絞り込み / 在籍チェック /
  **束ねを links 行に置かない** / **閲覧記録を束ね全体に広げない** /
  開いたリンクが失効していれば束ねていても見せない。

## 2026-09-03 職人が自分の施工実績を確認できるようにした（読み取り専用リンク）

- 背景: 外注職人はログインアカウントを持たない設計（`staff_members.user_id` は外注では
  null、`20260617000002`）。Web もモバイルも `tenant_memberships` 前提なので、
  **施工した本人が自分の記録を確認する手段が Ledra 上にひとつも無かった**。
  本人が見られるのは公開証明書の URL を自分で控えていた場合だけ。証明書には
  `craftsman_staff_id` が刻まれている（`20260617000004`）ので材料はあり、
  欠けていたのは本人へ渡す導線だけだった。
- 内容:
  - `staff_portfolio_links` を追加（`20260903000000`）。雛形は `customer_intake_links`
    （テナント発行・再利用可・無効化可能なトークンリンク）。職人1人につき1本。
  - 公開ページ `/w/[token]`。ログイン不要で、その職人が `craftsman_staff_id` に
    入っている証明書の一覧を出す。各行は公開証明書 `/c/[public_id]` へのリンク。
  - `/admin/staff` の各行に「実績リンクを発行 / 無効化」を追加。**raw token は発行
    レスポンスにしか出ない**（DB はハッシュのみ）ので、発行直後に一度だけ表示する。
    紛失時は再発行。
- 失効（「離職したらどう止めるか」への答え）: 有効条件は **token 一致 AND
  `link.is_active` AND `staff_members.is_active`** の3つ。ロスターで「休止中」にすると
  リンクが自動的に死ぬので、離職時に別の作業を覚える必要がない。
- PII: このページは顧客 PII を出さない（取得列は `public_id, service_type, created_at`
  の3列のみ）。職人が施工時に顧客を知っていたとしても、リンクは退職後も手元に残りうる
  ため恒久的な顧客名簿にしない。番人は `src/lib/staff/__tests__/portfolioLink.test.ts`
  で、許可リストとの完全一致・craftsman とテナントの絞り込み・在籍チェックの
  いずれが消えても落ちる（わざと壊して確認済み）。
- 残る穴: **これはテナント内の実績に限る。** 同じ職人が複数の元請けに登録されると
  別々の `staff_members` 行になるため、通算の実績はまだ出せない（OPEN_QUESTIONS）。
