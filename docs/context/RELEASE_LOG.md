...
- 通した経路（9箇所）: 証明書の作成（`createCertificate`）／予約4経路
  （管理・お客様予約・取引先 API・モバイル）／POS の売上記録（現金・タッチ決済・
  カード番号入力・QR がすべて `recordPosSale` を通る）／顧客登録の招待とリンク／
  レジの登録。
- **他テナントの店舗 ID を通さなくなった。** `store_id` の外部キーにテナントの
  条件が無いため、`/api/mobile/reservations` は送られた店舗 ID をそのまま
  書いていた（他店の行として記録できた）。同じ穴が顧客登録の招待・リンクにもあった。
