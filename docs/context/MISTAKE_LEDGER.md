...
| **C. 経路を1本しか見ない** | 同じ操作に複数の入口があるのに、目についた1本だけ直す。**同じ事実を2箇所に書いて片方だけ直す**のも同じ（文書に出た形） | M-005, M-007, M-013, M-019, **M-023**, **M-025**, M-038, **M-058**, **M-061**, **M-072**, **M-076**, **M-087**, **M-20260918-verification-skippable-via-transition-flag** |
...
**Before**: #1093 のブランチで3件のテストが落ちていたので直した。その直後にブランチの head が
別セッションによって動いた（`merge: main をマージしてコンフリクト解消`）。新しい head に切り替えて
テストを流すと、その3件が通った。**「別セッションが同じ3件を直した」と読み、代表にそう報告した。**

**After**: push の直前に `git status` を見たら、**4ファイルが modified のまま残っていた** ——
`git merge --abort` とブランチ切り替えを挟んでも、自分の未コミット変更が作業ツリーに残っており、
**テストを緑にしていたのはそれだった**。別セッションが直したのは `check:schema` のスナップショットだけ。
報告を訂正し、3件の修正を自分の commit に含めた。

**なぜ気づけなかったか**: **「head が変わった」を「作業ツリーが入れ替わった」と読み替えた。**
git は追跡ファイルの未コミット変更をブランチ切り替えで持ち越す（衝突しない限り）。
知っている挙動なのに、切り替えの成功だけを見て**`git status` を1回も打っていない**。
テストが緑になった理由を、**測らずに直前の出来事（他人の push）へ帰属させた**のがこの型の核心で、
「1コマンドで確かめられる事実を確かめない」（型 F）そのもの。
並行セッションが同じブランチを触っている状況では、**誰の変更が効いているかが既定で分からない**。
その前提に立てば、緑の理由を言う前に status か diff を見るのが先だった。

**再発防止**: 仕組み —— 無し（判断に依存）。検査は「緑か」を見るだけで、「誰の変更で緑か」は見ない。
習慣 —— **他人のブランチで検査結果を語る前に `git status` と `git diff --stat` を打つ。**
そして**「Xが直したらしい」と書きそうになったら、X の commit の中身を実際に見る**
（`git diff <前> <後> --stat` で済む。今回それを見れば、変わっていたのが
スナップショット1ファイルだけだと即座に分かった）。
