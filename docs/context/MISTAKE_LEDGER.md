...
| 型 | 中身 | 該当 |
|---|---|---|
| **A. 道具を検証しない** | 変更したコードは検証したのに、「何を変えるべきか判断するために使った道具」（検出器・正規表現・スクリプト・クエリ）を検証していない。道具の出力を事実として扱う。**そのクエリが何を確定させたのかを確かめずに、確定したつもりになる**のもこの型。**自分の想定だけで検証するのも同じ**（当たりを取る「既知の1件」を自分で作る） | M-001, M-002, M-003, M-006, **M-010**, M-012, M-016, M-017, **M-022**, **M-024**, **M-031**, M-036, **M-041**, **M-045**, **M-046**, **M-047** |
...
| **C. 経路を1本しか見ない** | 同じ操作に複数の入口があるのに、目についた1本だけ直す。**同じ事実を2箇所に書いて片方だけ直す**のも同じ（文書に出た形） | M-005, M-007, M-013, M-019, **M-023**, **M-025**, M-038, **M-048** |
| **D. 移設で弱める** | 「同じものを別の場所に置くだけ」のつもりが、検査の強さや信号が落ちている。**移設先の信頼境界（誰の書き換えを受けるか・どんな権限を持つか）が変わっているのに気づかない**のもこの型 | M-008, **M-028**, **M-029**, **M-042**, **M-044** |
| **E. 手元とCIの差を忘れる** | 手元では通るのに CI だけ落ちる構成を作る。書いた本人には見えない | M-009, **M-030** |
| **F. 確認できる事実を確認しない** | 環境から1コマンドで確かめられる事実（今日の日付・件数・バージョン・設定ファイルの中身・**CI が実際に走ったか**）を、確かめずに書く | M-011, M-014, M-015, M-016, **M-018**, **M-021**, **M-026**, **M-027**, M-034, M-037, **M-043** |
| **G. 構造テストを振る舞いの証明として扱う** | ソースを grep して「その語が書かれている」を確かめただけで、**値が通るか**を確かめていない。テストは緑、機能は壊れている | **M-033** |

---

## M-048 「公開ページに何が出るか」を、公開ページのコードだけ読んで判断していた（2026-09-06・型 C）
...
## M-047 指摘を検証せずに「実在」と判定し、直さなくていい所を直した（2026-09-06・型 A）
...
背景に「マージ前にレビューを見落とした（M-043）」の反動があり、
**急いで全部直すことが誠実さだと錯覚した**。
...
## M-046 掃除したつもりのファイルが残り、次の PR の検査を汚していた（2026-09-06・型 A）
...
## M-043 レビューが着いた45秒後にマージした。着いていることを確認しなかった（2026-09-06・型 F）
...
## M-044 一本化で、呼び出し側が持っていた保証を2つ落とした（2026-09-06・型 D）
...
## M-045 自分が書いた条件式を、通ると思った入力でしか試さなかった（2026-09-06・型 A）
