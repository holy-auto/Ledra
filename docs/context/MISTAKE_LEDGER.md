...
| 型 | 中身 | 該当 |
|---|---|---|
| **A. 道具を検証しない** | 変更したコードは検証したのに、「何を変えるべきか判断するために使った道具」（検出器・正規表現・スクリプト・クエリ）を検証していない。道具の出力を事実として扱う。**そのクエリが何を確定させたのかを確かめずに、確定したつもりになる**のもこの型 | M-001, M-002, M-003, M-006, M-012, M-016, M-017, **M-022** |
| **B. 読まずに分類する** | コードの形（関数名・構造）から中身を推測して分類し、実際に読んでいない | M-004, **M-020** |
| **C. 経路を1本しか見ない** | 同じ操作に複数の入口があるのに、目についた1本だけ直す | M-005, M-007, M-013, **M-019**, **M-023** |
| **D. 移設で弱める** | 「同じものを別の場所に置くだけ」のつもりが、検査の強さや信号が落ちている | M-008, **M-024**, **M-028** |
| **E. 手元とCIの差を忘れる** | 手元では通るのに CI だけ落ちる構成を作る。書いた本人には見えない | M-009, **M-025** |
...
M-025「手元で全部通ったを実物でも通ると読んだ」と同じ形を、
その M-025 を書いた PR の中でやった。しかも今回は自分で天井を書いており、
...
## M-025 「手元で全部通った」を「実物でも通る」と読んだ（2026-09-04）
...
## M-024 「落ちないようにする」だけを見て、飛ばした文が権限を開けたままにした（2026-09-03）
...
> 採番の注: この番号は**4回目の付け替え**。main と未マージの2本が並行して連番を消費する
> ため、マージのたびに衝突する（M-021/022/023 は main が先に取った）。連番のまま運用する
> 限りこれは繰り返す。日付ベースの ID にするかは OPEN_QUESTIONS で保留中。
