...
| 型 | 中身 | 該当 |
|---|---|---|
| **A. 道具を検証しない** | 変更したコードは検証したのに、「何を変えるべきか判断するために使った道具」（検出器・正規表現・スクリプト・クエリ）を検証していない。道具の出力を事実として扱う。**そのクエリが何を確定させたのかを確かめずに、確定したつもりになる**のもこの型。**自分の想定だけで検証するのも同じ**（当たりを取る「既知の1件」を自分で作る） | M-001, M-002, M-003, M-006, **M-010**, M-012, M-016, M-017, **M-022**, **M-024**, **M-031**, M-036, **M-041**, **M-044**, **M-048**, **M-049**, **M-050** |
...
| **C. 経路を1本しか見ない** | 同じ操作に複数の入口があるのに、目についた1本だけ直す。**同じ事実を2箇所に書いて片方だけ直す**のも同じ（文書に出た形） | M-005, M-007, M-013, M-019, **M-023**, **M-025**, M-038, **M-051** |
...
| **F. 確認できる事実を確認しない** | 環境から1コマンドで確かめられる事実（今日の日付・件数・バージョン・設定ファイルの中身・**CI が実際に走ったか**）を、確かめずに書く | M-011, M-014, M-015, M-016, **M-018**, **M-021**, **M-026**, **M-027**, M-034, M-037, **M-045**, **M-046** |
...
## M-051 「公開ページに何が出るか」を、公開ページのコードだけ読んで判断していた（2026-09-06・型 C）
...
## M-050 指摘を検証せずに「実在」と判定し、直さなくていい所を直した（2026-09-06・型 A）
...
背景に「マージ前にレビューを見落とした（M-046）」の反動があり、
**急いで全部直すことが誠実さだと錯覚した**。
...
## M-049 掃除したつもりのファイルが残り、次の PR の検査を汚していた（2026-09-06・型 A）
...
## M-046 レビューが着いた45秒後にマージした。着いていることを確認しなかった（2026-09-06・型 F）
...
## M-047 一本化で、呼び出し側が持っていた保証を2つ落とした（2026-09-06・型 D）
...
## M-048 自分が書いた条件式を、通ると思った入力でしか試さなかった（2026-09-06・型 A）
