...
| **B. 読まずに分類する** | コードの形（関数名・構造）から中身を推測して分類し、実際に読んでいない | M-004, **M-017** |
...
| **G. 前提が途中で変わったのに読み直さない** | 判断したときは正しかった観察が、その後の `main` 取り込みなどで無効になっているのに、変更を見直さない | **M-016** |

---

## M-018 既にあるものを見ずに、隣に同じものを作りかけた（2026-09-04）
...
「緑だから大丈夫」が2回とも通ってしまう（M-016 と同じ形）。
...
## M-017 リンクの行き先を読まずにコピーした（2026-09-04）
...
## M-016 マージ前の観察を根拠に、サインアップのメール確認を飛ばした（2026-09-03）
