...
## 単体テストが1回だけ落ちたが、どれかを取れていない（2026-09-10）

`4ad78e1e`（#1056、main 取り込み後）を手元で3回回したところ、**1回目だけ 1 件失敗した**。

| 回 | 結果 |
|---|---|
| 1回目 | `1 failed \| 5607 passed \| 5 skipped (5613)` |
| 2回目 | 失敗なし |
| 3回目 | `EXIT=0` / `571 passed \| 1 skipped`、`5608 passed \| 5 skipped (5613)` |

同じコミットで CI の `Lint, Type Check & Unit Tests` も success。

**問題は、失敗したテスト名を取れていないこと。** `--reporter=dot` の出力を絞り込む前に
次の実行で消えてしまい、2回目以降は再現していない。**「flake だった」と言える根拠は無く、
「1回落ちたが特定できなかった」以上のことは分かっていない。**

紛らわしい点: このリポジトリの `check-schema` の自己テストは**陰性対照として
わざと AssertionError を出す**（`nosuch_column_xyz` を含む一時ファイル）。ログを目で追うと
失敗に見えるが、これは想定された出力で、集計上は passed。切り分けのときにここで足を取られる。

**決まっていないこと**: 追うか、放置するか。追うなら次に落ちたときに
`--reporter=verbose` でログをファイルへ落とし、`Failed Tests` の節を確保する。
1/3 の頻度なら数回回せば捕まる見込み。**再現しないうちに「直った」と扱わない。**
...
（RELEASE_LOG 2026-09-09、M-072）。**構造は変えていないので、次の勧告で同じことが起きる。**
...
- **同じ赤を複数のセッションが並行して直す問題。** 2026-09-09 に実際に起きた。
  #1054 と #1056 が同じ CVE 3件を別々に直し、後者が丸ごと無駄になった（M-072）。
...
なお #1056 では、main 取り込みのたびに同じことが起き、**同一 PR 内で2回繰り上げた**。
M-070 を取られて M-071 へ、次の取り込みで M-071 も取られて M-072 へ。
7組目・8組目を作らずには済んだが、**これは毎回手で気づいた結果であって仕組みではない。**
