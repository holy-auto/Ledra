...
`20260921152000` は外部キーを **NOT VALID** で戻したので、**この行は残ったまま**で、
これから入る行だけが縛られる。
...
| `insurers.plan_tier` | `text`（CHECK も無かった → `20260921152000` で戻す） | `text` + CHECK |
...
**CHECK と外部キーは 2026-09-22 に名前まで突き合わせ済み**（`20260921152000` / `20260921152100`）。
...
#1056 のマージでも変わっていない（トークンが未登録である限り同じ）。**赤が常態になると
