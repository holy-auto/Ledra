...
- 確定済み（2026-08-25、代表）: 役割=GP / Max AL=1 / 申告 Spec=**2.4** / 登記住所=東京都港区北青山1-3-1 アールキューブ青山3F / 連絡先=info@holy-inc.jp。→ EOI 送信可能な状態。
- 残る論点と選択肢:
  - **英字社名（DN の O 欄）**: 登記の英字表記が未確定。EOI/証明書 DN に必要。
...
- 次のアクション: (a) 英字社名を登記で確認し EOI 送信、(b) CA候補調査、(c) 署名鍵のKMS移行PoC、(d) 90日修正ポリシー&OWASPカバレッジの運用文書化、(e) サンプルで v2.4 出力を検証。
