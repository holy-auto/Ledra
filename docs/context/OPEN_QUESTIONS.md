...
## AI を呼ぶルートのうち、まだ中身を読んでいないものが残っている（2026-09-03）

`@/lib/ai/client` からモデル選択を import しているルート29本は洗い終えた
（28本にレート制限あり、1本は QStash のキューワーカーで免除。構造テストで固定済み）。

**残っているのは、モデル選択を自分では import せず、下位のモジュール経由で AI を呼ぶ経路。**
import の推移到達で洗うと47本挙がるが、この数は**信用できない**。
`isMissingTableError`（エラー判定）や `calcSizeClass`（純粋関数）のような AI と無関係な
関数まで拾うため。「到達できる」は「モデルを呼ぶ」ではない（MISTAKE_LEDGER 型 A）。

読んで確認する必要があるのは主に次の系統:
- `cron/*`（7本）— cron 認証で守られておりユーザーが叩けないので優先度は低い。**未検証**
- `admin/reservations`・`admin/reservations/[id]/advance` — `maybeAuto*` 系の自動処理が
  内部で AI を呼ぶ可能性がある
- `webhooks/inbound-email`・`line/webhook` — 署名検証はあるが、AI 呼び出しの有無は未確認

やるべきこと: 1本ずつ読み、AI を呼ぶなら `checkRateLimit(req, "ai", ...)` を入れ、
呼ばないなら構造テストの対象外だと確認する。**推移到達の数字を根拠に一括で入れない。**
