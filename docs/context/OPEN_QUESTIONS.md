...
## 外注テナントに無料プランでどこまで使わせるか（2026-09-03）

- 前提: 外注職人にも Ledra 利用を必須にし、元請けの発行コードで連携する形にした
  （DECISION_LOG 2026-09-03、`20260903000000`）。「無料でもいいから導入させる」が前提。
- 決まっていないこと: `free` プランで何を使えるようにするか。現状の `free` は既存の
  `planFeatures` の定義に従うだけで、**外注用途（受注先での実績確認と、自分の直販顧客への
  証明書発行）に足りるかは未検証**。狭すぎると導入されず、広すぎると有料プランが売れない。
- 影響範囲: ここが決まらないと「無料でもいいから導入させる」が実際に成立するか分からない。
  連携の仕組みだけ作っても、入口のプランが合っていなければ使われない。
- 次のアクション: `free` で使える機能の実際の範囲を洗い出し、外注が最低限必要とするもの
  （実績確認・証明書発行・顧客1件〜）と突き合わせる。**【要確認】** 現時点で
  `planFeatures` の中身は確認していない。
