...
## 発注書の「価格」を請求額として使ってよいか（2026-09-23）

「書類を撮影して取込」は発注書・商談メモの価格欄を**そのまま明細単価に入れる**。
ところがディーラーの商談メモの価格は、多くが**ディーラーがエンドユーザーに売る用品代**
（例: ナビ 324,170円）で、取付店がディーラーへ請求する**取付工賃ではない**。
一方で「ナビ移植 10,000円」のように取付店の作業代そのものが書かれた様式もある。

- 今は下書きとして人が単価を直す前提（金額の確定は人＝壁3）。
- 取引先ごと・品番ごとに「この用品ならこの工賃」を `menu_items` と突き合わせて自動で差し替えるかは未決。
  取引先別の工賃表が【要確認】。
