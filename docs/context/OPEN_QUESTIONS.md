...
## `audit_logs` の8列をスキーマから正式に削るか（2026-09-20）

`20260918142610 remote_schema` が本番だけで実行した DROP のうち、**`audit_logs` の8列**
（`table_name` / `record_id` / `old_values` / `new_values` / `performed_by` / `reason` /
`ip_address` / `device_id`）の扱いだけが未決。ほかの差は `20260920120500` で解消した
（下の「測り直した結果」）。

- 代表判断は「**マイグレーション側を本番に合わせる**」だが、**監査テーブルの構造化列を
  スキーマから正式に削る**のは別の判断なので、この8列は今回の整合から外してある。
- **本番の監査ログは既にこの8列を持っていない**（2026-09-18 に消えた）。落としても本番は
  何も変わらず、変わるのは**これから作る DB の形**だけ。
- アプリは本番の12列の形で書いている（`src/lib/audit/tenantLog.ts` が `query_json` へまとめる）。
- 消えた8列のデータは戻らない。推定: 中身は空だった（`tenantLog.ts` の記述では、テナント側の
  書き込みは存在しない列へ入っていて1件も残っていなかった）。**未検証**。

選択肢は2つ。(a) 8列をマイグレーションからも落として形を一本化する。
(b) 監査の構造化列は本来あるべきものとして、**本番へ足し直す**（空の列が戻るだけ）。
どちらを採るかで、`audit_logs` に何を残す設計なのかが決まる。

**決まるまでの副作用**: `scripts/check-schema-drift.mjs` は「マイグレーションにあって本番に無い列」が
1件でもあれば exit 1 で、**除外リストを持っていない**。この8列が残っている間、週次の
Schema drift ジョブは赤のままになり、**新しいドリフトが出ても既知の差と区別できない**
（`/code-review` の指摘）。除外リストを今足すと「決まった」ことになってしまうので、
**決着まではこの副作用を承知で赤を残す**。決めたら、落とす側なら差が消え、足す側でも差が消える。

### 測り直した結果（2026-09-20・`--keep` の再生 DB ⇄ 本番）

| | 前 | 後 |
|---|---|---|
| 再生にだけある列 | 12 | **8**（`audit_logs` のみ） |
| 本番にだけある列 | 15 | **0** |
| 再生にだけあるトリガ | 5 | **0** |
| 表・ビュー | 277 / 277 | 277 / 277 |

**索引は本番が7本多いまま**（再生 1172 / 本番 1179）。名前の突き合わせは未実施で、
今回の整合の対象外【要確認】。性能の話なので動作は変わらないが、差は残っている。
