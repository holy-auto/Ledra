...
## 【解決済み 2026-08-23】Web 側にも同種のクエリ不整合が 186 件ある（2026-08-23 起票）
- 結果: **189 箇所・58 ファイルを修正し、残り 0 件**。照合は repo 直下の
  `scripts/check-schema.mjs` に移して Web とモバイルの両方を見るようにし、CI に入れた。
  詳細は RELEASE_LOG / DECISION_LOG（同日）。
- 「テストがあるので誤検知かもしれない」は**半分当たっていた**。最初の走査 334 件のうち
  約 145 件は検査側の誤検知（`.from` と `.select` のまたぎ、FK 列名での埋め込み、
  JSDoc の見本）。**検査を直してから**実 DB で1件ずつ裏を取って修正した。

## 【優先】Tap to Pay は決済成立後に capture が失敗すると二重請求になり得る（2026-08-24）
- 状況: iPhone の Tap to Pay は (1) PaymentIntent を確定してカードを切り、
  (2) その後に `/api/mobile/pos/terminal/capture` を呼んで支払レコードを作る。
  (1) が成功して (2) が失敗すると（通信断、あるいは IP 単位のレート制限
  10 req/60s に店舗の NAT でまとめて当たる等）、**カードは切られているのに
  Ledra には売上が残らない**。画面はエラーを出すので操作者は再実行し、
  新しい PaymentIntent が作られて**二重に請求される**。
  さらに `payments` に PaymentIntent の ID を持つ列が無いため、後から
  突き合わせて重複を見つけることもできない。
- 選択肢:
  - `payments.stripe_payment_intent_id` を足して一意制約を張る（capture を冪等にする）
  - capture の前に「同じ PaymentIntent の支払が既にあるか」を照会してから作る
  - capture のレート制限を IP 単位から利用者単位へ変える（`checkRateLimit` の
    第3引数に `caller.userId` を渡す。`/pos/checkout` は既にそうしている）
- 影響範囲: 実売上ではまだ起きていない（`payments` は 11 件・最終 2026-03-23 の
  試験データのみ）。Tap to Pay を現場で使い始める前に決着させる必要がある。
- 次のアクション: 列の追加は本番スキーマの変更なので、代表の判断を仰いでから着手する
- 起票日: 2026-08-24

## モバイルの POS 明細が在庫と紐付いていない（2026-08-24）
- 状況: `deductInventoryForPosItems` は明細のうち `inventory_item_id` を持つ行
  だけを引き落とす。モバイルのカートは `menuItemId` までしか持たないので、
  サーバ経由にしても引き落としは 0 件のまま。加えて Tap to Pay が通る
  `/pos/terminal/capture` は在庫処理自体を呼んでいない（`/pos/checkout` は呼ぶ）。
- 選択肢: 品目マスタから `inventory_item_id` を引いて明細に載せる /
  サーバ側で `menu_items` を引き直して補う（端末の実装を増やさない）
- 影響範囲: スマホで会計した分の在庫が減らない。パソコンと併用している店舗で
  在庫数が合わなくなる。
- 次のアクション: サーバ側で補う案が有力。`/pos/checkout` と `/pos/terminal/capture`
  の両方で同じ処理を通す
- 起票日: 2026-08-24

## POS の2画面（会計・ウォークイン）に同じコードが重複している（2026-08-24）
- 状況: `apps/mobile/src/app/pos/checkout/[id].tsx` と `walk-in.tsx` に、
  QR ポーリング、支払方法のセグメント定義、`isQrFlow` の判定、エラー整形が
  それぞれ丸ごと重複している。今回見つかった Tap to Pay の二重計上は
  **片方の画面にだけ**あり、QR をカードとして記録する不具合も片方だけだった。
- 影響範囲: POS の修正は毎回2箇所に書く必要があり、片方だけ直る事故が起きる。
- 次のアクション: 会計処理のフック（`usePosCheckout` 相当）に寄せる。
  今回は権限剥奪を通すのが目的だったので手を付けていない
- 起票日: 2026-08-24

## 残り 37 本の SECURITY DEFINER 関数の権限（2026-08-23 起票 / 2026-08-24 更新）
- 状況: 未認証で呼べる SECURITY DEFINER 関数 53 本のうち、**危険度が高い 16 本は
  2026-08-23 に本番へ剥奪済み**（適用後に `has_function_privilege` で全 16 本の
  anon = false を確認）。残り 37 本（`insurer_*` / `get_my_*` など）は未着手。
  内部で `auth.uid()` を見ており anon では空を返すため実害は小さいと見ているが
  【要確認】、多層防御としては剥奪が望ましい。
  RLS ポリシー内で使われている 19 本（`my_tenant_ids` など）は、剥奪すると
  公開ページの読み取りが壊れるため対象外（適用後も anon 実行可のままを確認済み）。
- 次のアクション: (1) 37 本の呼び出し経路を1本ずつ洗って第2弾を作る
  (2) `pos_checkout` / `upsert_agent_user` の**内部にも呼び出し元の検査を足す**
  （権限剥奪と多層で守る。今は権限の1層だけ）
- 起票日: 2026-08-23

## `20260823170001_pin_function_search_path_round2.sql` が本番未適用（2026-08-24）
- 状況: search_path が固定されていない 6 関数（`pos_daily_sales_totals` /
  `estimate_vehicle_size` / `check_reservation_overlap` ほか）を固定する
  マイグレーションを用意したが、本番へは未適用。6 本とも SECURITY DEFINER では
  ないため、既に対処した SECURITY DEFINER 群ほどの危険は無い。
- 選択肢: 次の作業のついでに適用 / まとめて第2弾（残り 37 関数）と同時に適用
- 影響範囲: 放置しても現状の悪用経路は見えていないが、advisor の警告は残り続ける。
- 次のアクション: 代表が適用可否を判断する（剥奪ではなく設定の固定なので、
  権限剥奪より障害リスクは小さい）
- 起票日: 2026-08-24
...
## マイグレーションが空 DB から再生できない（2026-08-23 更新）
- 状況: マイグレーション 417 本を空の PostgreSQL に流したところ、
  **1周目で 171 本が失敗**した。原因は順序で、基本テーブルを作る
  `20260313020000_core_tables.sql` より**前の日付**のファイル3本が
  そのテーブルを ALTER している。繰り返し流すと 334/417 まで通るが収束しない
  （冪等でないマイグレーションがある）。
- 影響: 新しい Supabase プロジェクト、ステージングのリセット、Supabase Preview で
  スキーマが再現できない。**同じコードが環境によって動いたり動かなかったりする。**
  今回見つかった `audit_logs` のずれもこの構造から来ている。
- 選択肢: 本番から baseline のスキーマダンプを作って起点にする /
  順序を直す（本番の適用履歴との整合に注意）/ 現状維持
- 次のアクション: baseline ダンプ方式の可否を検討する（Supabase の標準手法）
- 起票日: 2026-08-16（2026-08-23 に実測して更新）

## 保存先の列が無い項目が残っている（2026-08-23）
- **解決済みの分**: 代理店の銀行口座・郵便番号・ウェブサイトは `agents` に
  `bank_info`（jsonb、tenants と同じ形）/ `postal_code` / `website_url` を追加して
  保存できるようにした（本番適用済み、version 20260823154504）。
- 残っているもの:
  - **会社名**: `agents` は `name` 一本。別に会社名を持つ必要が出たら列を足す。
  - **メール通知の可否**: 代理店単位ではなく利用者単位で持つべきで、置き場所
    （`agent_users` か別テーブル）が決まっていない。
  - 証明書の等級 / 点検日 / PDF URL、`certificates` の `work_areas` / `category`。
- 現状の扱い: どれも**黙って捨てず**「この項目は現在保存できません」と返す。
- 次のアクション: 代表に、会社名とメール通知を Ledra 上で持つ必要があるか確認する
- 起票日: 2026-08-23

## スキーマ照合がフィルタの列名を見ていない（2026-08-23）
- 状況: `scripts/check-schema.mjs` は `.select()` の列と `insert/update/upsert` のキーを
  照合するが、`.eq()` / `.or()` / `.order()` などに書かれた列名は見ていない。
  存在しない列でフィルタしてもクエリは同じく 400 になる。
  実例: `followUp.ts` の `.or("warranty_period.not.is.null,...")`（今回は同じ箇所を
  直したので消えたが、仕組みとしては素通りしていた）。
- 選択肢: フィルタ系メソッドの第1引数も照合対象に足す / `db:typegen` の生成型に移行する
- 影響範囲: 同じ「データはあるのに空に見える」不具合が、フィルタ経由でまだ作れる。
- 次のアクション: フィルタの照合を足す（`.eq/.neq/.gt/.gte/.lt/.lte/.like/.ilike/.in/.is/.order`）
- 起票日: 2026-08-23

## モバイルの証明書作成がサーバの作成 API を通っていない（2026-08-23）
- 状況: モバイルの証明書作成は `certificates` へ直接 insert している。Web の
  `/api/certificates/create` は発行上限（プラン別）のチェック・監査ログ・撮影 nonce の
  発行・メーカー認定テンプレートの検証を行っているが、モバイルはそれらを飛ばしている。
  列名の不整合は直したので登録自体は通るようになったが、**上限を超えても発行できる**。
- 選択肢: `/api/mobile/certificates` を新設して作成ロジックを共有する /
  当面このまま（上限は Web 側でのみ効く）
- 影響範囲: プラン別の発行上限が実質無効。監査ログにモバイル発行分が残らない。
- 次のアクション: 発行上限を課金上どこまで厳密にするか決める
- 起票日: 2026-08-23
