...
- **`20260825000000` がいつ本番へ適用されたか特定できていない。** `fa14d46` の
  `db-migrate` は out-of-order で失敗しているのに、適用はされている。
  `supabase db push` が警告を出しつつ適用したのか、別経路で当てられたのかが
  分からない。**「失敗した run が、実は一部を適用している」なら、失敗を見て
  『本番は変わっていない』と判断する手順が全部危うい。**【要確認】 → 調査要
- **改名の前に本番の台帳を引く手順を CI で強制できないか。** `lint-migrations` は
  同一バージョンの重複しか見ず、`Migrations Replay` は空DBからの再生なので、
  どちらも本番の適用済みとの前後関係を見ていない。同じ失敗が3回起きている。 → 代表判断
