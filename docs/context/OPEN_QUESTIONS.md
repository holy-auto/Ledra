...
## `is_pii_disclosed()` 修正の残り3点 —— 壊れていた間の挙動と、検査が届かない範囲（2026-09-08）

本番の修正そのものは適用・実測済み（#1016、`db-migrate` run #65、`false` を返すことを確認）。
ここに残すのは、それでは分からない3点。

**未解決1: 壊れていた間、PII 開示同意の判定がどう振る舞っていたか【要確認】。**
`is_pii_disclosed` は呼べば必ず 42P01 で落ちていた。呼び出し元の
`insurer_get_certificate` がその例外をどう扱うか（全体が失敗するのか、開示なしとして
握り潰していたのか）を**読んでいない**。前者なら保険会社ポータルはその期間ずっと使えず、
後者なら**開示同意済みの PII が返らなかった**ことになる。影響の向きが逆なので、
外部に説明する必要が出た場合はここを先に確定させる。あわせて**いつから壊れていたか**
（`20260404000000` の適用日）と、その間に保険会社の閲覧があったかも未確認。

**未解決2: 常設した検査は LANGUAGE sql の本体にしか完全ではない【要確認】。**
`pg_get_functiondef()` を流し直す方式は、plpgsql の本体については**構文しか検証されない**
（名前解決は実行時）。動的 SQL も同様。同じ形の事故が plpgsql 側に残っていないかは
未調査で、そもそも別の手段が要る（実際に呼ぶしかない）。

**未解決3: 「実行時にしか落ちない DB オブジェクト」の他の種類が未調査【要確認】。**
ビュー・トリガも同じく、定義時に通って実行時に落ちる形を作れる。今回の検査は関数だけを見る。

- 起票日: 2026-09-08（DECISION_LOG 2026-09-07 の項目8から移設）
