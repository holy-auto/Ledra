...
## 台帳だけが進んで実体が無い版が、他にも無いか（2026-09-20）

`20260917000400` は本番の `schema_migrations` に**適用済みと記録されているのに**、
`workshop_capability_profiles` が本番に存在しなかった（`pg_class` を全スキーマで確認）。
`db push` は適用済み版を再実行しないので、通常の経路では永久に作られない状態だった。
`20260919150000` で作り直して解消したが、**なぜ台帳だけ進んだのかは不明**【要確認】。

- 同じ食い違いが他の版に無いかは**未調査**。台帳の 482 版すべてについて
  「その版が作るはずのオブジェクトが本番に在るか」を突き合わせていない。
- 版リストの一致（`md5` 一致・2026-09-20 実測）は**「台帳と repo のファイル名が一致している」だけ**で、
  **中身が本番に反映されている保証ではない**。この2つを同一視すると今回と同じ穴になる。
- 検出するなら、`check-schema-drift.mjs`（列・ポリシーを見る）を表の存在まで広げるのが近い。
