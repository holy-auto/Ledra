...
> 本文・他ファイル・コード内コメントからの**参照 446 箇所（MISTAKE_LEDGER 外 230 /
> 台帳本文 216）は1件も書き換えていない**（変更前後で `M-NNN` の多重集合を比較し、
> 減ったものが1件も無いことを確認）。数え方（ベースは `acf89574`）:
> `git grep -hoE '\\bM-[0-9]{3}\\b' acf89574 -- . ':!docs/context/MISTAKE_LEDGER.md' | wc -l` → 230、
> `git show acf89574:docs/context/MISTAKE_LEDGER.md | grep -vE '^## M-' | grep -ohE '\\bM-[0-9]{3}\\b' | wc -l` → 216。
