...
**入力欄の `type` を見ずに、変換コードの形だけで同型と判断した**（MISTAKE_LEDGER M-037）。
残り3画面は 2026-09-04 に JST 固定へ寄せて解消した。

### 併せて残っているもの: 連絡スケジュールの「今日 / 今週」の境界

`ContactSchedulesClient.tsx` は入力・表示を JST 固定に寄せた（DECISION_LOG 2026-09-04）が、
一覧を「今日 / 今週 / それ以降」に振り分ける境界（`ymd()` と `inRange()`）は
まだ**ブラウザローカルの `Date`** で作っている。表示専用で自己完結しているものの、
入力と表示が JST 固定になったぶん、**グルーピングだけ別の基準**という状態になっている。
JST 以外の端末では、日付表示は 9/5 なのに「今日」に入らない、といったずれが出る。
`jstParts()` で置き換えられるはずだが、週の開始曜日の扱いを含め未確認。
