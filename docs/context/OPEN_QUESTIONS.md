...
## 工数データの自動収集元 sfh.honda.co.jp を使ってよいか（2026-09-23）

工賃は `labor_hour_masters`（型式 × 品番 → 工数）× 店舗単価で算出する方式に決めた（DECISION_LOG 同日）。
工数の出典として代表から `https://sfh.honda.co.jp/T001` を示されたが、開発環境のネットワーク方針で
遮断されており、次が未確認:

- ログインが必要な販売会社向けシステムか（必要なら認証情報の扱いが別途要る）
- 利用規約で自動取得・転載が許されるか
- 型式 × 品番ごとの工数がどの形式で載っているか（HTML 表 / PDF / CSV 出力の有無）

確認できるまでは CSV 一括登録（`/admin/labor-hours`）で運用する。
店舗ごとの端数処理ルール（10円単位の切上げ等）も【要確認】。
