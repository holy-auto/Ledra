...
- 追記（2026-09-24 前提が確定）: 代表が**指定工場（民間車検場）を対象顧客に含める**と判断（DECISION_LOG 2026-09-24）。これで G5 は実要件に格上げ。方針は「自前レンダリング（第三号/四号様式・令和6年10月版）」を第一候補、AIRAS/継続検査OSS 連携は保適証・車検申請の別テーマとして分離。残る論点は次の2点:
  - (a) 様式各欄（点検・整備の概要／検査機器による検査の測定値／目視検査／軸重 等）を Ledra が**データ保持しているか**。無い項目があれば入力・保存の追加が要る（自前レンダリングの工数を左右）。要棚卸し。
  - (b) 指定工場ユーザーが**完成検査を Ledra 上で行い、Ledra が記録の source of record になるか**。別の車検ソフト併用なら記録簿はそちらに出るため、Ledra は「取り込み・保存・表示」止まりになる可能性。→ 対象工場の運用実態を1〜2社で確認。
- 追記（2026-09-24 データモデル棚卸し／コード実査）: 第三号様式の各欄を現行スキーマに突き合わせた結果、**PDF の様式描画は軽い（既存基盤の応用）が、様式を埋めるデータの保持が薄い**ことが判明（前回「自前レンダリングで軽く充足可」は要訂正）。
  - **検査機器の数値測定値（肝）＝構造化スキーマ該当なし**: `inspection_records.answers`(jsonb) は `{item_id:{value:string|bool|null}}` の汎用KV、テンプレ項目型は `ok_ng/text/numeric` の3種のみ。numeric は**文字列**格納で単位・左右前後・上下限判定を持たない。制動力/軸重/スピードメーター誤差/前照灯光度・光軸L・R/排出ガスCO・HC/サイドスリップ/騒音を様式どおり保持する専用スキーマは無い。
  - **完成検査（保安基準適合検査）の型が無い**: `inspection_records.inspection_type` は `intake/delivery/periodic` のみ。
  - **車両情報の欠落**: 原動機型式・軸重・寸法・重量・初度登録年月日は車検証OCR(`shakensho.ts`)で抽出できるが**保存先カラム無し**（`vehicles.year` は年のみ）。
  - **検査員/整備主任者**: `inspection_records.inspector_name` 単一自由文字列のみ、法定資格ロール無し（G1 と一体）。
  - 実装スコープ（推定）: 【大】測定値の構造化スキーマ新設／【中】完成検査レコード型／【中】車両情報の永続化（OCR抽出済み項目の保存先追加）／【中】整備主任者・検査員（G1連動）／【小】第三号/四号様式PDFレンダラ（ここだけ軽い）／【小〜中】入力UI。合計で**中〜大の機能開発**（「PDFを1枚足す」ではない）。
  - **着手前に潰す前提＝上記(b)の source of record**。Ledra が測定値を持つのか、外部車検テスタ/ソフトで測って取り込むだけかで【大】【中】の設計が根本的に変わる。代表確認待ち。
  - 出典/未取得: 様式PDF本体（MLIT・各振興会）は egress ブロックで未取得のため欄名は推定。実装前に e-Gov 第三号様式で最終照合。
