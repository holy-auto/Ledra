...
## 【要判断・優先】未認証（anon）から呼べる SECURITY DEFINER 関数が 53 本ある（2026-08-23）
- 状況: Supabase の security advisor の指摘を実際に確認したところ、`anon` が
  `/rest/v1/rpc/<name>` から呼べる SECURITY DEFINER 関数が **53 本**あった
  （`has_function_privilege('anon', ..., 'EXECUTE')` が全て true）。
  うち2本は**関数の中に呼び出し元の検査が無い**:
  - `pos_checkout(...)`: 引数の tenant_id / user_id をそのまま使って payments と
...
- 残り: `insurer_*` / `get_my_*` など。内部で `auth.uid()` を見ており anon では
  空を返すため実害は小さいと見ているが【要確認】、多層防御としては剥奪が望ましい。
  RLS ポリシー内で使われている 19 本（`my_tenant_ids` など）は、剥奪すると
  公開ページの読み取りが壊れるため対象外。
- 次のアクション: (1) 代表に剥奪マイグレーションの適用可否を確認
  (2) 残りの関数の呼び出し経路を洗って第2弾を作る
  (3) `pos_checkout` / `upsert_agent_user` の**内部にも呼び出し元の検査を足す**
  （権限剥奪と多層で守る）
- 起票日: 2026-08-23
