...
## 外注職人が自分の施工履歴を確認する導線が無い（2026-09-01）

- 状況: 外注施工の記録を発注に紐付ける実装（`certificates.job_order_id`）で、
  元請け・外注先の**テナント**同士は成果物を共有できるようになった。しかし
  **作業した職人本人**が確認する導線は依然として無い。外注職人は
  `staff_members.user_id = null`（`20260617000002` の設計）でログインアカウントを持たず、
  Web 管理画面もモバイルアプリも `tenant_memberships` 前提
  （`apps/mobile/src/lib/auth.ts`）。本人が見られるのは公開URL `/c/[public_id]` を
  自分で控えていた場合だけで、一覧する手段が無い。
- さらに重い問題: 同じ職人が複数の元請けに登録されると**元請けごとに別の
  `staff_members` 行**になる（一意制約はテナント内）。名寄せの手段が無いため
  「この職人の通算施工実績」が原理的に出せない。47都道府県で複数の元請けの下に入る
  働き方では、そのまま自分の実績が分散する構造になる。
- 選択肢: (1) 職人に読み取り専用の1URL（トークン）を配り、自分が `craftsman_staff_id`
  に入っている証明書を横断で見せる。マイページ（`customer_sessions`）と同じ作りが流用できる。
  (2) 職人に軽量アカウントを持たせる（`staff_members.user_id` を外注でも埋める）。
  RBAC とプラン課金の扱いが増える。(3) 現状維持（元請けの管理画面でのみ確認）。
- 影響範囲: 放置すると「職人の実績が Ledra に貯まる」という価値が外注職人には届かない。
  代表自身が外注側に回る働き方なので、自社のドッグフーディングでも欠ける。
- 次のアクション: 代表判断。1 を採るなら発行・失効の運用（誰がURLを配り、離職時に
  どう止めるか）を先に決める必要がある。
