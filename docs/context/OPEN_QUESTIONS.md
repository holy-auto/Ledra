...
## d-Happy の工数は「選んだ組み合わせ」と「車（グレード）」で変わる —— 1型式1値で持ってよいか（2026-09-23）

代表が d-Happy から4台分（JF5×2・DG5・RP8、585行）を収集。変換して分かったこと（数え直し済み:
585 = 工数あり 570 + 空欄 15、型式×品名 451 = 取込可 443 + 食い違い 8）:

- **組み合わせで変わる**: 同じ JF5 の「ETC2.0車載器 取付アタッチメント」が 1.4h と 0.0h。後者はナビパッケージ
  選択時に自動追加された扱いで、作業がパッケージ側に載っている。全品目を同時に選んで収集すると、こうした
  「同時装着なら0h」が混ざる（工数0は 570 行中 279 行）。
- **同じ型式でも車で変わる**: JF5 の2台で共通する116品目中6品目が不一致（1.2h と 1.1h など）。グレード・年式差か【要確認】。
- 同じ車の中でも同名で値が2つある行（リモコンエンジンスターター 取付アタッチメント 0.5h / 1.0h 等）。

**2026-09-23 決着（代表判断、DECISION_LOG 同日）**: 食い違いは「あとから入ってきた値で上書き」、単品工数への
統一はしない。ファイル内では後の行、登録済みとの違いは今回の値を採り、上書きした行は画面に一覧で出す。
追加の決着（同日）: 食い違いでは 0h を採らない。TC コードで差があれば TC 別に持つ（`tc_code`、マイグレーション `20260923150000`）。
残る論点: いまの収集表には TC コード列が無いので、TC 付きで収集し直さないと差が出るか分からない【要確認】。
（発注書の写真から TC コードを読む件は 2026-09-24 に実装: 書かれていれば読み、無ければ空のまま。DECISION_LOG 同日）

## 工数データの自動取得を d-Happy 運営者に問い合わせるか（2026-09-23）

工賃は `labor_hour_masters`（型式 × 品番 → 工数）× 店舗単価で算出する方式に決めた（DECISION_LOG 同日）。
工数の出典は Honda Access の用品適用検索 d-Happy（`https://sfh.honda.co.jp/T001`）。分かったこと:

- ログイン不要。車台番号（例 `GP3-1017220`）で検索 → 装着可能用品を選択 → 「装着用品確認」に
  **品番ごとの取付工数**が出る（代表のスクリーンショットで確認）。
- 同画面の数字 ドアバイザー 価格 9,900・工数 0.4・工賃 9,000 → 合計 13,860 は
  `9,900 + 0.4 × 9,000 × 1.1` と一致 ＝ 工賃単価は税抜で、Ledra の算出式と同じ考え方。
- **`robots.txt` が `User-agent: * / Disallow: /`**（2026-09-23 に取得して確認）。運営者は自動アクセスを
  望んでいないので、**自動収集は保留**。取得したのは入口ページと robots.txt の各1回のみ。

当面は人がコピーした表の貼り付けで登録する（実装済み、DECISION_LOG 同日）。残る判断:
Honda Access / 販売会社に自動取得の可否・データ提供を問い合わせるか。
未確認: 同じ型式でもグレード・TC コードで工数が変わるか。店舗ごとの端数処理ルールも【要確認】。
...
### 【2026-09-24 追記】サイズを実測した。結論は「今は直さない」

起票時の【要確認】（自己ホストの実サイズ）を測った。`npm run build` を通し、
`.next/static/media` に出たものを数えた（`find` と `python3` の2経路で一致）。

| 測ったもの | 実測 |
|---|---|
| ビルド時に落ちてくる woff2 | **131ファイル・5,317,348 bytes（5.07 MB）** |
| 既に在る `public/fonts/*.ttf` 2本 | 4,726,492 bytes（4.51 MB） |
| `@fontsource/noto-sans-jp@5.3.0` の unpacked | 80,398,272 bytes（**76.67 MB**） |

**`subsets: ["latin"]` は効いていない。** 3箇所とも latin を指定しているのに、
生成された CSS には日本語の unicode-range（U+30xx 等）が入り、日本語グリフ一式が
**約130分割**で落ちてくる。ブラウザは必要な分割だけ取るので**実行時の負担は増えていない**
——5.07 MB はビルド時のダウンロード量である。

**CI でキャッシュして避ける案も成立しない。** `.next/cache` に woff2 は1つも無く
（キャッシュ本体は turbopack の 823 MB）、Actions でキャッシュできる大きさでもない。

### 選択肢と、それぞれの実コスト

| 案 | コスト |
|---|---|
| (a) woff2 131本を vendor して `next/font/local` | git に **+5.07 MB**（永久）。`@font-face` と unicode-range を手で約130本維持する |
| (b) `@fontsource/noto-sans-jp` を入れる | 全 CI ジョブの `npm ci` に **+76.67 MB**。ビルド時のフェッチ先が Google から npm に移るだけ |
| (c) 既存の `public/fonts/*.ttf` を流用 | **退行する。** あれは PDF 用の TTF で、woff2 の unicode-range 分割が無いため**1ページ 2.3 MB** を丸ごと落とすことになる |
| (d) 直さない | 外部フェッチは残る。落ちたら人手で再実行 |

**(c) にはもう1つ地雷がある。** `public/fonts/*.ttf` は**7,466 グリフのサブセット**で、
`src/lib/marketing/resourcePdf.tsx` に「グリフの無い字は豆腐になり、PDF を開くまで
誰も気づかない。料金プランの比較表の `✓` が全部豆腐だった」と実例が残っている。
**日本語フォントのサブセット化は、この製品で一度事故を起こしている。**
Web 側は顧客名・車両情報など任意の日本語を出すので、同じ形の事故が起きうる。

### 推奨

**(d)（今は直さない）を推す。** 観測された失敗は**1回**で、同じコミットの再実行で消えた。
一方 (a)〜(c) はどれも恒久的なコスト（git 5 MB / npm ci 76 MB / 実行時 2.3 MB と豆腐）を
払う。**直す costs が、今のところ問題より大きい。**
...
## `certificate_images.tenant_id` に外部キーが2本あり、順序で振る舞いが変わる（2026-09-22）

同じ列に `ON DELETE` の違う外部キーが2本ある。

| 制約 | 動作 | 本番の OID | 再生 DB の OID |
|---|---|---|---|
| `certificate_images_tenant_id_fkey` | CASCADE | 26232 | 25739 |
| `fk_certimg_tenant` | RESTRICT | 39885 | 18244 |

同じイベントに付いた RI トリガは**名前順**に発火し、その名前は制約の OID を含む。
つまり**どちらが効くかは制約を作った順で決まる**。上の表のとおり本番と再生 DB は逆順で、

- 本番: CASCADE が先 → **テナント削除が `certificate_images` を黙って道連れにする**（推定・未検証）
- 再生 DB: RESTRICT が先 → テナント削除が止まる（**実測**。実際に削除して確認した）

**名前を揃えても振る舞いは揃わない。** `check-schema-drift.mjs` は名前しか見ないので、
この差はどの検査にも映らない。

**未決**: 2本のうちどちらを残すか。テナント削除時に証明写真を消してよいなら CASCADE、
消してはいけないなら RESTRICT。**どちらかを消すのが根治**で、残す側の挙動が全環境で決まる。
本番で確定させるには、`certificate_images` に行があるテナントを1件消してみればよいが、
本番データの削除なので手を付けていない【要確認】。

## 利用者の消えた owner membership が本番に1件ある（2026-09-22）

`tenant_memberships` に、`user_id` が `auth.users` に存在しない行が**1件**ある
（role=`owner`・2026-07-26 作成）。`tenant_memberships_user_id_fkey` は `ON DELETE CASCADE` なので、
制約が生きていればこの行は利用者の削除と一緒に消えていた。
2026-09-18 に `remote_schema` がその外部キーを本番だけで落としたあと、利用者が消えて残った形。
...
| 本番にだけある列 | 15 | 0 | **0** |
| 再生にだけあるトリガ | 5 | 0 | 0 |
| 表・ビュー | 277 / 277 | 277 / 277 | 277 / 277 |

`public` の実表の列を全部並べた md5 が両側で `09fe8d69088f885e0e754a8ef3ee271d`（3549列）。
`audit_logs` 単体でも `b3c2feb021889c3831d69b20060a638b`（12列）で一致する。

`audit_logs` の8列の扱いは **DECISION_LOG 2026-09-20「`audit_logs` の旧8列を落とす」** で決着した。

### `invoices` は表ではなくビューだった

差の2列（`job_status` / `assigned_user_id`）は列の欠落ではなく**ビューの SELECT 一覧の差**で、
`ALTER TABLE ADD COLUMN` では揃わない（1回目の再生がここで落ちた）。
本番の `pg_get_viewdef` と同じ定義で置き直して解消した。
**「本番にだけある列」を数えるとき、表とビューを混ぜると同じ罠を踏む。**
...
- **ラッパの `rateLimit` は IP 単位**。`withCaller(h, { rateLimit: "ai" })` は
  `checkRateLimit(req, preset)` を識別子なしで呼ぶため、IP にフォールバックする。
  店舗の NAT で全端末が1つの枠を共有するので、**同じ店の2人目が弾かれる**。
  AI を呼ぶ 13 本はテナント/ユーザー単位の呼び出しに戻したが、**他の withCaller ルートの
  `rateLimit` は IP 単位のまま**。実運用での当たり具合は未測定【要確認】。
  直すなら `WithCallerOptions` に識別子（`caller` から作る関数）を足す形になる。
- **`CALLER_WRAPPERS` は `withCaller` 1つだけ**。別の認証ラッパを足したとき、
  ここへ登録し忘れると**そのラッパのルートが再び検査の外に出る**。
  登録漏れを止める仕組みは無い（判断に依存）。
- **復元した 15 本（AI 13 + 非AI 2）のレート制限は実 API で発火させていない**。検出器・型検査・
  ユニットテストは通っているが、429 が実際に返るところまでは見ていない。
- **AI を呼ばないルートのレート制限は、どの検出器も見ていない。** 同じリファクタで
  `admin/inspection-records/images` と `admin/square/sync` の2本が剥がれていたのを
  `/code-review` が見つけて復元したが、**他に残っていないかは未調査**【要確認】。
  網羅するには「リファクタ前後で `checkRateLimit` が消えたルート」を洗う必要がある。

## 外注施工履歴（MVP）で仕様に無かった判断と、まだ無いもの（2026-09-18）

仕様（TR-001〜052 / PER-001〜029 / AC-001〜031）に書かれていなかったため実装側で決めた3点。
代表の確認が要る。いずれも `src/lib/domain/transitions.ts` と `src/lib/outsourcedWork/rules.ts` の表1箇所を直せば変わる。

- **作業保留への入口**: 仕様は出口（TR-041〜046）しか書いていない。AC-022「例外または差戻しにより」と
  AC-015「施工中断 → 保留」に従い、**数量不足・品番不一致・破損確認・施工中断・差戻し**からだけ入れるようにした。
  再施工待ち・受領確認中からの保留は通していない。
- **TR-047「任意の取消可能ステータス」**: 終端（完了・作業取消）と、承認操作の中でシステムが即座に抜ける
  例外承認済み・例外却下を除く**全状態**から作業取消できる。承認済みのまま取り消すと「承認したのに復帰しなかった」記録になるため除いた。
- **TR-018「権限保持者」（例外承認の申請者）**: 両社の実務者・管理者（発注元店舗担当者・発注元管理者・施工担当者・施工会社管理者）。閲覧者・確認者は申請しない。

まだ無いもの:

- **東日本三菱での実運用確認（AC-030）**。通常フローと主要例外フローを一巡させる相手と日程が未定。
- 写真アップロード UI（今はストレージパスの文字列入力）。既存の証跡ストレージへ相乗りする形。
- 店舗単位の閲覧絞り（PER-023「自店舗・担当拠点」）。`client_store_id` は列だけ。
- 担当者割当・承認者/確認者の指名がメンバー選択でなく user_id 入力。
- 証明データのアンカー先（Polygon）が本番で有効か【要確認】。無効でも証明データとハッシュは残る。
...
- **追記(2026-09-21・再発)**: PR #1109（`claude/issuance-failure-ug8bdo`、head `bc71fde`）でも
  `ci.yml` の run が作られなかった。付いたのは Vercel / Supabase / dependabot の3件だけで、
  このブランチの最新 CI 実行は 2026-08-15 の `0b7b3a2` のまま。
  同時刻帯に他ブランチの CI は正常（run 3221 が 14:31、3222 が 14:32、3223 が 14:40、いずれも成功）で、
  **GitHub 側の全体障害ではなくブランチ単位の事象**である点は 08-27 と同じ。
- 08-27 との違い（新しい手がかり）: 今回は**オープンな PR が無い状態でブランチへ force-push し、
  その後に PR を作成**した（前の PR #919 はマージ済みでクローズ済みだった）。
  08-27 は「既にオープンな PR への push」だったので、`pull_request` の
  どのアクティビティで取りこぼすかは両者で違う可能性がある。
  なお `ci.yml` の `pull_request:` は `types` を指定していないので既定の
  `opened` / `synchronize` / `reopened` のみ。**`ready_for_review` では起動しない**ため、
  ドラフトを ready 化しても復旧手段にはならない。
- **回避策は今回どれも効かなかった（同日中に実測）**。同じブランチで 3 回 push したが、
  `ci.yml` の run は1本も作られていない —— `bc71fde`（PR 作成時）/ `f159e7b`（中身のある
  コミットの push＝`synchronize`）/ `b42e504`（**main を取り込むマージコミット**、
  08-27 の起票にある唯一の既知回避策）。付いたのは毎回 Vercel / Supabase / dependabot だけ。
  つまり **08-27 の「main を取り込んで押し直す」は再現性のある回避策ではない**
  （あのときは時間経過で解消しただけの可能性がある）。
- 起動手段が他に無い: `ci.yml` の `on:` は `push`(main/staging) と `pull_request` のみで
  **`workflow_dispatch` が無い**ため、手動起動もできない。`types` 未指定＝既定の
  `opened`/`synchronize`/`reopened` なので、ドラフトの ready 化でも起動しない。
  空コミットと close/reopen は使わない方針。
- **この状態の PR は、必須チェックが設定されていればマージできない。**
  `ci.yml` に `workflow_dispatch` を足しておくと、再発時に手動で起動できて手詰まりを避けられる。
- **追記(2026-09-21 15:10 UTC・自然復旧)**: 6 回目の push（`edcb386`）で**何もしていないのに起動した**。
  run 3228 以降は正常で、10 チェックすべて成功。08-27 と同じく **原因不明のまま解消**した。
- **「main を取り込めば直る」は今回も否定されたまま**: 復旧した `edcb386` は main 取り込みマージだが、
  その 8 分前の `b42e504` も**同じく main 取り込みマージでコンフリクトを解消したもの**で、
  こちらでは起動しなかった。2 つのマージの違いは時刻だけなので、**マージが引き金だった根拠は無い**。
  最初の push（14:55）から復旧（15:10）まで約 15 分。08-27 も「次の push で直った」のではなく
...
- main を取り込んだ次の push（`1804721`、00:52 UTC）では正常に起動し、全ジョブ緑。
  **原因不明のまま解消**した。GitHub 側の一時障害か、ブランチ単位のスロットリングかは
  切り分けられていない。
  ~~再発したら「main を取り込んで押し直す」で逃げられる~~
  → **2026-09-21 に否定された。**（上の追記のとおり、main 取り込みマージでも起動せず、
  8 分後の別 push で自然に復旧した。当時も「main を取り込んだから直った」のではなく
  **時間経過で直っただけ**だった可能性が高い。対処は「15〜20 分置いてから push し直す」。）
...
- **`resolvePosAppSale`（`src/lib/pos/squareSale.ts`）に、2件の会計が同時に
  同額を引き当てようとしたときのレースが残っている。** 仮定: 同額・同時間帯の
  未記帳会計が2件（例: 客Aが実際に支払った分と、客Bの会計を先に「引き当て」
  ボタンで試した分）あり、どちらの `resolvePosAppSale` 呼び出しも
  `payments` テーブルへの書き込み（`recordPosSale` 経由の `pos_checkout` RPC）が
  完了する前に実行されると、両方とも「まだ記録されていない」候補として
  **同じ Square の payment_id** を1件に絞り込んでしまいうる。誤っていた場合、
  この項目自体が実在しないレースを指しており対応不要になる。
  - 確実: `payments.square_payment_id` には一意インデックスがあるため
    （`recordSale.ts:82-97`, `resolveKey`/既存チェック）、**同じ payment_id が
    2行として二重に記帳されることは無い**。先に書き込みを終えた側が勝つ。
  - 推定: 後から書き込む側は一意制約違反または「候補なし」で失敗するだけだが、
    **先に書き込みが通った側が、実際には支払っていない会計に他人の支払いを
    紐付けてしまう**可能性がある（二重計上ではなく取り違え）。未検証 —
    実際に2つの同時リクエストを発生させて確認していない。
  - 対応しなかった理由: 塞ぐには `findRecentPayment` での候補選定と
    `payments` への記録を1つのアトミックな操作にする必要があり
    （例: 候補の payment_id を先に `UPDATE ... WHERE square_payment_id IS NULL`
    で「予約」してから金額検証する、またはテナント×金額×時間帯単位の
    アプリケーションロック）、クライアント/サーバ双方の冪等キー設計に
    踏み込む変更になる。PR #1092 の差分規模を超えるため、この PR では
    見送り、ここに記録するに留めた。
  - 次にやること: (1) 上の仮定を再現テストで確認する、(2) 再現したら
    `resolvePosAppSale` に「候補を排他的に確保してから記帳する」形の
    アトミック化を別 PR で入れる。
...
## 外部サイト投稿のトークンが、権限の粒度と合っていない（2026-09-15）

`/admin/site-content` から holy-inc.jp / MobileWash に投稿できるようにした。
コミットは Ledra の環境変数 `GITHUB_CONTENT_TOKEN` 1本で行う。

つまり **`site_content:manage` を持つ人 = 他2サイトの内容を変えられる人**になる。
今は super_admin（代表）だけなので実害は無いが、広報や編集の担当者を増やすときに
「Ledra にだけ投稿できる人」を作れない。

選択肢は3つ。どれも今すぐやる必要は無いが、人が増える前に決める。

1. 投稿先ごとに権限を分ける（`site_content:manage:holy-inc` 等）。アプリ側だけで済む
2. サイトごとに別トークンにする。漏れたときの影響範囲は狭まるが、権限の分離にはならない
3. コミットを PR にする（直 push をやめる）。代表のレビューが挟まるが、
   「管理画面から公開できる」という今回の目的は薄れる

あわせて未確認: **holy-inc / MobileWash の Vercel プロジェクトが push で自動デプロイ
される設定になっているか。** この環境からは Vercel の設定を見られない。なっていなければ
コミットはされてもサイトに出ないので、最初の1本を公開したときに実際の反映を見る。
...
## C2PA 適合性ゲートがフェイルソフトで、CI が緑のまま検査が沈黙しうる（2026-09-14 起票 / 2026-09-21 解決）

> **解決（2026-09-21・代表判断「fail-closed」）**: ネイティブ依存を読み込めないことを
> `ctx.skip()` で逃がすのをやめ、**失敗として出す**ようにした。読み込みは
> `src/lib/anchoring/__tests__/nativeImaging.ts` の `requireNative()` に一本化し、
> 4箇所あった「読み込めなければ skip」を全部そこへ寄せた
> （`c2paSignValidate` ×2 / `c2paSignValidateProduction` ×1 / `imageExif` ×1。
> 最後の1つは C2PA ではないが**同じ形の兄弟**で、直さないと同じ沈黙が残る）。
>
> 陰性対照で確認した —— `node_modules/@contentauth/c2pa-node` を退避して実行すると、
> **変更前は「1 passed / 4 skipped」で緑、変更後は「1 failed」で exit 1**。
> 失敗メッセージには「fail-closed である」「未インストールなら npm ci を見る」
> 「invalid ELF header なら別プラットフォームのバイナリ」が入り、元の例外は `cause` に残る。
>
> なお**起票時の前提のうち1つは、すでに解消していた**: 現在 `npm ci` は
> `@contentauth/c2pa-node` 0.9.5 を実際にインストールし、ゲート4本は実走して通る。
> lockfile が 0.6.0 を記録していた当時とは状況が違う。今回の変更は
> 「また入らなくなったときに黙らせない」ための保険である。
>
> 採らなかった選択肢と、残した論点は下記のとおり。判断の9項目は DECISION_LOG 2026-09-21。
...
- ~~ゲートをフェイルソフトのままにするか、**本番向け検査として fail-closed にするか**。~~
  → **2026-09-21 に fail-closed で決定（代表判断）。** 代償（ネイティブ依存が入らない
  開発環境でローカルテストが落ちる）は承知のうえで採った。
- ~~代替として、**CI でだけスキップ数を検査する**~~ → 採らなかった。CI と手元で
  検査の強さが変わる形は、**手元で緑だったものが CI で落ちる**（型 E）を作る。
  それに「スキップ数が 0 か」は検査の入口が増えるたびに書き足す必要があり、
  書き足し忘れが新しい沈黙になる。
...
- **未決（今回の変更で残ったもう1つ）**: `providers.test.ts` の
  ...
  環境次第で消える**点は同じ。モック駆動にして両方の分岐を常に走らせるのが筋。
...
「falls back to the normal inbound record when there is no active vehicle-photo flow」
付近（133〜135 行目）。前回「どれか取れていない」と書いた失敗も、同じ現れ方なので
これと同一の可能性が高い（**同一だと確認はできていない** —— 前回はテスト名を取れていない）。

**再現条件が絞れた。**

| 実行の仕方 | 結果 |
|---|---|
| 全体実行（`CI=1 npx vitest run`） | 3回に1回ほど落ちる（実測: 5回中2回） |
| **このファイル単体を12回連続** | **12回とも通る** |

つまり**このファイル単体の問題ではない。** 並列実行時の他テストとの干渉か、負荷による
タイミングのどちらか。CI は同じコミットで success（CI の方が実行環境が軽い可能性がある）。

**まだ取れていないもの**: 失敗時の assertion そのもの。全体実行で落ちたときの出力は
ソース断片までしか残っておらず、`--reporter=verbose` で回し直すと通ってしまう。
次に捕まえるなら、全体実行の出力を最初からファイルへ落としておく
（`CI=1 npx vitest run --reporter=verbose > log 2>&1` を失敗するまで繰り返す）。

**この PR の変更とは無関係。** #1056 が触っているのは `db-typegen.yml` と `docs/context/` だけ。

紛らわしい点: `check-schema` の自己テストは**陰性対照としてわざと AssertionError を出す**
（`nosuch_column_xyz` を含む一時ファイル）。ログを目で追うと失敗に見えるが集計上は passed。
切り分けのときにここで足を取られる。

## 誰も何も変えていないのに CI 全体が赤くなる（2026-09-09）

**起きたこと**: 2026-09-09 に main の CI が赤くなった。直前のコミットは
ドキュメントのみ（#1053）。原因は `npm audit` に**新しい勧告が公開されたこと**で、
リポジトリ側の変更は一切ない。

**効き方が重い**: `Security audit` は `Lint, Type Check & Unit Tests` ジョブの
**ステップ6**で、失敗するとステップ7の lint / tsc / テスト一式が `skipped` になる。
つまり赤くなるのは1チェックではなく**検査全体**で、依存を更新するまで
どの PR も中身を検証されない。今回は #1056 がこれに当たった。

**今回の対処は対症療法**: #1054 が `npm audit fix` で CVE 3件を解消して緑に戻した
（`b9dba57e`）。#1056 でも並行して同じ修正を作っていたが、重複していたので破棄した
（RELEASE_LOG 2026-09-09、M-081）。**構造は変えていないので、次の勧告で同じことが起きる。**

**決まっていないこと**:
- audit を**別ジョブに切り出す**か。そうすれば勧告1件で lint / tsc / テストまで
  止まることはなくなる。ただし「赤いが検査は通っている」状態を許すことになり、
  M-047 の系列（赤が常態化して見なくなる）に近づく。
- しきい値 `high` のままでよいか。critical の RCE を無視する気はないが、
  **本番の実行環境に無関係な勧告**（Windows ホスト前提の RCE など。Ledra は
  Vercel の Linux 上で動く）でも同じ強さで全部を止めてよいかは別の話。
- Dependabot に自動更新させるか。現状は人（Claude）が気づいて直しており、
  **気づくきっかけが「別件の PR が赤い」しかない。**
- **同じ赤を複数のセッションが並行して直す問題。** 2026-09-09 に実際に起きた。
...
SQL は**文字列が出来上がるのが実行時**なので対象外。道具を変えても動かない上限で、
...
**未解決4b: ドリフト検出器は「列の欠落」を見ていない【2026-09-10 に実例が出た】。**
`scripts/check-schema-drift.mjs`（#1045）が突き合わせるのはオブジェクトの有無
（テーブル・ビュー・関数・トリガ・enum・イベントトリガ）で、**列は見ていない**。
実例: `vehicle_histories.updated_at` が本番にだけ在り、再生 DB に無かった。
`20260907010100` で本番から書き起こしたトリガ `trg_vehicle_histories_set_updated_at` が、
列の無い再生 DB では UPDATE のたびに 42703 で落ちる（本番は無事。列があることを実測確認）。
このPRでは実例1件を `20260910010100` で塞いだが、**同じ形が他に何件あるかは未調査**。
検出器を列（名前・型・NOT NULL・既定値）まで広げるか、それとも #1052 で見送った
「型名の食い違いは残す」判断と整合するどこかで線を引くかは未決。

**未解決4: 本番に在るトリガが2本、マイグレーションから再生されない【要確認】。**
`trg_set_case_number`（`generate_case_number`）と `trg_job_orders_updated_at`
（`handle_updated_at`）は本番には付いているが、再生 DB では関数だけが在ってトリガが
無い。#1045 が潰したドリフトと同じ形（本番にだけ在る）。**そのせいでこの2本は
plpgsql 検査の対象外になる**ので、検査の穴を塞ぐ意味でも再生側に足したい。
どのマイグレーションに置くか（新規か既存の末尾か）は未判断。

### 決着した3点（2026-09-08）

- **壊れていた間の PII 開示判定の振る舞い** → `insurer_get_certificate` に例外ハンドラは
  無いので握り潰しではない。ただし**そもそも到達していなかった**（出力列と同名の
  `tenant_id` で 42702）。`insurer_access_logs` の `action='view'` は本番で0件、
  `pii_disclosure_consents` も0件。**PII が漏れたことも、同意済みの PII が黙って
  伏せられたこともない** —— 機能が一度も動いていない。
- **plpgsql を検査できるか** → できる。`plpgsql_check` を `replay-migrations.mjs` に
  常設した（DECISION_LOG 2026-09-08）。初回で本番の2件を検出。
- **ビュー・トリガ** → **ビューは構造的に免疫**（定義が作成時に解決されて保存される。
  `search_path='__vs'` で作ったビューが `__vs.t` に書き換わることを実測）。
...
## MISTAKE_LEDGER の ID 衝突が 6 組に増えた（2026-09-07 起票 / 2026-09-15 解決）

> **解決（2026-09-15）**: 連番をやめ、ID を `M-<YYYYMMDD>-<スラッグ>` に変えた。
> 既存 104 件を改番し、**旧番号は見出し末尾に `旧 M-NNN` として残した**ので、
> 本文・他ファイル・コード内コメントからの**参照 450 箇所（MISTAKE_LEDGER 外 234 /
> 台帳本文 216）は1件も書き換えていない**（変更前後で `M-NNN` の多重集合を比較し、
> 減ったものが1件も無いことを確認）。数え方（マージ時のベースは `a55b7cae`）:
> `git grep -hoE '\bM-[0-9]{3}\b' a55b7cae -- . ':!docs/context/MISTAKE_LEDGER.md' | wc -l` → 234、
> `git show a55b7cae:docs/context/MISTAKE_LEDGER.md | grep -vE '^## M-' | grep -ohE '\bM-[0-9]{3}\b' | wc -l` → 216。
> 下で4回失敗している一括置換を、今回は実行していない。
> 重複していたのは6組ではなく **10組**（`M-060`〜`M-065` と `M-081`〜`M-084`）だった
> —— この項が6組で止まっていたのは 2026-09-11 以降に **4組**（`M-081`〜`M-084`、
> いずれも相方が 2026-09-13）増えたため
> （数え直しの経緯は MISTAKE_LEDGER `M-20260915-dupe-count-from-truncated-grep`）。
> 対応表は MISTAKE_LEDGER 冒頭の「ID について」節。
> 再発は `npm run check:ledger-ids`（CI + pre-commit）が止める。
> 判断の9項目は DECISION_LOG 2026-09-15。

**M-047 の重複は解消済み。** 2026-09-06 の「『失敗を知らせる』通知が、シェルの引用ミスで
一度も飛んでいなかった」の方が **M-065 へ改番**され、`## M-047` は1件になった。

**ただし改番先の M-065 がまた衝突している。** 現在 main 上で番号が重複しているのは
**6組（12エントリ）**で、いずれも `origin/main` 側に元からあり、マージ操作が作ったものではない
（マージベース・`origin/main`・こちらの head の3点で同一であることを確認済み）。

| 重複している ID | 片方 | もう片方 |
|---|---|---|
| M-060 | 2026-09-07 型 A（陰性対照を置かなかった） | 2026-09-08 型 A（fail-open） |
| M-061 | 2026-09-06 型 C（参照ゼロ＝未使用と読んだ） | 2026-09-08 型 A（x-forwarded-for） |
| M-062 | 2026-09-06 型 A（検出器の数字を未検証で報告） | 2026-09-08 型 A（周辺だけ test） |
| M-063 | 2026-09-06 型 D（security_invoker を剥がした） | 2026-09-08 型 B（別軸の混同） |
| M-064 | 2026-09-07（十分条件が成り立っていなかった） | 2026-09-08 型 A（文脈を見ない一括置換） |
| M-065 | 2026-09-06（通知が飛んでいなかった＝旧 M-047） | 2026-09-08 型 A（走査スクリプトを信用しかけた） |

**改番は衝突を移動させるだけだった。** M-047 → M-065 の改番が、そのまま M-065 の衝突を作った。
空き番号を探して動かす運用は、並行 PR がある限り同じことを繰り返す。

**決めること**: 連番をやめるか（日付＋連番、あるいはブランチ名を含む ID など）。
「片方を改番する」は既に一度試して失敗しているので、対症療法としても筋が悪い。
なお #1056 では、main 取り込みのたびに同じことが起き、**同一 PR 内で4回繰り上げた**。

| 取り込み | main が取った ID | こちらの移動先 |
|---|---|---|
| 1回目 | M-070 | M-070 → M-071 |
| 2回目 | M-071 | M-071 → M-072 |
| 3回目 | M-072 と M-073（2つ） | M-072 → M-074 |
| 4回目 | M-074〜M-080（**7つ**） | M-074 → **M-081** |

**空き番号は毎回飛ぶ。** 3回目は2つ、4回目は7つ。「main の最大 ID + 1」を毎回引き直さないと当たらない。
衝突を増やさずには済んだが、**これは毎回手で気づいた結果であって仕組みではない。**
繰り上げるたびに、型の表の ID 一覧と他3ファイルからの参照も追随させる必要がある。

**一括置換で3回壊した。同じ壊し方である。**

1. **3回目**: 参照を一括置換したとき、この段落自身にある**経過の表の旧 ID まで置換され**、
   存在しない履歴になった。
2. **4回目（同じことを繰り返した）**: 同じく経過の表が壊れた。
3. **4回目、さらに悪い方**: main 側の docs には **main 自身の M-074 への参照**が
   2箇所あった（DECISION_LOG と RELEASE_LOG。main の M-074 は型 B の別エントリ）。
   一括置換がこれも書き換え、**他人の参照を自分のエントリに向けた。**
   `git show origin/main:<file> | grep -c 'M-074'` と突き合わせて気づき、2箇所とも戻した。

**警告を書いても効かなかった。** 3回目のあと、この項に「一括置換は危険」と明記し、
次回の作業手順にも同じ注意を書いた。**4回目はその手順を読んだうえで、同じ一括置換を実行した。**
手順に書く方式は、この失敗に対しては機能していない。

**何が効くか**: 置換の前後で **main 側の旧 ID 出現数と突き合わせる**
（`git show origin/main:<file> | grep -c '<旧ID>'` が置換後の自ファイルの残存数と一致するか）。
これは機械的に確認できるので、次からは繰り上げ手順の一部として必ず実行する。
根本的には**連番をやめる**方が速い（下記）。

**一括置換は「現在の参照」と「過去の経過」を区別しない。そして「自分の参照」と
「他人の参照」も区別しない。**

**検査で止められるか**: 止められる。`## M-` の重複を数えるだけなので、
`check:context-dates` と同じ場所に1本足せば CI で落とせる。
**2026-09-15 に実装した** —— `scripts/check-ledger-ids.mjs`、CI の並列チェック8本目と
pre-commit フック。書式から外れた見出し・ID と見出しの日付の食い違い・件数の下限割れ・
新 ID の重複・旧番号の余剰の増加の5つで落ちる。
陰性対照10件を `scripts/__tests__/check-ledger-ids.test.ts` に固定した。
...
- **どれだけ露出していたか（2026-09-11 に計測済み）。** 公開ページ側は修正後 0 件。
  顧客ポータル側は **14 行（IP 6 / uid 8、9 証明書・4 テナント）** が見えていた
  （RELEASE_LOG 2026-09-11）。**いつからか**は `performed_at` で遡れるが、
  **顧客が実際に開いたかの記録が無い**ので「見られたか」は確定できない。ここは未解決。
- **既定で公開されている他の列が無いか。** 今回は `vehicle_histories.type` を
  許可リストに反転したが、同じ「除外リストで守っている」形が他に無いかは未点検。
...
- ~~**他の公開経路**（公開 PDF・外部 v1 API）が同じテーブルを読んでいないかは未確認。~~
  → 2026-09-11 に `vehicle_histories` に触る全 15 ファイルを列挙して分類した。
  テナント外へ出るのは公開証明書ページと顧客ポータルの 2 経路のみで、どちらも
  許可リストで絞っている。**分類漏れはテストが落とす**
  （`audit/__tests__/privateAuditTypes.test.ts`）。
...
## MISTAKE_LEDGER の連番 ID が、並行 PR のたびに衝突する（2026-09-04 起票 / 2026-09-15 解決）

> **解決（2026-09-15）**: 対策候補 (a)（日付ベースの ID）を採用した。
> `M-<YYYYMMDD>-<スラッグ>` で既存 104 件を改番し、旧番号は別名として見出しに残した
> （既存 26 件という起票時の数字は、その後 104 件まで増えている）。
> 「新規分だけ新形式にして混在させるか」は**全件改番**を選んだ —— 混在させると
> 「この ID はどちらの方式か」を毎回判断することになるため。詳細は DECISION_LOG 2026-09-15。
...
~~**代表判断待ち。** それまでは連番のまま運用する。~~ → 2026-09-15 に (a) で決定。
...
- **書き起こし**: 関数 18・トリガ 3・ビュー 1・enum 型 5・イベントトリガ 1
...
## 追加（2026-09-13・モバイル表示）

- **`src/app/admin/documents/DocumentForm.tsx` の行アイテム編集がモバイルで破綻している。**
  `grid-cols-[28px_96px_minmax(0,1fr)_64px_60px_88px_72px_96px_104px_56px]` の固定10トラックで、
  固定幅の合計だけで 664px、gap を入れると 736px 以上必要。**モバイル用の代替表示が無い。**
  class の接頭辞では直らず、行ごとのカード表示にする**再設計**が要る。
  回帰検査（`responsiveGrids.test.ts`）は入力欄入りの固定グリッドを見るので、
  この形は検出対象だが**直せていない**ため対象から外していない —— 直すまで落ち続ける
  のを避けるため、検査は素の `grid-cols-N` を見ており、この行は算出幅の指定なので拾わない。
  **つまり自動では止められない。** → 判断待ち
...
- **【判断済み・実施待ち】Supabase の GitHub 連携（Branching）による本番への自動適用を切る。**
  **2026-09-13 に代表が「切る」と判断**（DECISION_LOG 2026-09-13）。設定変更はダッシュボード操作で、
  ...
  完了後に `list_branches` で既定ブランチが本番に紐づかなくなったことを確認する。
...
- 起票日: 2026-07-28
- 追記（2026-09-21 G5＝指定整備記録簿の様式まわりを Web 調査）:
  - 様式の実体: 指定整備事業規則第10条の２の法定様式＝**第三号様式（四輪）／第四号様式（二輪）**。**令和6年10月に新様式へ変更済み**（e-Gov・各振興会資料で確認）。記録簿は法第94条の６の**内部保存帳簿**で、どこかへ提出するものではない。→ G5（様式適合）は本来「文書レンダリング」の要件で、既存 PDF 基盤の応用で**自前充足が可能**。API/OSS 連携は必須依存ではない。
  - 別レイヤーの整理: 記録簿様式(G5) と 電子保適証・継続検査OSS申請 は別物。保適証（適合証）は「保適証サービス（電子保安基準適合証システム＝通称 AIRAS、日整連運営）」経由で継続検査OSSへ提出。整備ソフトからの集約ファイル送信など第三者ソフトの参入経路はあるが、AINAS（クライアント証明書）・記録等事務代行者指定・日整連/自検協会員といった**認定ゲートのある重い連携**で、開放 REST API ではない。開放 API は国交省の**車検証閲覧API（2024年4月〜・読み取り専用）**程度。
  - 代表の見立て「指定様式は API/OSS 連携次第」への回答: 記録簿様式そのものには当てはまらない（自前レンダリングで可）。連携が“次第”になるのは**保適証・継続検査OSS の側**で、そこは認定・連携先次第で重い。
  - 最大の前提（未確定）: そもそも Ledra の顧客に**指定工場（民間車検場）が居るか**。居なければ G5・保適証・OSS はすべて N/A。ここが未確定のうちは G5 実装に着手しない。
  - 次アクション更新: (1) 顧客に指定工場が含まれるかを代表が確定〔最優先・全体の前提〕。(2) 含まれる場合のみ、様式出力は「自前で第三号/四号様式をレンダリング（軽い）」を第一候補、「既存OSS連携ソフト/AIRAS へ委譲（重い・認定要）」を対抗として比較。(3) 保適証/継続検査OSS 連携に踏み込むかは別テーマとして切り出す。
  - 出典: e-Gov 指定自動車整備事業規則（第10条の２・第三号/四号様式）／各振興会「令和6年10月 記録簿新様式」／日整連 保適証サービス・継続検査OSSマニュアル／自検協 スマート継続OSS／国交省 車検証閲覧API。
...
## ~~db-migrate 失敗通知の「通知経路そのものが生きているか」の確認方法~~（2026-08-15 起票 / 2026-09-21 解決）
- 結論: **案C（次に本当に失敗したときに届いたか見る）で答えが出た。**「確認用ワークフローを足す」
  （案B）は不要だった。通知ステップ自身が失敗すると**ステップが赤くなる**ように作ってあり
  （`curl --fail-with-body` / `set -euo pipefail` / `continue-on-error` を付けない）、
  その赤が実際に発見の手がかりになった（M-20260906-notify-never-delivered）。
- 実測（2026-09-21・`db-migrate` の実行履歴と各実行の head_sha のファイル内容で判定）:
  2026-08-16 以降 40 回実行し **17 回失敗**。そのうち通知が
  **配信されなかった 8 回 / 通知ステップが成功した 9 回**。
  ただし後者を「配信された」と言い切れるのは **#76 の 1 件だけ**（ログに Slack webhook の
  応答 `ok`）。残り 8 件はステップの conclusion が success であることまでしか見ておらず、
  ログは未確認【要確認】。シークレット未設定なら warning を出して `exit 0` する経路も
  同じく success になるため、**「壊れていないコード＋ステップ成功」は配信の証明ではない**
  （Codex レビューの指摘。2026-09-21）。
  配信されなかったのは #47・#48・#49（08-29）、#51（08-30）、#56（09-04）、#59（09-05）、
  #61・#62（09-06）。原因は jq プログラム内に直書きした文言のシングルクォートで、
  `b82e2a0`（#1042）で `--arg` 化して修復済み。#63（09-06）以降は配信されている。
- 判定の当たり取り: 「配信された」側は #76 のログに Slack webhook の応答 `ok` があり
  シークレット未設定時の warning が無いこと、「配信されなかった」側は #51 の通知ステップの
  conclusion が `failure` であることで、陽性・陰性の両方で確認した。
- 残った天井: **気づくまで 8 日・8 回分の失敗通知が落ちた。** 通知の故障は「ジョブが赤い」
  としか見えず、赤の理由がマイグレーション失敗なのか通知の故障なのかは、ログを開くまで
  区別できない。ここは今も同じ。
- 再発防止として入ったもの: `scripts/__tests__/dbMigrateNotify.test.ts`
  （ワークフローYAMLから `run` を抜き出して実行し、jq プログラム本文にシングルクォートが
  無いことまで見る）。**ただし対象は `db-migrate.yml` のみ** — 下の項目へ続く。
- 起票日: 2026-08-15（解決: 2026-09-21）

## db-migrate 以外の重要ワークフローの失敗可視性（2026-08-15・2026-09-21 に実測で更新）
- 状況（2026-09-21 実測）: ワークフローは全 **17 本**、うち「失敗を Slack に通知」ステップを
  持つのは **2 本**（`db-migrate.yml` / `vercel-deploy.yml`）。
  **ただしカバレッジは実質 1 本のまま。** `vercel-deploy.yml` のトリガーは
  `workflow_dispatch:` **のみ**（`branches: [main]` はコメントアウト）で、ヘッダにあるとおり
  「Vercel の GitHub 連携が止まったときに**リポジトリ側から明示的に叩く**ための緊急レバー」。
  通常の本番デプロイは Vercel の Git 連携が行うので、**この起票が想定している
  「無音で止まる」経路はこのワークフローでは覆えない**（2026/08 の 9 コミット取り残しが、
  まさにその経路で起きた）。通知ステップが在ることと、その経路が監視されていることは別
  （Codex レビューの指摘。2026-09-21）。
  残り 15 本（`ci` / `supabase-advisors` / `db-typegen` / `stale-migration-check` など）は未対応。
- **先に片付けたい穴**: コピー先の `vercel-deploy.yml` が
  `scripts/__tests__/dbMigrateNotify.test.ts` の対象外（テストは `db-migrate.yml` を名指しで読む）。
  しかも通知本文を **jq プログラム内に直書き**している（`text: "2026/08 には Vercel の…"`）。
  これは db-migrate の通知を 8 回分落としたのと同じ形で、現時点ではシングルクォートが
  たまたま入っていないだけ。**この一文を誰かが編集した瞬間に、同じ壊れ方をして誰も気づかない。**
- 選択肢: 案A 同じ通知ステップを重要ワークフローへ個別にコピーする（明示的だが重複し、
  今回のようにテストの対象漏れが起きる）／案B `workflow_run` をトリガーにした共通の失敗通知
  ワークフローを1本作り、対象を列挙する（重複しないが間接的で、対象の登録漏れに気づきにくい）／
  案C 現状維持。
- 影響範囲: 誤ると「気づかないまま壊れている」状態が他の領域でも起きる。緊急度は中。
  ただし上記の「テスト対象漏れ」だけは、通知を増やすほど広がるので先に塞ぐ価値がある。
- 次のアクション: (1) `dbMigrateNotify.test.ts` を「通知ステップを持つ全ワークフロー」を
  走査する形に変え、`vercel-deploy.yml` も対象にする。あわせて本文を `--arg` へ追い出す。
  (2) そのうえで案B（共通化）をやるかを判断する。コピーが 3 本目に増える前が分かれ目。
- 起票日: 2026-08-15（2026-09-21 に実測で更新）
