...
- **カード番号決済（Stripe Checkout）経路に重複防止が無い。** Tap to Pay 側は
  `payments_stripe_payment_intent_id_key` で塞いだが、こちらは `/pos/checkout` を
  呼ぶだけで PaymentIntent を持たない。ポーリング完了後に記録が失敗して再操作されると
  二重に記録されうる。`qr-status` は `payment_intent_id` を返しているので、
  `/api/mobile/pos/checkout` に受け口を足せば同じ形にできる。→ 技術的負債
- **Stripe Terminal のリーダー（S700/S710 等）の手入力が日本で使えるか。【要確認】**
  使えるなら、店の端末で開く方式より現場の手順が短くなる。→ 実測待ち
- **対面で店員がカード番号を打つことのカードブランド規則上の扱い。【要確認】**
  現状は「この端末で開いてお客様に入力していただく」前提で文言を書いている。→ 代表判断

## 追加（2026-08-24 夕）

- **Web の作成経路で `store_id` を入れるか。** 現在、証明書・予約・入金は Web から
  作ると `store_id` が null になる（本番の全 225 行が null）。入れるなら各作成画面に
  店舗の選択が要る。入れたらモバイルの `scopeToStore()` は `.eq()` に戻してよい。
  入れない場合、複数店舗のテナントでは店舗未設定の行がどの店舗にも出続ける。→ 代表判断
- **既存 225 行の `store_id` をどうするか。** どの行がどの店舗のものかはデータから
  決められない。推測で埋めない。単一店舗のテナントだけ埋める案はあるが未検討。→ 代表判断
...
- 残っているもの: Terminal 以外の決済経路（Stripe Checkout / advance）の冪等キー付与と、
  モバイル/Web の advance 処理の共通化。
- 次のアクション: advance の共通化のリファクタ規模を見積もってから判断
- 起票日: 2026-07-24（2026-08-24 更新）
