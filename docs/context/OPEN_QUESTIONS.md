...
> 既存 102 件を改番し、**旧番号は見出し末尾に `旧 M-NNN` として残した**ので、
> 本文・他ファイル・コード内コメントからの**参照 440 箇所（MISTAKE_LEDGER 外 229 /
> 台帳本文 211）は1件も書き換えていない**（変更前後で `M-NNN` の多重集合を比較し、
> 減ったものが1件も無いことを確認）。数え方:
> `git grep -hoE '\bM-[0-9]{3}\b' origin/main -- . ':!docs/context/MISTAKE_LEDGER.md' | wc -l` → 229、
> `git show origin/main:docs/context/MISTAKE_LEDGER.md | grep -vE '^## M-' | grep -ohE '\bM-[0-9]{3}\b' | wc -l` → 211。
...
> `M-<YYYYMMDD>-<スラッグ>` で既存 102 件を改番し、旧番号は別名として見出しに残した
> （既存 26 件という起票時の数字は、その後 102 件まで増えている）。
