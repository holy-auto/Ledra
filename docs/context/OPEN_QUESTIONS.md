...
- **アプリのデプロイとマイグレーション適用が競合しうる。** `is_system=eq.false` を送る
  ...
  400（column does not exist）を返す。実測では `DB migrate` は **25〜60 秒**で終わり
  （run 88: 14:53:06→14:54:06、run 90: 15:50:09→15:50:34 UTC）、Vercel のビルドは
  **2〜8分**かかる（プレビュー実測）ので通常は適用が先に終わる。
  #1151 でも実際に適用が先だった。ただし**順序が保証された作りではない**
