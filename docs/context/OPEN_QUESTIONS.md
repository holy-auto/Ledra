...
pre-commit フック。書式から外れた見出し・新 ID の重複・旧番号の重複増加・件数の下限割れの
4つで落ちる。陰性対照7件（重複 ID／旧形式の残存／大文字スラッグ／表題の付け忘れ／
旧番号の重複11組／見出し書式の変更／エントリ削除）を
`scripts/__tests__/check-ledger-ids.test.ts` に固定した。
