...
（RELEASE_LOG 2026-09-09、M-071）。**構造は変えていないので、次の勧告で同じことが起きる。**
...
- **同じ赤を複数のセッションが並行して直す問題。** 2026-09-09 に実際に起きた。
  #1054 と #1056 が同じ CVE 3件を別々に直し、後者が丸ごと無駄になった（M-071）。
...
なお 2026-09-09 の #1056 では、main が先に M-070 を取っていたため**こちらを M-071 へ回して
7組目を作らずに済んだ**が、これは手で気づいた結果であって仕組みではない。
