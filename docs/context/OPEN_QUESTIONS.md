...
## `insurer_tenant_accesses`（複数形）と、全組み合わせを自動付与する2本のトリガをどうするか（2026-09-03）

保険会社の閲覧許可テーブルが**2つ並存している**。実害は現時点ゼロだが、放置すると危険。

- **単数形 `insurer_tenant_access`** — 実際の認可判定が読む方。`insurer_accessible_tenant_ids()`
  および `/api/insurer/tenants`・`/api/insurer/vehicles/[id]`・`/api/insurer/btob-match`・
  管理側の `/api/admin/insurers/tenant-access` が参照。現在 **2行**（デモ→デモ、
  および無効化済みの `東京海上日動`→`HOLY AUTO`）。
- **複数形 `insurer_tenant_accesses`** — **アプリコード・DB関数・RLSポリシー・ビューの
  いずれからも読まれていない**（境界をアンカーした正規表現で確認済み）。
  にもかかわらず `insurers` と `tenants` の2本のトリガ
  （`trg_seed_all_tenant_accesses_for_new_insurer` /
  `trg_seed_all_insurer_accesses_for_new_tenant`、どちらも有効）が
  **保険会社・テナントが増えるたびに全組み合わせを `is_active=true` で書き込み続けている**。
  現在 **48行 = 保険会社2社 × テナント24社**。
- **なぜ危険か**: 今は誰も読んでいないので実アクセスは発生しない。しかし
  **この表を参照するコードが1行書かれた瞬間に、全テナントが全保険会社に見える。**
  しかも中身は「全組み合わせ許可」なので、書いた人は「既存の許可テーブルを使った」と
  思うだけで穴が空く。テーブル名が単数/複数の1文字違いなのが特に危ない。
- **判断が必要な点**: 複数形は単数形の後継として作りかけたものか、それとも残骸か。
  前者なら移行を完了させる（ただし「全組み合わせ自動付与」は設計として妥当か別途判断が必要）。
  後者ならトリガ2本と表を落とす。**コードからは意図を判定できない。**【要確認】
- 今回は範囲外として手を付けていない（DECISION_LOG 2026-09-03 参照）。
