...
## 人が入力しない証明書作成パスで走行距離をどう埋めるか（2026-08-25）
- 状況: 走行距離の必須化はフォーム・Server Action・外部/オフラインJSON API・モバイルに入れたが、
  **人が値を入力する画面を持たない2経路が残っている**（コードレビュー2巡目で発覚）。
  1. `src/lib/ai/automation/certificateRecordAuto.ts` — AIが作業記録から証明書を自動生成する。`active` で自動発行もしうる。
  2. `POST /api/certificates/create` — もう1つの作成API。
  どちらも `maintenance_json` を書かないため `vehicle_mileage_logs` に何も積まれない。
  必須化しても入力する人がいないので、今回は変更していない。
- 選択肢:
  - 案A: 元データ（作業記録・予約・車検証OCR）に走行距離があれば拾って `maintenance_json.mileage` に載せる。
    人手を増やさずに埋まる可能性がある。元データに無ければ従来どおり空。
  - 案B: これらの経路で作られた証明書を「走行距離なし」として一覧で可視化し、後から入力させる。
  - 案C: 現状のまま（自動生成分は走行距離を持たない前提で運用する）。
- 影響範囲: 自動生成の割合が高いテナントでは、必須化しても走行距離が思ったほど溜まらない。
  残価判定や劣化予測の母数に直結する。
- 次のアクション: まず必須化後の実データで「手入力ぶんと自動生成ぶんの比率」を見る。
  自動生成が多いようなら案Aを検討する。
- 起票日: 2026-08-25
...
- 状況: 証明書の新規作成で走行距離を必須化した（DECISION_LOG / RELEASE_LOG 2026-08-25）が、3つ残っている。
  ...
  3. **モバイルの変更は実機未確認。** 型検査・単体テストはローカルと CI の両方で緑だが、
