...
## `remote_schema` が本番から消した11列をどうするか（2026-09-20）

**原因は解けた**（DECISION_LOG 2026-09-20）。`20260918142610 remote_schema` が本番で
`DROP TABLE workshop_capability_profiles` と **`DROP COLUMN` 13件**を実行していた。
repo 側の同じファイルは「破壊的操作を含むので空にした」と書かれた**コメント4行だけ**なので、
再生 DB では一度も落ちない。本番だけが実行した形。

復旧済みは2つ（`certificates.certificate_no` = `20260918150000`、
`workshop_capability_profiles` = `20260919150000`）。**残り11列は本番に無いまま**:

| 表 | 列 |
|---|---|
| `audit_logs` | `device_id` / `ip_address` / `new_values` / `old_values` / `performed_by` / `reason` / `record_id` / `table_name` |
| `insurers` | `max_users` |
| `templates` | `updated_at` |
| `tenant_memberships` | `updated_at` |
| `tenants` | `updated_at` |

**アプリは壊れていない**（2026-09-20 実測）。`scripts/schema.snapshot.json` は本番から
取り直した版で、本番と**表 277・列差 0 で一致**しており、`check:schema` が通る＝
消えた列を引くコードは1本も無い。`audit_logs` への書き込みは `src/lib/audit/tenantLog.ts`
に集約され、**意図的に `query_json` へ入れている**（消えた列名は使わない）。

決めていないこと:

- **11列を本番へ戻すか、マイグレーション側を本番に合わせるか。** マイグレーションは今も
  この11列を作るので、**再生 DB と本番が11列ぶんずれたまま**。新しい環境（プレビュー分岐・
  再生）と本番で形が違う状態が続く。`audit_logs` はアプリが12列の形を前提にしているので、
  マイグレーション側（20列）を本番に寄せるのが素直に見えるが、product 判断が要る【要確認】。
- **消えた列のデータは戻らない**（DROP COLUMN のため）。2026-09-18 14:26 UTC より前の
  バックアップからしか取れない。推定: `audit_logs` の8列は空だった可能性が高い ——
  `tenantLog.ts` の記述では、テナント側6箇所の書き込みは存在しない列へ入れていて
  **1件も残っていなかった**（349行はすべて保険会社側の RPC 経由）。**未検証**（列自体が無いので確かめられない）。
- **誰がいつ本番へ当てたのか**【要確認】。台帳の `created_by` は全行 null で、出所を持っていない。
  当時 `db-migrate` は動いていた（連続失敗は #1094 のマージ 2026-09-18 16:03 から）ので、
  14:26 の適用はワークフロー以外の経路（手元の `db push` か `apply_migration`）と推定。**未検証**。
- 同じ形（台帳にあるのに実体が無い）が他の版にも無いかは**未調査**。表とビューは
  277 で一致したが、**関数・トリガ・ポリシー・索引は突き合わせていない**。
