...
- 状況: クエリの不整合は解消したが、「画面にはあるのに DB に置き場が無い」項目が残る。
  - **代理店の銀行口座（銀行名・支店・口座種別・口座番号・名義）とウェブサイト URL**:
    `agents` に列が無い。現在は保存しようとすると「この項目は現在保存できません」を返す。
  - 証明書の等級 / 点検日 / PDF URL、`certificates` の `work_areas` / `category`。
- 選択肢: `tenants.bank_info` と同じ `jsonb` を `agents` に足す / 画面から項目を消す /
  当面このまま（保存できないと返す）
- 影響範囲: 代理店への支払いに口座情報が要るなら、現状は別管理になっている【要確認】。
- 次のアクション: 代表に、代理店の口座情報を Ledra 上で持つ必要があるか確認する
- 起票日: 2026-08-23
