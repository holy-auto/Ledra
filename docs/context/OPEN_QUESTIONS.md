...
3サイト（holy-inc.jp / Ledra / MobileWash）の相互リンクを張る作業で、
MobileWash について2つの食い違いが見つかった。どちらもこの環境からは
決着できない（外部ドメインへ出られず、登記情報も参照できない）。

**1. ドメイン**

- MobileWash リポジトリ: `index.html` の canonical が `https://mobilewash.app/`。
  リポジトリ内の全URL（14箇所）も `.app`。
- holy-inc.jp リポジトリ: トップの事業紹介カードが `https://mobilewash.jp` を
  参照していた（1箇所のみ）。

相互リンクは canonical 側（`.app`）に統一した。残る問いは
**`mobilewash.jp` を自社が保有しているか、保有しているなら `.app` へ
301 リダイレクトしているか**。保有していて未設定なら設定が要るし、
保有していないなら他社ドメインへリンクしていたことになる。

**2. 運営主体**

- MobileWash リポジトリ `src/mocks/companyAbout.ts`: 「MobileWash株式会社」
  （設立 2025年4月1日、資本金10万円、代表 堀越友輔、連絡先 info@holy-inc.jp）。
- holy-inc.jp リポジトリ `src/mocks/company.ts`: 株式会社HOLY の事業として
  「出張洗車サービス（MobileWash）」を掲載し、`/mobilewash` にブランドページを持つ。

MobileWash が別法人なのか、株式会社HOLY の一事業ブランドなのかが読み取れない。
この状態だと、どちらのサイトの会社概要が正しいのかを訪問者が判断できない。
相互リンクの文言は、決着するまで関係を断定しない書き方（「関連サイト」「グループサイト」）
にしてある。Ledra 側だけは `/law` の特定商取引法表記で販売事業者名が
株式会社HOLY と明記されているため、「運営会社」と書いてよいと判断した。

決着に必要なもの: (a) 保有ドメインの一覧、(b) MobileWash の法人格の現状。
どちらも事業側にしか無い情報。
