...
## 職人の実績がテナントを跨いで名寄せできない（2026-09-01 → 2026-09-03 更新）

- **解決した部分**: 「本人が確認する導線が無い」は 2026-09-03 に解決した。テナントが
  発行する読み取り専用リンク `/w/[token]` で、本人が自分の施工証明を一覧できる
  （DECISION_LOG 2026-09-03、`20260903000000`）。
- **残っている部分（本丸）**: 同じ職人が複数の元請けに登録されると**元請けごとに別の
  `staff_members` 行**になる（一意制約はテナント内）。名寄せの手段が無いため
  「この職人の通算施工実績」が原理的に出せない。実績リンクも1テナント分しか出せない。
  47都道府県で複数の元請けの下に入る働き方では、そのまま自分の実績が分散する構造。
- 選択肢: (1) 職人側の識別子（電話番号ハッシュ / メール）でテナントを跨いで同定し、
  同意した分だけ束ねる。(2) 職人に軽量アカウントを持たせて `staff_members.user_id` で
  束ねる（RBAC とプラン課金の扱いが増える）。(3) 現状維持（テナントごとの実績のみ）。
- 影響範囲: 「職人の実績が Ledra に貯まる」という価値が、複数元請けで動く職人ほど
  薄くなる。代表自身の働き方がまさにこれ。
- 次のアクション: 代表判断。1 も 2 も**他社に自分の稼働先が見えてよいか**という
  同意設計が先に要る（A社の元請けが「この職人はB社でも働いている」と分かってよいか）。
  そこを決めないと実装の形が決まらない。
