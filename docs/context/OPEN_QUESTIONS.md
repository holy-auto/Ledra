...
## 本番へのスキーマ適用が止まっている —— 案1を採用、残りは #1097 待ち（2026-09-19 更新）

**決着した部分**: 代表判断で案1（#1093 を先にマージ）。#1093 は7版すべてをファイルで持っており、
CI の赤3件は `c14f339` で解消した。案2・案3は採らない（下に理由を残す）。

**まだ終わっていない**: 本番台帳はその後さらに動き、`20260919132119` / `20260919134412` が
main に無い状態になった（#1097 が持つ）。**復旧には #1093 と #1097 の両方のマージが要る。**

**新しく出た未解決**: `workshop_capability_profiles` は台帳に適用済みと記録されているのに
本番に存在しない。`20260919150000` で作り直すが、**なぜ台帳だけ進んだのかは不明**【要確認】。
同じことが他の版で起きていないかは未調査（台帳の全版と実体の突き合わせは未実施）。

以下は判断時の記録。


`db-migrate` が2回続けて失敗している（#1094 のマージ・#1095 のマージ）。
本番の台帳に main へ無い版が7つあり、`supabase db push` がそこで止まる。
**止まっている間、`certificates.certificate_no` も外注施工履歴の4表も本番に入らない。**
`/admin/outsourced-work` は本番で動かず、保険会社ポータルの証明書詳細も 42703 のまま。
**以降のスキーマ変更も同じ理由で全部止まる。** どれを採るかは代表判断が要る。

- **案1: PR #1093 を先にマージする。** `ft_` 系5版と `workshop_capability_profiles` の6版が
  main に入る。残るのは `20260918142610 remote_schema` 1版だけなので、これをファイル化すれば揃う。
  **#1093 のマージ可否とタイミングが未確定**【要確認】。
- **案2: 7版のファイルを本番台帳から復元して main へ置く。** `statements` が本番に保存されているので
  内容は忠実に戻せる。リポジトリ側だけで済み、git で戻せる。ただし **#1093 がマージされたとき
  同じ版番号のファイルが二重になる**ので、#1093 側の削除とセットでないと成立しない。
- **案3: `supabase migration repair --status reverted` で7版を台帳から外す。** 1コマンドで通るが、
  **本番の台帳を書き換える不可逆操作**。実体（`ft_*` 12 表など）は残るため、後から #1093 が
  マージされると同じ DDL が再適用される。冪等でない文があれば、そこで再び止まる。

付随して未確定:

- `20260918142610 remote_schema` を**誰がいつ本番へ入れたか**【要確認】。368 文の `db pull` 由来で、
  REVOKE と COMMENT が中心。main には対応するファイルが無い。
...
**【2026-09-20・顧客データ側は解決】** `20260920092000` で
...
**2026-09-20 追記**: `20260920092000` で並び順だけは揃えた
