...
1. マイグレーション適用（`documents.public_id` ＋ 索引）
2. Web デプロイ（`/receipt/[public_id]` と `/api/receipt/pdf`）
3. モバイルのビルド提出
...
（`src/lib/api/__tests__/aiRouteRateLimit.test.ts`）。**残っているのは検出器の前提の話。**

この検出器の根は「モジュールが `getAnthropicClient()` を呼ぶか」。
将来、別ベンダーの SDK を直接叩く経路や、HTTP で外部の推論 API を叩く経路が入ると、
**この検出器は見落とす**。検出器は必ず不完全であるという前提は変わらない
（MISTAKE_LEDGER 型 A）。

案: 費用が出る外部呼び出しを1箇所に集約し（`src/lib/ai/client.ts` に相当するものを
ベンダーごとに作るのではなく、共通の入口を1つ持つ）、検出器の根をそこに置く。
...
## 免除した2つのジョブが持つ件数上限が妥当か（2026-09-03）

AI のレート制限を免除したのは2つ。どちらもユーザーが繰り返し叩ける経路ではないが、
**自前で持つ上限が妥当かは検証していない**。

- `cron/daily-digest` — cron 認証 + `withCronLock(600秒)`。テナント数ぶんループするので、
  テナントが増えると1回の実行あたりの AI 呼び出しも増える。上限の記述が無い。
- `qstash/line-history-import` — QStash 署名必須。1回の実行件数の上限は
  `LINE_HISTORY_IMPORT_MAX`（既定80）。この80という数字の根拠は不明。**【要確認】**

- 起票日: 2026-09-03
- 判断者: 未定

## 事業ログの見出し日付をコミット日と照合できないか（2026-09-03）

`docs/context/` のエントリ見出しに書いた日付が、そのコミットの日付とずれていても
何も落ちない。実際 2026-09-03 に、2日先の `2026-09-05` を6つのソースコメントと
事業ログ4ファイルに書いた（MISTAKE_LEDGER M-011）。マージ直前に人力で気づいた。

DECISION_LOG の「1. 日付」は**その記録自体が唯一の出典**なので、誤ると後から
検証する手段が無い。コードコメントより実害が大きい。

案: `scripts/check-context-dates.mjs` — 追加された `## YYYY-MM-DD` 見出しの日付が
`git log -1 --format=%ad --date=short` と一致するかを検査する。
過去エントリの遡及修正は正当な操作なので、**新規追加行のみ**を対象にする必要がある。

未解決: 遡及追記（後から気づいた出来事を過去の日付で書く）をどう許すか。
`<!-- backdated: 理由 -->` を許容する形が考えられるが、抜け道にもなる。
...
test:coverage / check:schema）を `&` で並列実行し `wait $PID || exit 1` で待つ。
そのため**ログの末尾は失敗したチェックの出力とは限らない**。

2026-09-01 に実際に2回誤読しかけた。「lint の出力の直後に exit 1」と出るのに
`npm run lint` 自体は 0 errors で、失敗元は別のチェックだった。生ログの配信元
（Azure Blob）はこの環境のプロキシで遮断されており、GitHub の API も末尾しか
返さないため、切り分けにローカル再現が必要になった。

決めたいこと: 失敗元を明示する形にするか（各チェックの終了コードを個別に出す、
...
- **レビュー待ちの長い PR が自分自身のマイグレーションバージョンを陳腐化させる。**
  マイグレーションファイルの日付をドラフト作成時点で決め打ちしていると、レビュー・
  マージ待ちの間に main 側で日付の新しいマイグレーションが先に本番へ適用され、
  自分の方が「本番の適用済み最新より古い未適用ファイル」（out-of-order）になって
  db-migrate が停止する。このリポジトリで4回目（run #972・#976・#977・今回）。
  対策候補: (a) マイグレーションのバージョン採番をマージ直前に行う運用へ変える、
  (b) レビュー期間が一定日数を超えたドラフト PR は事前に日付の妥当性を再チェックする
  CI ステップを足す、(c) 現状維持（改名で都度対処）。代表判断待ち。
...
- テーブル自体も未完成の疑いがある: RLS は有効だが SELECT ポリシーのみで、INSERT/UPDATE の
  ...
  存在している状態。
- 今後の運用として、Supabase MCP で本番へ直接マイグレーションを当てる方式を続けるか、
  `db-migrate.yml` 経由（PR レビュー・CI 通過後にのみ適用）へ一本化するかは、
