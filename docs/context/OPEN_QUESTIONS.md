...
## 通知の遷移先がモバイルに存在しない（タップしても飛べない通知が残る）（2026-08-23）
- 状況: 通知タップで該当画面へ飛ぶ導線を実装したが、`notifications.link_path` は管理画面向けの
  Web パスで入る。現在アプリケーションが実際に発行しているのは `/admin/messages` と
  `/admin/documents?doc_type=estimate` の2種のみで、**どちらもモバイルに対応画面が無い**。
  変換表（`apps/mobile/src/lib/notificationTarget.ts`）は用意し、対応が無いものは遷移させず
  chevron も出さない（押せそうに見えて何も起きない行を作らない）が、結果として
  「タップして飛べる通知」は今のところ存在しない。
- 選択肢:
  - 案A: モバイルにメッセージ画面・見積書画面を作る（対応画面が増えれば変換表に1行足すだけで有効化）
  - 案B: 通知の発行側（Web）で `link_path` を予約・証明書などモバイルにもある画面へ向ける
  - 案C: 対応画面が無い通知はモバイルへ配信しない
- 影響範囲: 放置しても壊れはしないが、「通知から作業に入る」という導線が機能しないままになる。
- 次のアクション: 代表がモバイルで受け取りたい通知の種類を決める → 案A/B/C を選ぶ
- 起票日: 2026-08-23
