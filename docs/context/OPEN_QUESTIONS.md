...
## Web 側にも同種のクエリ不整合が 186 件ある（2026-08-23）
- 状況: モバイル用に作ったスキーマ照合の仕組みを `src/`（Web）に対して試しに走らせたところ、
  2015 件のクエリ中 **186 件が実スキーマに無い列・テーブルを参照**していた。
  例: `src/app/admin/vehicles/[id]/page.tsx` と `src/app/api/certificates/edit/route.ts` が
  `certificates.certificate_no`（モバイルで存在しないと確認した列）を SELECT している。
  未対応。モバイルと違って Web にはテストがあるため、全部が壊れているとは限らない
  （ビュー経由・動的な列名・検出側の誤判定の可能性がある）【要確認】。
- 選択肢: 同じ照合を Web にも入れて潰す / `db:typegen` の生成型で型付けして tsc に任せる /
  当面モバイルだけ守る
- 影響範囲: 同じ「データはあるのに空に見える」不具合が管理画面側にもある可能性。
- 次のアクション: 186 件の内訳を確認し、本当に壊れているものを切り分ける
- 起票日: 2026-08-23
