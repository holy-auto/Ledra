...
- 次のアクション: モバイルの次回ビルド前に案A〜Cを決める。型検査は CI（`Mobile Typecheck & Unit Tests`）で
  通っているので、残るのは実機での動作確認のみ。
- 起票日: 2026-08-25
...
- 状況: 証明書の新規作成で走行距離を必須化した（DECISION_LOG / RELEASE_LOG 2026-08-25）が、3つ残っている。
  ...
  3. **モバイルの変更は実機未確認。** CI の `Mobile Typecheck & Unit Tests` は緑なので型検査は通っているが、
