...
- **この状態の PR は、必須チェックが設定されていればマージできない。** 代表かリポジトリ管理者が
  手で close→reopen するか、Actions 側の設定・稼働状況を見るしかない。
  次に着手するなら `ci.yml` に `workflow_dispatch` を足しておくと、
  再発時に手動で起動できるようになる（今回のような手詰まりを避けられる）。
