...
（RELEASE_LOG 2026-09-09、M-074）。**構造は変えていないので、次の勧告で同じことが起きる。**
...
- **同じ赤を複数のセッションが並行して直す問題。** 2026-09-09 に実際に起きた。
  #1054 と #1056 が同じ CVE 3件を別々に直し、後者が丸ごと無駄になった（M-074）。
...
## MISTAKE_LEDGER の ID 衝突が 6 組に増えた（2026-09-07 起票 / 2026-09-10 更新）
...
なお #1056 では、main 取り込みのたびに同じことが起き、**同一 PR 内で3回繰り上げた**。

| 取り込み | main が取った ID | こちらの移動先 |
|---|---|---|
| 1回目 | M-070 | M-070 → M-071 |
| 2回目 | M-071 | M-071 → M-072 |
| 3回目 | M-072 と M-073 の2つ | M-072 → **M-074** |

3回目は**空き番号が2つ飛んでいた**。「main の最大 ID + 1」を毎回引き直さないと当たらない。
衝突を増やさずには済んだが、**これは毎回手で気づいた結果であって仕組みではない。**
繰り上げるたびに、型の表の ID 一覧と他3ファイルからの参照（合計6箇所）も追随させる必要がある。
1回でも見落とせば、参照が別のエントリを指す。

**実際、この3回目で一度壊した。** 参照を一括置換したとき、上のこの段落自身に書いてあった
「M-072 へ」という**経過の記述まで置換され**、「M-071 も取られて M-074 へ」という
存在しない履歴になった（気づいて書き直した）。**一括置換は「現在の参照」と
「過去の経過」を区別しない。**
