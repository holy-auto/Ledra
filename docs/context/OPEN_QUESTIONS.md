...
- 状況: 「履歴には適用済みと記録されているのに DDL が本番に反映されていない」ドリフトが2回発生した
  ...
  今回 `--include-all` を入れて「履歴に無いものが飛ばされる」経路は塞いだが、「履歴にあるのに中身が無い」型は
  依然として検知できない。発覚が常に本番障害後になっている。
- 選択肢:
  - 案A: 今回手作業でやった突合（`supabase/migrations` を機械パースして期待テーブル/列を抽出し、
    ...
    短所: SQLパースが素朴（正規表現）なので取りこぼし・誤検知があり得る。本番DBへの認証情報をCIに置く必要がある
    （db-migrate.yml の `SUPABASE_DB_URL` を再利用できる想定）。
  - 案B: `supabase db diff --linked` の差分をCIで見る。長所: 公式ツールで網羅的。
...
- 次のアクション: まず案Aを小さく作る（今回の突合スクリプトを `scripts/` に置き、手動実行できる形にする）。
  日次ワークフロー化と通知先は、誤検知の量を見てから決める。
- 起票日: 2026-08-15

## 「記録済みなのにDDL未反映」がどの操作で生まれたのかの特定（2026-08-15）
- 状況: 該当マイグレーションは `supabase_migrations.schema_migrations` に version と name だけがあり
  `statements` が NULL。`supabase migration repair --status applied` 相当の履歴書き込みが疑わしいが、
  実行ログが残っておらず断定できない【要確認】。ただし `statements` が NULL の行は全406件中279件あり、
  古いCLIでも NULL になり得るためこれ単体はドリフトの証拠にならない。
- 選択肢: (a) 原因特定を諦め、案A（定期突合）で検知に振り切る。(b) db-migrate ワークフローの実行ログを
  遡って push が途中失敗した回を探す。(c) 今後 repair 系コマンドを人が手で叩くことを運用上禁止し、
  適用経路をワークフロー1本に限定する。
- 影響範囲: 原因が塞がらない限り、同じドリフトが再発しうる。
- 次のアクション: 代表またはClaudeが GitHub Actions の db-migrate 実行履歴（2026-07-10前後・07-16前後）を確認する。
- 起票日: 2026-08-15
