...
## 課金・Stripe 系 API が選択中テナントではなく「最初の所属テナント」で動く（2026-09-23）

管理画面 18 ファイルは選択中テナント（`active_tenant_id`）で引くよう直した（`resolveCallerWithRole` に統一）。
同じ「`tenant_memberships` を `limit(1)` で引く」書き方が、課金まわりの API に5本残っている:
`/api/admin/billing-state`・`/api/admin/billing-status`・`/api/stripe/checkout`・`/api/stripe/portal`・`/api/stripe/resume`。
複数テナントに所属するユーザーが別テナントを選んでいる状態でプラン購入・請求ポータルを開くと、
**最初の所属テナントの契約が操作される**おそれがある（推定。本番で複数所属ユーザーが何人いるかは【要確認】）。
お金の外向き操作なので、直す前に確認したい:

- 課金操作は owner 限定などのロール条件を同時に入れるか（今は所属していれば誰でも通るかも未確認）
- 直した後、既存の Stripe 顧客とテナントの対応がずれていないか本番で確かめる手順
