...
## LINE 連携を「ログインだけ」にするためにモジュールチャネルを申請するか（2026-08-16）
- 状況: 外部連携6件のうち5件（Square / freee / マネーフォワード / Google カレンダー / Stripe）は
  既にワンクリックの OAuth で、2026-08-16 に Slack も OAuth 化した。**残るのは LINE だけ**。
  LINE は加盟店が Developers Console でチャネルを作り、Channel ID / Channel Secret /
  長期アクセストークンを発行して貼り、さらに Webhook URL を Console に貼り戻す7手順が必要で、
  導入のたびにこちらのサポートが張り付いている。
  調査の結果、LINE の「モジュールチャネル」を使えば、LINE公式アカウント管理者が
  `https://manager.line.biz/module/auth/v1/authorize` で認可するだけで連携でき、
  加盟店の発行作業はゼロになる（確認済み。詳細と出典は `docs/line-module-channel-research.md`）。
  ただしこれは**申請を完了した法人ユーザーのみが使える限定機能**で、申請の可否・費用・期間・
  審査基準はいずれも【要確認】。
- 選択肢:
  - 案A: LINEヤフーの担当者 / LINEマーケットプレイスの問い合わせ窓口から申請する。
    長所: 通れば LINE の導入コストが7手順→1手順になり、サポート工数が消える。実装側は
    汎用 OAuth 基盤に `providers/line.ts` を1ファイル足すだけで載る。
    短所: 審査・費用・期間が読めない。通らなければ調査工数が空振りになる。
  - 案B: 申請せず、導入代行（こちらが Developers Console 作業を代行する）を制度化する。
    長所: 今日から確実に加盟店の負担はゼロにできる。 短所: 加盟店が増えるほど人的コストが線形に増える。
  - 案C: 現状維持（手順書とサポートで凌ぐ）。 短所: 導入のボトルネックが残り続ける。
- 影響範囲: LINE は予約確認・リマインダー・書類送付の主要チャネル。連携の難しさが
  そのまま導入完了率と初期サポート工数に効く。判断が遅れると、加盟店が増えるほど代行工数が積み上がる。
- 次のアクション: LINEヤフーへ照会し、`docs/line-module-channel-research.md` の
  「まだ分かっていないこと」1〜5（審査基準 / 費用 / 期間 / 既存テナントの移行手順 /
  使えるスコープ範囲）に回答を得る。照会前に、egress 制限で直接読めていない
  `developers.line.biz` の一次ドキュメントを目視確認すること。
- 起票日: 2026-08-16
