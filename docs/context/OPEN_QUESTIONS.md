...
## 職人が束ねた実績への到達手段が、最後の1本の失効で消える（2026-09-03）

- 前提: 「複数元請けで実績が分散する」は 2026-09-03 に解決した。本人が両方のトークンを
  持っているときだけ束ねられる方式（DECISION_LOG 2026-09-03、`20260903000001`）。
  制約「他社に自分の稼働先が見えないように」は構造で守っている（束ねの情報はテナントから
  読めないテーブルにあり、`staff_portfolio_links` には何も足していない）。
- 残っている問題: **束ねた状態は、開いたリンクが有効であることに依存する。** 各店舗は
  自分のリンクをいつでも失効できるので、本人が持つリンクが全部失効すると、束ねた実績
  そのものに到達できなくなる（データは残るが入口が無い）。
- 選択肢: (1) 本人が設定できる復旧用の合言葉/連絡先を1つ持たせる（本人の持ち物が増える）。
  (2) 束ねを作った時点で「本人用の恒久トークン」を別途発行する（店舗の失効から独立するが、
  店舗が止めたはずの実績が見え続ける形になり、失効の意味が薄れる）。(3) 現状維持。
- 影響範囲: 全部の元請けとの関係が切れた職人は、自分の通算実績を見られなくなる。
  代表の働き方（案件ごとに元請けが変わる）だと現実に起きうる。
- 次のアクション: 代表判断。(2) は「店舗が失効させた実績を本人が見続けてよいか」という
  別の論点を開くので、そこの是非から。
