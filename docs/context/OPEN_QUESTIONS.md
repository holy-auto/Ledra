...
## 保険PoCの先行ユースケースと進め方（事故前状態証明が査定現場で刺さるか・A/B/C）
- 状況: 12店舗はコーティング・PPF中心で保険修理を扱う板金店がゼロ（堀越確認済み）。目玉の「修理案件の確認工数削減PoC」は対照実験の題材が存在せず現時点で実行不可。代替として、いまのコーティング・PPFデータで今日から成立する「事故前状態の証明」（施工前全周写真＝時刻固定・改ざん不可で、既存損傷/事故損傷の切り分けに効く）を先行ユースケースに据える案を検討中。ただし「査定現場で本当に揉めていて刺さるか」は推論であり、堀越の肌感で裏取りが必要。詳細は `docs/insurance-poc-strategy-2026-07.md`。
- 選択肢: 案A 事故前状態証明で先に価値を出す（すぐ着手可・訴求は工数削減より小さい）／案B 自力で板金店3社を入れてから保険へ（3〜6ヶ月・展示会機会を逃すリスク）／案C 保険会社に提携板金工場の紹介を頼む（6〜12ヶ月・実績ゼロでは順序が逆）。推奨はA＋B併走。
- 影響範囲: 誤ると、検証できないPoC（工数削減）を主役に据えたまま保険会社・投資家に当たり、「母数がないのにPoCできるのか」に答えられない。事故前状態証明が現場感と食い違えばリーフレット再構成が無駄になる。
- 次のアクション: 堀越が「事故前状態の証明が査定現場で刺さるか」を現場感覚で判断。刺さるならA着手＋リーフレットを時間軸2段に組み直し。並行してBの板金店3社獲得の可否を確認。
- 起票日: 2026-07-24

## 12店舗の定量エンゲージメント指標をDBから抽出（VC面談前の宿題）
- 状況: ANOBAKA（VC）が2回、別文脈で「チャーン率0%を支える定量エンゲージメント指標の可視化」を求めた。月間アクティブ店舗（12/12なら休眠ゼロ）、1店舗あたり月間ログイン/操作回数、予約・作業記録・請求の各累計件数、機能別利用率を面談前にDBから引くと「解約0%」が感覚からデータに変わる。「37件÷12店舗」問題への回答材料でもある。運営向けに `/admin/platform/store-usage` が既にあり流用できる（ログイン回数は未記録のため last_sign_in_at ベースの近似）。詳細は `docs/internal/investor-qa-anobaka-feedback-2026-07.md`。
- 選択肢: 案A 既存の `/admin/platform/store-usage` の数字をそのまま面談資料に転記／案B バトルカード（A4 1枚）用に指標を再集計。
- 影響範囲: 面談までに数字が揃わないと、最後のピースが欠けたまま（デックは8.5ラインで他は準備済み）。
- 次のアクション: 面談前にDBから12店舗の稼働データを引く（約1時間の作業）。数字が出たらバトルカードとデック（P17）の両方へ反映。
- 起票日: 2026-07-24
