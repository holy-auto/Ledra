...
- **Vercel が止まった原因が特定できていない。** デプロイ記録が1件も作られない
...
- **`VERCEL_TOKEN` / `VERCEL_ORG_ID` / `VERCEL_PROJECT_ID` が未登録。** 追加した
  `vercel-deploy.yml` は3本が揃うまで緑のままスキップする。**登録するまで
  デプロイは起きない。** → 代表対応
- **Vercel の Git 連携が復活すると二重にデプロイされる。** 復活させるなら
  Vercel 側の自動デプロイを切るか、`vercel-deploy.yml` を `workflow_dispatch`
  のみに変える。どちらを本線にするか。 → 代表判断
