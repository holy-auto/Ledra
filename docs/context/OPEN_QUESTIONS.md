...
## 走行距離の必須化: デプロイ前にオフラインキューへ滞留している証明書がないか（2026-08-25）
- 状況: `certCreateJsonSchema` の `mileage_km` を必須にしたため、**デプロイ前にオフラインキューへ積まれた
  証明書作成アイテムは 400 で弾かれる**。`src/lib/outbox/queue.ts` は 2xx と 409 のときだけアイテムを削除し、
  それ以外は `attempts++` して**削除せず永久にリトライする**（上限もデッドレターも無い、`queue.ts:392` にその旨の記載あり）。
  結果、そのアイテムは一生同期されず、後続の写真アップロードや発行アイテムも道連れになる。
  値を自動補完することはできない（走行距離は捏造できない）ので、コード側だけでは解決しない。
- 選択肢:
  - 案A: デプロイ前に滞留アイテムがゼロであることを確認してから出す。**本番は22テナント・証明書45件と小さく、
    オフライン下書きの滞留は実質ゼロと見込まれる**が、キューはブラウザの IndexedDB にあるためサーバーからは確認できない。
    加盟店に「オンラインで一度アプリを開いて同期してから」を案内するのが現実的。
  - 案B: `mileage_km` を API スキーマ上は optional に戻し、必須はフォーム側だけにする。
    互換性は保てるが「フォームだけ必須・APIは素通り」の抜け道が戻る。
  - 案C: outbox に上限リトライ／デッドレターを実装し、恒久的に失敗するアイテムを利用者に見せる。
    永久リトライは今回の変更とは無関係に存在するバグなので、直すなら根本的。ただし 401（再ログインで回復）などを
    誤って捨てないよう、リトライ可否の分類が要る。
- 影響範囲: 該当アイテムがあると、その証明書は永久に発行されない。しかも静かに失敗する。
- 次のアクション: デプロイ前に案A（同期の案内）を実施する。案C は今回の範囲外だが、
  「永久リトライするキュー」は独立したバグとして別途起票する価値がある。
- 起票日: 2026-08-25

## モバイル: 車両マスタを選ばずに発行すると走行距離が履歴に積まれない（2026-08-25）
- 状況: モバイルの証明書作成 (`apps/mobile/src/app/certificates/new.tsx`) は Supabase へ直 insert していて、
  WEB の `createCertAction` と違い**車両マスタの自動作成をしない**。一方 `validate()` は
  「車両を選択するか、メーカー・車種を入力」のどちらでも通す。
  メーカー・車種の手入力だけで発行すると `vehicle_id` が null になり、
  DBトリガー `fn_sync_mileage_from_certificate` が早期 return するため、
  **必須にしたはずの走行距離が `vehicle_mileage_logs` に入らない**（エラーは出ない）。
  コードレビューで発見。コメントは実態に合わせて訂正済みだが、挙動は未修正。
- 選択肢:
  - 案A: モバイルでも車両マスタを自動作成する（WEB の `createCertAction` と同じ挙動に揃える）。筋は良いが実装量がある。
  - 案B: モバイルでは車両マスタの選択を必須にする。実装は最小だが、手入力での発行ができなくなる。
  - 案C: 車両未選択のときだけ走行距離の入力欄を隠し、「車両を選ぶと走行距離を記録できます」と案内する。
- 影響範囲: モバイル発行の割合次第。手入力発行が多いと、必須化してもモバイル分の走行距離が溜まらない。
- 次のアクション: モバイルの次回ビルド前に案A〜Cを決める。あわせて、この環境では
  `apps/mobile` の依存が未インストールで型検査も実機確認もできていない点を解消する。
- 起票日: 2026-08-25

## 走行距離の必須化で塞ぎ切れなかった経路と、既存証明書の遡及入力（2026-08-25）
- 状況: 証明書の新規作成で走行距離を必須化した（DECISION_LOG / RELEASE_LOG 2026-08-25）が、3つ残っている。
  1. **証明書の編集 `/api/certificates/edit` は対象外。** 編集経由なら走行距離が空のまま保存できる。
     新規作成を塞いだので新しい穴は開かないが、「編集で消す」ことは今も可能。
  2. **既存45件には走行距離が無く、遡及入力の手段が無い。** 車両パスポートの走行距離履歴は
     必須化以降に作られた証明書からしか積み上がらない。
  3. **モバイルの変更が未検証。** `apps/mobile` の依存がこの環境に無く、型検査も実機確認もできていない。
     ロジックは WEB と同じだが、実際に画面が出て保存できるかは確認していない。
- 選択肢:
  - 1について: (a) 編集APIでも必須にする（既存45件の編集が全部ブロックされるので、既存行の扱いを決める必要がある）
    (b) 編集では「空→値あり」だけ許し「値あり→空」を禁じる (c) 現状のまま。
  - 2について: (a) 車検証OCR（`InspectionOcrIntake` にメーター撮影が既にある）で後から入れる導線を作る
    (b) 次回入庫時に自然に入るのを待つ (c) 手入力の一括画面を作る。
- 影響範囲: 2を放置しても新規データは積み上がるので致命的ではないが、
  「Ledra に履歴がある車」の母数が増えるスピードに直結する。3は次回モバイルリリース前に必ず潰す必要がある。
- 次のアクション: 3をモバイルの次回ビルド時に実機確認する。1と2は、必須化後1〜2週間の入力率を見てから判断する
  （そもそも現場が入力してくれるかを先に確認する）。
- 起票日: 2026-08-25
