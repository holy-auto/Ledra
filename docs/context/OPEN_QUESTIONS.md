...
- **未解決の前提そのもの**: `evaluatePaymentPolicy()`/`evaluateCertificateGate()` は本番のどこからも呼ばれておらず（呼び出し元ゼロを確認済み）、「合算払いのジョブの `paymentState` は何から導出するのか」という、この判断の前提となる設計自体がまだ存在しない。加えて「合算払いかどうか」の判定基準自体が3箇所で食い違っている（上記訂正2）ため、まずこの基準を統一するところから始める必要がある。
