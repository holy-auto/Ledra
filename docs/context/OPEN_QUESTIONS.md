...
## LINE連携直後のマイページ・ログイン摩擦（2026-08-15）
- 状況: LINE連携完了時にマイページURLを送るようにしたが、開いた先ではOTP（メールアドレス＋電話番号下4桁）が必要。顧客はLINEで本人確認を済ませた直後なのに、もう一度別の本人確認を求められる。
  さらに強い制約として、**email を持たない顧客はマイページに入れない**（`listPortalMemberships` が email 一致で顧客を引き、OTPもメール配信）。受信箱からスタッフが作った顧客（名前＋電話のみ）や、email を空で出した登録フォームの顧客が該当する。そのためURL自動送信は email がある顧客に限定して送っている（開けない導線を案内しない方針）。**email 無し顧客の割合は【要確認】**。多ければこの機能の到達範囲がそのまま狭い。
- 選択肢: (A) 現状維持＝認証経路を増やさず安全側。摩擦は残る。(B) 連携時に短命のワンタイムトークン付きURLを発行しポータルセッションを自動発行＝ワンタップで開けるが、OTP以外のログイン経路が増えるのでURL転送・LINEアカウント乗っ取り時の影響範囲を設計する必要がある。
- 影響範囲: 誤ると顧客ポータルの認証が弱くなる（他人の施工履歴・車両情報が見える事故）。放置するとマイページの到達率が上がらない。
- 次のアクション: 代表がAかBを判断。Bを採る場合はトークンのTTL・単回使用・失効条件を先に決めてからDECISION_LOGへ。
- 起票日: 2026-08-15
