...
- 論点と選択肢:
  1. **Apple 本番(Distribution) entitlement の付与状況【要確認】**: 未付与だと production ビルドが entitlement 署名で失敗する。→ Developer Portal で `com.ledra.app` の Provisioning Support を確認。未付与なら `withRemoveTapToPayEntitlement` を戻してTTP無しで先行公開→承認後に外す（submission-guide の Go/No-Go 参照）。
...
- 次のアクション: 代表に #851 の送金機能の完成見込みと、#760 を今レビューするか分割するかを確認。
- 起票日: 2026-08-05
