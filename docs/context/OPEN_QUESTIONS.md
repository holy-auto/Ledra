...
## Web の帳票一覧で種別フィルタが効いていない（2026-08-23）
- 状況: `src/app/admin/documents/page.tsx` は `searchParams.type` を読むが、AI 見積ドラフトの
  通知 (`src/lib/ai/automation/quoteDraftCore.ts`) は `link_path` に
  `/admin/documents?doc_type=estimate` を書いている。**キーが違うため、その通知から
  Web を開いても見積書に絞り込まれない**（全帳票の一覧が出る）。
  モバイル側は両方のキーを拾うようにしたので影響を受けない。
- 選択肢: 通知側を `?type=estimate` に直す / Web 側で `doc_type` も読む / 両方
- 影響範囲: 帳票が増えるほど、通知から目的の見積を探す手間が増える。壊れはしない。
- 次のアクション: どちらを正とするか決めて1行直す
- 起票日: 2026-08-23

## モバイルのメッセージ機能で管理画面に残した操作（2026-08-23）
- 状況: モバイルのメッセージはテキストの返信と既読化までにした。管理画面にはあるが
  モバイルに載せていないのは、画像の送信・AI 返信ドラフト・未紐付け LINE スレッドの
  顧客紐付け・予約候補の確認。サーバ側は画像送信も含めて対応済みで、必要なのは UI。
- 選択肢: 現状維持 / 画像送信だけ足す（現場写真を送りたい場面はありそう）/ 全部載せる
- 影響範囲: 出先で写真を送りたいときに管理画面へ戻る必要がある。
