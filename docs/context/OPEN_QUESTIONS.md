...
- **Stripe Terminal のリーダーを導入するか。** 代表より「別端末なら手入力は可能」との
  回答（2026-08-25）。導入すれば、店の端末で決済ページを開く方式より現場の手順は短くなる。
  一方、現行のカード番号決済（Checkout）は端末の追加費用ゼロで動く。
  導入台数・費用・どの店舗から入れるかは未定。→ 代表判断
  - 【要確認】リーダー上の手入力が日本の Stripe アカウントで有効化できるか。
    ドキュメントからは日本での可否を確認できていない（未検証）。導入を決める前に
    Stripe に直接確認すること。
