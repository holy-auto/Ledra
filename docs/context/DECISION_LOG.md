...
4. 違和感・問題: (a) **部品の修正を機能の修正と書いていた**（MISTAKE_LEDGER M-061）。直した関数が値を返すことは確かめたが、利用者が触る入口を一度も通していない。(b) `agent_rankings` は**失敗が見えない形**で壊れていた —— 呼び出し元 `/api/agent/rankings` が `const { data } = await supabase.rpc(...)` と書いて `error` を捨てているため、**200 で空のランキング**が返る。画面には「該当なし」と出る。落ちるより悪い。(c) 「plpgsql は見られない」は**調べずに諦めていた**。実際には処理系の外に道具があった。
