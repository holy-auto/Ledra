...
1. 日付: 2026-08-29
2. 起きたこと: #936（IMP-021）を main へ取り込んだ直後の `/code-review` で、`src/lib/sync/` が**再び**復活していた。前日 #935 で全く同じ理由（削除より前の commit から分岐したブランチのマージが衝突を出さない）で一度直したばかりの穴に、今回も同じ手順で気づかず踏んだ。
3. 以前の考え: 「`git log --diff-filter=D --name-only main` で main の削除履歴を洗い出し、マージ後の作業ツリーと突き合わせる」を機械的な検出方法の候補として OPEN_QUESTIONS に残していたが、実装も検証もしていなかった。#936 のマージ直後、まず前回と同じ「merge-base から見た main の純粋な削除ファイル一覧」で確認したが、これは**外れていた** —— `src/lib/sync/` は merge-base（IMP-000 開始よりずっと前）の時点でそもそも存在せず、「追加されてから削除される」が丸ごと main の履歴内で完結していたため、merge-base 基準の削除一覧には現れない。
4. 違和感・問題: 「min-base から見た削除ファイル一覧」という発想そのものが、追加と削除が両方とも比較区間の中で起きたケースを拾えない構造的な穴だった。正しく拾うには「今の作業ツリーにあるファイル」と「main の現在の tip にあるファイル」を直接比較し、差分（作業ツリーにだけある物）それぞれについて「main の履歴上で一度でも削除されたことがあるか」を確認する必要がある。
5. 決めたこと: (a) `comm -23 <(git ls-files | sort) <(git ls-tree -r --name-only origin/main | sort)` で「作業ツリーにあって main の tip に無いファイル」を洗い出す。(b) その各ファイルに対し `git log --oneline --diff-filter=D -- <file> origin/main` を実行し、1件でもヒットしたら復活の疑いありとして中身を確認する。(c) この手順を今後すべてのスタック PR マージで、コンフリクト解決の直後・コミット前に必ず実行する。(d) #936 では実際にこの手順で `src/lib/sync/` の復活を検出し、削除して解決した。
6. 捨てた選択肢: merge-base 基準の削除一覧だけで足りるとみなす — #936 で実際に外れることを確認済みなので却下。
7. 判断理由: 一度目（#935）で立てた仮説（merge-base 基準の削除検出）を、二度目（#936）で実際に動かして検証したところ機能しなかった。「未検証の対策を対策済みとみなさない」。動く手順に置き換えた。
8. まだ答えが出ていないこと: この2ステップ確認をスクリプト化して `npm run` から呼べるようにするか、毎回手動で打つかは未定。残り約18本のスタック PR がある間は手動でも実行できるが、スクリプト化すれば取りこぼしのリスクを減らせる。
