...
## 2026-09-03 職人の実績の名寄せは「本人がトークンで束ねる」方式にする

1. 日付: 2026-09-03
2. 起きたこと: 実績リンクを入れた直後、残った本丸（複数元請けで実績が分散する）について
   代表から制約が示された:「**他社に自分の稼働先が見えないように**」。
3. 以前の考え: 電話番号やメールのハッシュでテナントを跨いで同一人物を自動同定する。
4. 違和感・問題: 自動同定は**元請け側から観測できてしまう**。番号を入れて実績件数の
   変化を見れば「この職人は他所でも働いている」が分かる。示された制約に真っ向から反する。
   さらに、テナントが読める行に束ねの印（`identity_id` 等）を1つでも置くと、
   **非 NULL であること自体**が同じ漏洩になる。
5. 決めたこと:
   - **束ねるのは本人だけ。** 両方のリンクのトークンを持っている人しか実行できない
     （`/w/[token]` の画面で他店のリンクを貼る）。テナント側には操作も表示も無い。
   - **束ねの情報はテナントから読めないテーブルに置く。** `staff_portfolio_identities` /
     `staff_portfolio_identity_members` を RLS `USING (false)` のサービスロール専用にした
     （`customer_sessions` と同じ形）。`staff_portfolio_links` には一切足さない。
   - **閲覧記録は開いたリンクだけに付ける。** 束ねた他店の `last_viewed_at` を動かすと
     「本人が別経路で見た」がその店に伝わる。
   - 束ねは**外せる**。戻せない仕組みは本人にとって取り返しがつかない。
6. 捨てた選択肢:
   - **電話/メールのハッシュで自動名寄せ** → 上記のとおり元請け側から観測できる。
   - **職人に軽量アカウントを持たせて束ねる** → RBAC とプラン課金が増えるうえ、
     アカウントの存在自体をテナントが見るので、結局同じ観測経路が残る。
   - **束ねに相手テナントの同意を求める** → 同意を求める行為そのものが「他所でも
     働いている」の通知になる。実行できるのが本人だけなら同意確認は要らない。
7. 判断理由: 制約を運用ルールではなく**構造**で守れる形を選んだ。「見せない運用にする」は
   必ず破れるが、「テナントが読めるところに情報が無い」なら破りようがない。
8. まだ答えが出ていないこと:
   - 束ねた状態は、開いたリンクが失効すると辿れなくなる（各店舗が自分のリンクを止められる
     ことの裏返し）。本人が最後の1本を失うと束ね自体に到達できない。回復手段は未設計。
   - リンクを本人へ渡す経路（LINE / メール / 口頭）は依然として運用任せ。
9. 公開区分: 公開可（顧客名・取引先名を含まない。プライバシー設計の判断として発信価値がある）

## 2026-09-03 職人本人の導線は「テナント発行の読み取り専用リンク」にする

1. 日付: 2026-09-03
2. 起きたこと: 外注施工の記録を発注に紐付けたあと、代表から「続けて」の指示。
   OPEN_QUESTIONS に残していた「作業した職人本人が自分の施工履歴を確認できない」に着手した。
3. 以前の考え: 記録がテナントに残っていれば、本人はテナント経由で確認すればよい。
4. 違和感・問題: 外注職人はログインを持たない設計で、Web もモバイルも
   `tenant_memberships` 前提。「テナント経由で確認」の具体的な手段が存在しなかった。
   本人が見られるのは公開証明書 URL を自分で控えていた場合だけで、一覧する術が無い。
5. 決めたこと:
   - **テナントが職人ごとに読み取り専用リンクを1本発行する**（案1）。雛形は
     `customer_intake_links`。公開ページは `/w/[token]`。
   - **失効はロスターの在籍状態に相乗りさせる**。有効条件を token 一致 +
     `link.is_active` + `staff_members.is_active` の3つにして、離職時に別作業を
     覚えなくてよい形にした。「誰が配り、離職時にどう止めるか」という
     OPEN_QUESTIONS の宿題への回答でもある。
   - **raw token は保存しない**。`customer_intake_links` は QR 再表示のため暗号化して
     持つが、これは個人に渡すリンクなので、復号可能な控えを持つ利点より持たない安全側を取る。
   - **顧客 PII は出さない**。取得列は3列のみ。
6. 捨てた選択肢:
   - **職人に軽量アカウントを持たせる**（`staff_members.user_id` を外注でも埋める）→
     RBAC とプラン課金の扱いが増える。今回の目的（本人が見られる）に対して大きすぎる。
   - **顧客名も見せる** → 職人は施工時に顧客を知っているので一見自然だが、リンクは
     退職後も手元に残りうる。恒久的な顧客名簿を配る形にはしない。
   - **有効期限を付ける** → 期限切れの再発行という運用が増える。在籍状態に相乗りする
     ほうが、既にある離職の手続きだけで止まる。
7. 判断理由: 一番小さくて、かつ**運用が増えない**形を選んだ。失効を在籍状態に
   相乗りさせたのが要点で、これがないと「リンクを止め忘れた退職者」が必ず出る。
8. まだ答えが出ていないこと:
   - **テナント内の実績に限る。** 複数の元請けに登録された同じ職人の通算実績は
     依然として出せない（`staff_members` が元請けごとに別行で、名寄せの手段が無い）。
     代表自身の働き方に直結する問題だが、テナントを跨いで同一人物を同定する仕組みは
     プライバシーの検討が別途要る。
   - リンクを本人へ渡す経路（LINE / メール / 口頭）は運用任せのまま。
9. 公開区分: 公開可（顧客名・取引先名を含まない）
