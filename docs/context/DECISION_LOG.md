...
4. 違和感・問題: 500 が消えた後の検索画面が「該当なし・RESULTS 0」だった。件数を調べると、
   デモ保険会社が見られるテナントは**デモテナントではなく実テナント `HOLY AUTO`**（証明書1件）
   だった。つまり検索を実行させて「結果が並んだ絵」にすると、**実業務の証明書が配布 PDF に載る**。
   RPC は顧客名をマスクするが、車種・ナンバー・車台番号・画像はマスクされない。
5. 決めたこと: (a) DB 修正は適用する（挙動を変えず、壊れた参照を直すだけ）。
   (b) **検索は実行せず、空の初期状態のまま撮影する。** 実データを載せない方を優先した。
   (c) デモ保険会社にデモテナントを見せるかは本番のアクセス制御データを増やす話なので
   OPEN_QUESTIONS に起票して代表判断に回す。
6. 捨てた選択肢: (a) 検索を実行して結果が並んだ絵を撮る…見栄えは良いが実データが配布物に載る。
   (b) `insurer_tenant_access` にデモテナントの行を足してから撮る…絵は最善になるが、
   撮影のついでに本番のアクセス制御行を増やすことになる。許可の範囲は「関数の修正」であって
   「アクセス権の付与」ではないので採らなかった。
   (c) 3枚目を諦めて 13 ページのまま出す…DB 修正の許可が出た以上、諦める理由がない。
7. 判断理由: 配布 PDF は社外に渡る。スライドの見栄えと、実顧客・実業務データを外に出さないことの
   どちらを取るかは、比べるまでもない。空の検索画面でも「保険会社が自分のポータルから検索できる」
   という主張は成立する。また、許可された変更（壊れた関数を直す）と許可されていない変更
   （アクセス権を足す）は別物として扱った。
8. まだ答えが出ていないこと: 本番の `insurer_tenant_access` に「デモ保険会社 → 実テナント
   HOLY AUTO」の行が存在してよいのか【要確認】。保険会社スライドを実データ入りにするなら
   デモテナントへのアクセスを与えるかどうか。
