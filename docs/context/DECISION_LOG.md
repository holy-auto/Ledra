...
## 2026-09-21 8月の宿題2件は「実測で閉じる」—— 通知は 17 回中 9 回しか飛んでいなかった
...
5. 決めたこと: (a) **17 回すべてを機械判定する。** 各実行の `head_sha` 時点の `db-migrate.yml` を取り出し、コメント行を除いた実行コードに壊れた文言が含まれるかで分類した。結果は**配信 9 回 / 不配信 8 回**（不配信は 08-29〜09-06、`b82e2a0` で修復済み）。(b) **判定器は陽性・陰性の両方で当たりを取ってから使う。** 最初の判定器は修復コミットの**説明コメント**に同じ文字列が入っていたため #76 を誤って「壊れている」と判定した。既知の1件（#76 はログに Slack の `ok`）で外れることに気づき、コメント行を除いて直し、陰性側（#51 の通知ステップが `failure`）でも確かめた。(c) 「通知経路の確認方法」の起票は**解決として閉じる**。案B（確認用ワークフローの新設）は作らない —— 通知の故障はステップが赤くなる形で既に見えており、それが実際の発見経路だった。(d) 「他ワークフローの可視性」は**実測値に書き換えて開いたまま残す**。
