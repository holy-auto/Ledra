...
8. まだ答えが出ていないこと: (a) **6引数の `insurer_search_vehicles(text,integer,integer,text,text,text)` をどうするか**【要確認】。消えた `insurer_is_active_subscription` を呼び、その根拠表 `insurer_subscriptions` も無いので、呼ばれれば必ず落ちる。アプリからの呼び出しは0件。消すか、依存ごと戻すか。(b) **本番に main へ無い版が7つ残っている**（`20260917000000`〜`20260917000400`, `20260917100000`, `20260918142610`）。`db-migrate` は止まったまま。(c) **今回のように手で当てた版が今後も増えると、repo と本番の一致を守る仕組みが無い**。`migrations.production-ledger` は誰が更新するか未決。(d) `audit_logs` の形の差（本番12列 / マイグレーション20列）は未調査のまま。
