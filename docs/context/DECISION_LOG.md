...
4. 違和感・問題: **これは新しい判断ではなく、2026-08-23 に予告した後始末だった。** `20260823000000_audit_logs_reconcile.sql` のヘッダに「列そのものは次のマイグレーションで落とす（データがある環境で先に落とすと戻せない）」と書いてあり、その「次」が1か月来ていなかった。保留の理由に挙げた「別の判断」は、すでに一度下されていた判断の実行待ちだった。**自分が根拠として読んだファイルの中に、答えが書いてあった**（同じ形を前日にも踏んでいる —— `M-20260920-hashed-a-file-i-never-opened`）。
