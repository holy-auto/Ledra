...
5. 決めたこと: (a) **1つのマイグレーションで、3本の書き起こしを先に、ゲートを後に置く。** 順序が意味を持つので分割しない。(b) **ポリシーは1本も触らず、`my_insurer_ids()` だけを直す。** 14本すべてがこの関数を通り、他の呼び出し元が 0 本であることを本番の `pg_proc` で実測した。(c) **`current_insurer_access()` は再利用しない。** あちらは `LIMIT 1` で1件だけ選ぶ設計で、RLS は所属する保険会社すべてが要る（複数所属が出た瞬間に片方の自社データが消える）。同じ条件を集合で返すのが `my_insurer_ids()` の役目。(d) `search_path` を `''` に直す（従来は `'public','extensions','pg_temp'` で lint の `security-definer-mutable-search-path` に反する形だった）。(e) **停止中に見えなくなるものを受け入れる** —— 同僚のスタッフ一覧・閲覧許可テナント一覧・AI 利用ログ・案件・メッセージ・添付・PII 開示同意。残るのは自社の1行と自分のメンバーシップ行、および `get_my_insurer_status()`（SECURITY DEFINER なので RLS 非依存）。
