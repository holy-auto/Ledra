...
## 2026-09-04 レート制限を免除した2ジョブの上限は妥当。ただし守っているのは費用ではなく実行時間だった

1. 日付: 2026-09-04
2. 起きたこと: OPEN_QUESTIONS「免除した2つのジョブが持つ件数上限が妥当か」を検証した。
   結論は**どちらも妥当**。ただし**上限が何を守っているかの理解が間違っていた**。

   | ジョブ | 費用を止めているもの | 件数上限が守っているもの |
   |---|---|---|
   | `cron/daily-digest` | テナント別の月次コストキャップ | （件数上限は無い。要らない） |
   | `qstash/line-history-import` | ループ内の月次コストキャップ判定 | `maxDuration = 300` 秒の実行時間 |
   ...
   - `daily-digest` は `vercel.json` で `0 22 * * *` = 1日1回。`withCronLock(600秒)` で同時実行なし。
     ...
     全部通ったテナントだけが AI を消費する。`settings.enabled` は
     `loadAiAutomationSettings` 内で月次コストキャップ超過なら false に倒れる
     （`policy.ts:183`、`applyCostCap` 既定 true）。
   - `line-history-import` は `Math.min(n, 500)` でハードクランプ。ループの各周で
     `baseSpentJpy + inJobJpy >= capJpy` を同期的に見て break する。
     enqueue 元は `linkLineUserToCustomer`（顧客に LINE を紐づけた瞬間）のみ。
