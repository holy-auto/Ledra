...
## 2026-09-03 AI ルートのレート制限漏れ6本を塞ぎ、報酬適用の AAL2 は見送った

1. 日付: 2026-09-03
2. 起きたこと: #1021 でアカデミーの AI 3経路に staff 以上の認可を入れたあと、
   「認可を入れただけで費用は止まっていない」ことに気づいた。認証済みの1人が
   ボタンを押し続ければ AI の課金は無制限に伸びる。調べたところ、モデル選択を
   import している API ルート29本のうち **6本にレート制限が無かった**。
   残り23本は同じ `checkRateLimit(req, "ai", ...)` を使っていた。
3. 以前の考え: 「AI は staff 以上」（2026-09-01 の代表判断）で費用の問題は片付いたと
   思っていた。**認可は「誰が呼べるか」であって「何回呼べるか」ではない。**
4. 違和感・問題: 漏れていた6本は
   `admin/academy/{cases,feedback,qa}`、`admin/certificates/{ai-draft,ai-explain}`、
   `admin/purchase-orders/ai-message`。いずれも慣行から漏れただけで、
   新しい preset もヘルパーも要らなかった（`"ai"` プリセットは既存）。

   **洗い出しの道具でまた誤りかけた。** 最初 import の推移到達で洗ったところ47本挙がったが、
   `isMissingTableError`（エラー判定）や `calcSizeClass`（純粋関数）まで拾っていた。
   「`@/lib/ai/client` に到達できる」は「モデルを呼ぶ」ではない。
   MISTAKE_LEDGER の型 A そのものなので、**47という数字は出さず**、
   ルート自身がモデル選択を import しているかという狭い signal に切り替えた。
5. 決めたこと:
   - **6本すべてに `checkRateLimit(req, "ai", ...)` を入れる**（代表判断）。
     既存23本と同じ書き方をコピーする。キーはテナント単位。
   - **構造テストで固定する。** `src/lib/api/__tests__/aiRouteRateLimit.test.ts`。
     (a) 検出器が空振りしていないこと、(b) モデル選択を import するルートは
     レート制限を課していること、(c) 免除リストに不要なものが残っていないこと、の3本。
     **ガードを1本消すと落ちることを実際に確認した**（`admin/academy/qa` で検証）。
   - **免除は1本のみ。** `qstash/line-history-import` は QStash 署名付きの非同期ジョブで
     auth セッションが無く、ユーザーが直接叩けない。1回の実行件数に上限
     （`LINE_HISTORY_IMPORT_MAX`、既定80）を自前で持ち、月次コストキャップも尊重する。
   ...
   - **報酬適用（`admin/academy/rewards/[id]/apply`、Stripe の credit を動かす）の
     ...
     追加の再認証は運用の負担に見合わない。現状のインライン `super_admin` 判定を維持する。
6. 捨てた選択肢:
   - **academy の3本だけ直す** → 元の指摘範囲だが、同じ穴が certificates と
     purchase-orders にもあった。指摘された経路だけ塞ぐのは根治ではない。
   - **推移到達の47本すべてに一括で入れる** → 数字自体が信用できない。
     純粋関数を呼ぶだけのルートに 429 を持ち込むのは害。読んでから入れる。
...
7. 判断理由: 認可と費用上限は別の軸で、どちらか一方では守れない。既に23本で確立していた
   慣行に6本を揃えるだけなので、判断の余地はほとんど無かった。
   洗い出しの範囲は「読んで確認できたものだけ」に絞った。**道具の出力を事実として
   扱わない**（型 A の再発防止）。
8. まだ答えが出ていないこと:
   - モデル選択を自分では import せず、下位モジュール経由で AI を呼ぶ経路が残っている
     （cron 7本ほか）。1本ずつ読む必要がある。OPEN_QUESTIONS に起票済み。
   - 免除した QStash ワーカーの件数上限（既定80）が妥当かは未検証。
9. 公開区分: 公開可（「認可を入れても費用は止まらない」は一般論として書ける。
   具体的なルート名・プリセットの窓幅は書かない）
