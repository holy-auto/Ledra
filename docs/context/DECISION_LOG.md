...
2. 起きたこと: 同日の整合（`20260920120500`）で保留にしていた `audit_logs` の8列（`table_name` / `record_id` / `old_values` / `new_values` / `reason` / `performed_by` / `device_id` / `ip_address`）について、代表が「**8列は落として**」と判断した。`20260920140000` で落とした。
