...
6. 捨てた選択肢:
   ...
   - **エントリに「差分ゼロを確認」と追記して残す。** 却下。解決済みは消す（M-029 の教訓）。
...
4. 違和感・問題: **今日これで5件目**（M-025 / M-026 / M-027 / M-029 / これ）。
...
6. 捨てた選択肢:
   - **「修正済み」と追記して残す。** 却下。未解決事項ファイルに解決済みが残ると、
     次のセッションが着手対象として拾う（M-029 で実際に起きた）。
...
8. まだ答えが出ていないこと:
   ...
   - **他にも同じ形で古くなっているエントリが無いかは未確認。** 今日だけで
     ...
     （M-025・M-026・M-027・これ）。1件ずつ着手するたびに前提を実測しているので
     順次見つかるが、一括で洗う手段は無い。
