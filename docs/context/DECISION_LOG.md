...
8. まだ答えが出ていないこと: (a) ラッパの `rateLimit` は IP 単位のまま。AI を呼ぶ13本は個別に直したが、**他の withCaller ルートの `rateLimit` も同じ性質**で、店舗 NAT の影響は未測定【要確認】。(b) `CALLER_WRAPPERS` は `withCaller` の1つだけ。別のラッパを足したらここに登録する必要があり、登録漏れは再び盲点になる（検査で止められていない）。(c) 13本の復元は実 API で叩いて確認していない（レート制限の発火は手元で再現していない）。検出器と型検査は通っている。
