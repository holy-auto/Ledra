...
## 2026-09-05 決済・帳票送付の権限を決定、あわせて「未強制24本」の数え間違いを訂正

1. 日付: 2026-09-05
2. 起きたこと: 認可未強制として残していた24ハンドラの方針を決めるため、1本ずつ中身を
   読んだ。**そのうち18本は既に別の形で守られていた。**「24本」は私の数え間違いだった。
...
4. 違和感・問題: 認可は任意のヘルパーで書ける。正規表現で網羅はできない。実際に
   ...
   - 事例の所有者チェック1本、クイズ編集・動画アップロードの著者判定2本
   - 受講5本（完了・評価・クイズ挑戦）は自分の行にしか書けず、自分のレッスンは操作不可
...
5. 決めたこと（代表判断）:
   ...
   - **顧客への決済リンク発行は `payments:create`（staff 以上）。** 現場が作業後に請求を
     出すのは通常業務。
   - **備品購入は `billing:manage`（admin 以上）。** 会社のお金を使う操作なので、顧客への
     請求とは分ける。
   ...
   - **検出器の限界を明示する。** 既知リストの意味を「認可が無い」から
     **「この検出器が認可を認識できない」**に改め、28件すべてに分類コメントを付けた
     （自己完結16・認証前2・読み取りのみ1・Server Action 委譲1・受講5・所有者判定3）。
     インラインのロール判定と2つのヘルパーは認識できるようにした（46→28）。
6. 捨てた選択肢:
   - **決済系をすべて admin 以上にする** → 現場が請求を出せなくなる。作業後にその場で
     決済リンクを出すのは通常業務なので、リンク発行だけは staff に開く。
   - **Stripe 連携を `billing:manage` に揃える** → admin も通ってしまう。入金口座の解除は
     不可逆なので owner のみにした。本番は owner 23名なので運用に支障はない。
