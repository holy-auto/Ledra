...
## 2026-08-30 IMP-026（#941）を main へ取り込み。check:schema が実在しない列名を検出、resurrection バグを6度目の再削除

1. 日付: 2026-08-30
2. 起きたこと: IMP-026（#941, branch impl/IMP-026-customer-concern）を main へ取り込む際、(a) IMP-024/025 と同じ根本原因で `src/lib/sync/`・`WorkScopeProvider.tsx` が6度目の復活、(b) マージ後の検証スイート実行で `npm run check:schema` が実際に失敗——本PR自身の新規コード `src/app/api/customer/concerns/route.ts` が `part_confirmation_signatures` テーブルの `part_installation_id` という**存在しない列**を SELECT していた（実列名は `installation_id`）。加えて本PR自身が新設した `customer_concerns` テーブルが `scripts/schema.snapshot.json` に未登録で、テーブル自体のクエリも「テーブルが存在しない」として全件検出されていた。
3. 以前の考え: PR 本文の「検証」節は「全テスト 4193 件パス、TypeScript・lint クリーン」とのみ記載しており、`check:schema` の実行結果には触れていなかった。マージ時の標準検証手順（tsc/vitest/lint/check:schema/lint:migrations）を機械的に流すまで、この列名ミスは気づかれていなかった。
4. 違和感・問題: `part_installation_id` は本PRのどのファイルにも定義されていない列名で、`part_confirmation_signatures` migration（既存）にも `part_installations` テーブル（既存）にも存在しない。命名の類推（「`part_confirmation_signatures` だから `part_ + installation_id`」）による typo と推定。本番へこのままデプロイされていた場合、`parts_confirmation` 経由の「気になる点を伝える」submission は毎回 PostgREST の 400 で失敗し、ジョブ紐付けができないまま懸念情報だけが記録される（またはエラーで submission 自体が失敗する）状態になっていた。
5. 決めたこと: (a) `part_installation_id` → `installation_id` に修正（2箇所: SELECT 句と参照）。(b) `customer_concerns` テーブルを、本PR自身のマイグレーション DDL から列名を書き起こして `schema.snapshot.json` に追加（本番にまだ適用されていないマイグレーションだが、schema.snapshot.json の目的は「マージ後に成立するはずのスキーマ」との突き合わせなので、DDL から直接転記して問題ない）。(c) resurrection ファイル5件を再度削除（IMP-024/025 と同じ手順）。
6. 捨てた選択肢: なし——`check:schema` が落ちた時点で単純な列名修正であり、他の選択肢を検討する余地がない種類の修正。
7. 判断理由: `check:schema` はまさにこの種のバグ（存在しない列名の参照が実機で「登録がまだなのだろう」という誤読を招く）を防ぐために作られたツール（`scripts/schema.snapshot.README.md` 参照）。検証手順を省略せず機械的に実行したことで、マージ前に検出できた。
