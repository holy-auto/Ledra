...
8. まだ答えが出ていないこと: (a) `ft_*` が何のために作られたか。Postgres に作成時刻が無いので**作った人に聞く以外に辿る方法がない**【要確認】。(b) `audit_logs` が本番 12 列 / マイグレーション 20 列で**形そのものが違う**。どちらが正か、この表に書くコードが本番で落ちていないかは未調査【要確認】。(c) 残る本番だけの 14 列を書き起こすか。(d) `certificate_no` は列を足すのが正か、2関数から外すのが正か【要確認】。(e) **シークレット（`SUPABASE_ACCESS_TOKEN` / `SUPABASE_PROJECT_ID`）が未登録なので、このマージ後は週次ジョブが赤くなる。** 登録は代表の操作が要る。
