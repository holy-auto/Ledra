...
## 11. 既存データの取り扱い（レガシー証明書・車両記録）

本機能（`part_installations` テーブル）は **新規施工から順次適用** する設計であり、Phase 1 導入以前に発行済みの証明書・車両記録に対して自動バックフィルは行わない。

**理由:**
- 既存の `certificates` / `certificate_images` には `part_installations` に対応する施工品目データが存在しない
- 過去データを遡及的に補完すると不完全情報によるデータ品質悪化のリスクがある
- Toyota POC 等の用途では「新規施工の完全性」が評価対象であり、レガシーデータの有無は要件外

**取り扱い方針:**

| 対象 | 方針 |
|------|------|
| Phase 1 以降の新規施工 | `part_installations` に記録・アンカー対象 |
| Phase 1 以前の既存証明書 | `part_installations` レコードなし（正常状態） |
| 顧客向け UI での表示 | 「部品整合性データなし（旧施工）」と明示。エラー扱いにしない |
| データ完全性ダッシュボード | 「整合性記録済み件数 / 全証明書数」の分子・分母を明示し、分母は Phase 1 以降の件数のみでよい |

**運用メモ:** `/api/cron/parts-anchor` は `part_installations.status = 'customer_verified'` の未アンカー行のみを対象とするため、既存証明書に誤って作用することはない。
