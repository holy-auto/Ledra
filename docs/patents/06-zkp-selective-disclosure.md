...
- **国内先行調査の結果（2026-06 予備・[08 先行技術調査](./08-prior-art-search.md) §5-4/5-5）**：選択的開示×車両/部品を請求項に含む国内特許は **3 件のみ**で、いずれも直撃せず。**最近接は富士通「情報秘匿化プログラム、情報秘匿化方法及び情報管理装置」（特開2024-022912、審査中）**＝サプライチェーンの**累積価値**をコミットメント＋**ゼロ知識証明**で台帳記録する発明。差別化軸：(i) 富士通は**累積値の計算正当性の ZKP**であって、当社の**Merkle 木による属性クレームの選択的開示（メンバーシップ証明）ではない**、(ii) 富士通は**工程間の価値連鎖**／当社は**装着属性を保険会社がサーバ非依存で検証**（適用が別）、(iii) 当社は**開示／非開示／粗粒度混在＋シークレット nonce**を持つ。→ 明細書で本件を引用し (i)〜(iii) で差別化。**審査中のため登録時のクレーム確定を弁理士が監視**する。
