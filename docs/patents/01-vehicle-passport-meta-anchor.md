...
> 注：3〜4 は単体では弱い可能性がある。**独立項は「越境集約 ＋ 順序非依存ダイジェスト ＋ サーバ非依存検証」の組み合わせ**で構成するのが堅い。
...
- 差別化の生命線は「**複数の独立事業者をまたいだ集約**」と「**順序非依存ダイジェストによるサーバ非依存の第三者再計算検証**」。ここを独立項の中心に据える。
- **公開タイミング厳守**：`PASSPORT_PUBLIC_ENABLED=true` 化／HP 掲載／プレス／PoC 資料配布の**前**に出願すること（README の §出願タイミング 参照）。
