...
2. B-1 のアドレスを入力して追加
3. Gmail が確認コード／リンクを専用アドレス宛に送る。この確認メールは **Ledra の
   横断受信箱（メッセージ受信箱）に表示される**ので、記載のコードを Gmail の設定画面へ
   入力（またはリンクをクリック）して転送を有効化する
   - 補足: 自動クリック（自動承認）は行わない。Gmail 側の仕様上、コード／リンクの確認が
     必須のため、スタッフが 1 回だけ承認する
