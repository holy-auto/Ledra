...
6. 店舗選択画面 → 新規テナントは必ず0店舗なので「続行する」をタップ
   （/api/signup は auth.users / tenants / tenant_memberships の3つしか作らない）
7. 生体認証の設定 → オンボーディング → ホーム画面が表示される（=利用開始）
...
2. もしくは「会計」タブ → 「新規会計（飛び込み）」を選択
   （2026-08-26: 飛び込み側にも専用 Tap to Pay ボタンを追加したので、どちらの経路でも
...
| 5 | 5.2 | ✅ Completed | 明細カード直後・支払方法より上に配置。**予約会計と飛び込み会計の両方**（2026-08-26 に飛び込み側を追加） |
...
| 5 | 5.9 | ✅ Completed | 会計画面のインライン UI（`pos/checkout/[id].tsx` の「タッチ決済ができませんでした」等）。**`components/PaymentOutcome.tsx` はどこからも import されていないデッドコードなので、この行の根拠ではない**（2026-08-26 に確認） |
| 5 | 5.10 | ✅ Completed | ReceiptShareDialog (SMS/Email/Share Sheet)。**予約レシートと飛び込みレシートの両方**（2026-08-26 に `receipt-standalone` へ追加） |
