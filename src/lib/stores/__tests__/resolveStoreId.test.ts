 ...
 */
function fakeClient(opts: { list?: Array<{ id: string }>; found?: { id: string } | null }) {
  const node: Record<string, unknown> = {
    eq: () => node,
    limit: async () => ({ data: opts.list ?? [], error: null }),
    maybeSingle: async () => ({ data: opts.found ?? null, error: null }),
  };
