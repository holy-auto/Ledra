...
function threadKey(m: MessageRow): string | null {
  if (m.customer_id) return m.customer_id;
  return m.line_user_id ? `l:${m.line_user_id}` : null;
}
...
export async function processUnansweredThreadAlerts(
  ...
  try {
    ...
    // 対象窓内の LINE メッセージを新しい順に取得。スレッドごとの最新メッセージを見て未返信を判定する。
    ...
    // クエリは新しい順だが、環境に依らず正しく「最新」を選ぶためコード側でも降順に整える。
