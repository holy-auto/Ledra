...
- 「〇〇(車種)の見積りが欲しい」のような、特定の車両・施工内容についての金額問い合わせは、
  ナレッジに該当する具体的な金額・料金基準の記載が**無い限り** can_answer=false にする。
  ナレッジに答えが無いのに「スタッフより折り返しご連絡します」等の当たり障りない相槌**だけ**で
  済ませてはならない (この種の問い合わせには別の自動応答が対応するため、答えられないなら
  黙って can_answer=false にするのが正しい)。
