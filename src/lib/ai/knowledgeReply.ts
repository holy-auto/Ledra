...
// ponytail: 金額問い合わせを can_answer=false に倒す新ルールは、実際に無関係な
// ナレッジで can_answer=true と誤判定していた1事例を根拠にした指示であり、
// 効果は本番の監査ログ (inbound_message.auto_reply_knowledge の confidence /
// can_answer) でしか検証できない (ANTHROPIC_API_KEY が無い開発環境のため未検証)。
// アップグレード経路: 同種の誤判定が別パターンで再発したら例を追加するか、
// 金額問い合わせ (PRICE_INQUIRY_PATTERN 相当) を呼び出し側で先に判定して
// このAI呼び出し自体をスキップする決定的ゲートに置き換える。
...
- 「〇〇(車種)の見積りが欲しい」のような、特定の車両・施工内容についての金額問い合わせは、
  ナレッジに該当する具体的な金額・料金基準の記載が**無い限り**、その金額問い合わせ部分に
  ついては答えない (この種の問い合わせには別の自動応答が対応するため、ナレッジに答えが
  無いのに「スタッフより折り返しご連絡します」等の当たり障りない相槌だけで金額問い合わせに
  答えたことにしてはならない)。メッセージに他の質問 (営業時間・駐車場等) が含まれ、それが
  ナレッジで答えられる場合は、上の部分回答ルールどおりその部分だけで can_answer=true にして
  よい。金額問い合わせがメッセージの主旨で他に答えられる質問が無い場合のみ can_answer=false。

返信文のスタイル:
- 丁寧で親しみやすい日本語。LINE のメッセージとして自然な長さ (目安 300 文字以内)。
- 顧客の質問に直接答えることから始める。署名・店舗名の名乗りは不要。
- 「詳しくはスタッフが折り返しご連絡します」は、ナレッジに基づく具体的な回答を**述べた上で**
  締めの一文として使ってよい。具体的な回答の代わりにこの一文だけで返信してはならない。
