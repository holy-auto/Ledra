 ...
 *
 * 本人が複数店舗のリンクを束ねていれば店舗ごとに並べる。束ねの情報はテナントからは
 * 読めない場所にあるので、**店舗側の見え方は束ねても変わらない**（20260903000001）。
 *
...
export default async function StaffPortfolioPage({ params }: { params: Promise<{ token: string }> }) {
  ...
  const [primary, ...others] = portfolio.shops;
    ...
    <main className="mx-auto max-w-2xl p-6 font-sans">
      <div className="rounded-3xl border border-border-default bg-surface p-6 shadow-sm">
        ...
        <p className="mt-2 text-sm leading-6 text-secondary">
          全 {portfolio.total_certificates} 件{portfolio.shops.length > 1 ? ` / ${portfolio.shops.length} 店舗` : ""}
        </p>

        {portfolio.shops.map((shop) => (
          <section key={shop.link_id} className="mt-6">
            <h2 className="text-sm font-semibold text-primary">
              {shop.shop_name}
              <span className="ml-2 text-xs font-normal text-muted">{shop.certificates.length} 件</span>
            </h2>
            {shop.certificates.length === 0 ? (
              <div className="mt-2 rounded-2xl border border-border-default px-4 py-5 text-center text-sm text-muted">
                まだ施工証明が記録されていません。
              </div>
            ) : (
              <ul className="mt-2 space-y-2">
                {shop.certificates.map((cert) => (
        ...
        <PortfolioActions
          token={token}
          otherShops={others.map((s) => ({ link_id: s.link_id, shop_name: s.shop_name }))}
        />

        <div className="mt-6 text-xs leading-6 text-muted">
          このページはお客様の情報を含みません。各施工の詳細は証明書を開いてご確認ください。
          <br />
          {primary.shop_name} のリンクは、発行元の店舗がいつでも無効にできます。
        </div>
      </div>
    </main>
  );
