...
export default function BillingHubClient() {
  ...
  // Fetch summary counts（帳票管理は請求書も含め同一 documents テーブルを見るため、
  // /api/admin/documents だけで doc_type ごとの件数が揃う）
  useEffect(() => {
    (async () => {
      try {
        const docRes = await fetch("/api/admin/documents", { cache: "no-store" })
          .then((r) => r.json())
          .catch((): null => null);
  ...
  // If a specific view is active, show the document list/create UI filtered to that type
  if (activeView) {
