...
const DEFAULT_FAQ = [
  {
    q: "証明書の発行方法を教えてください",
    a: "左メニューの「証明書管理」→「+ 新規発行」から車両情報・施工内容を入力して発行できます。テンプレートを選択すると、施工種別に応じたフォーマットが自動適用されます。",
  },
  {
    q: "テンプレートを変更したい",
    a: "「ブランド証明書」メニューからプランを選択し、ライト（既製テンプレ+ロゴ反映）またはプレミアム（オリジナルデザイン制作）をお申し込みください。",
  },
  {
    q: "請求書のロゴや角印を設定したい",
    a: "「店舗設定」→「ロゴ設定」からロゴ画像をアップロードできます。角印は「角印・社印」欄からアップロード可能です。帳票・請求書に自動反映されます。",
  },
  {
    q: "プランを変更したい",
    a: "「請求・プラン」メニューから現在のプランを確認し、変更したいプランの「このプランを選択」ボタンからお手続きいただけます。",
  },
  {
    q: "Googleカレンダーと連携したい",
    a: "「予約管理」ページ上部の「Googleカレンダーと連携」ボタンからGoogleアカウントで認証すると、予約が自動同期されます。※環境設定が必要な場合は運営にお問い合わせください。",
  },
  {
    q: "NFCタグの追加購入方法",
    a: "現在NFCタグの追加購入ページは準備中です。追加ご希望の場合は下記フォームよりお問い合わせください。",
  },
  {
    q: "CSVでデータをインポートしたい",
    a: "各管理ページ（車両、顧客、品目など）に「CSVインポート」ボタンがある場合、見本CSVをダウンロードしてデータを記入後アップロードしてください。",
  },
  {
    q: "パスワードを忘れた",
    a: "ログイン画面の「パスワードを忘れた方」リンクからメールアドレスを入力してリセットできます。",
  },
];
      ...
      <div className="space-y-6">
        ...
        <div className="glass-card p-8 text-center space-y-4">
          ...
          <p className="text-sm text-secondary">
            ...
            <br />
            お急ぎの場合は{" "}
            <a href="mailto:info@ledra.co.jp" className="text-accent hover:underline">
              info@ledra.co.jp
            </a>{" "}
            までご連絡ください。
          </p>
    ...
    <div className="space-y-6">
        ...
        <div className="divide-y divide-border-subtle">
          {faqItems.map((item, i) => (
            <div key={item.id ?? i}>
              ...
              {openFaq === i && <div className="pb-3.5 pl-1 text-sm text-secondary leading-relaxed">{item.a}</div>}
            </div>
          ))}
        </div>
      </section>
      ...
      <section>
        ...
        <form onSubmit={handleSubmit} className="glass-card p-6 space-y-5 max-w-2xl">
          ...
          {error && (
            <div className="rounded-lg bg-danger-dim border border-danger/20 p-3 text-sm text-danger">{error}</div>
          )}
