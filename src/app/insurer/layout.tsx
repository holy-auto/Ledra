...
function InsurerSidebar() {
  ...
  const renderItem = (item: NavItem) => {
    const isActive = item.exact ? pathname === item.href : pathname.startsWith(item.href) && item.href !== "/insurer";
  ...
  return <SidebarShell>{sidebarContent}</SidebarShell>;
}
  ...
  return (
    <InsurerRouteGuard>
      <div className="min-h-screen bg-[var(--bg-inset)]">
        <InsurerSidebar />
        <main className="px-4 pt-16 sm:px-6 lg:pt-0 lg:ml-60">{children}</main>
      </div>
    </InsurerRouteGuard>
  );
}
