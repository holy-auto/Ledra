/**
 * 代理店の供給パートナープロフィール + API 連携設定 (代理店アカウント統合版)。
 *
 * パートナー ≡ 代理店。GET 時に代理店へ supply_partner を自動プロビジョニングする
 * ため、別途「登録 (POST)」は不要 (旧 /api/supply/profile の POST は廃止)。
 *
 * - GET: 自代理店のパートナー情報 + 認証情報の「設定済みか」だけを返す (鍵は返さない)。
 * - PUT: 連絡先 / API エンドポイント / 認証方式 / API 鍵を更新する。
 *
 * セキュリティ:
 *   - status / agent_id は DB トリガが owner 変更を差し戻す。ここでも送信しない。
 *   - API 鍵は secretBox で暗号化し supply_partner_credentials に隔離保存。
 *     レスポンスには credentials_configured (boolean) のみ。
 *   - RLS (my_supply_partner_ids = agent 経由) で自代理店の行のみ読み書き可。
 */
...
import { supplyProfileUpdateSchema } from "@/lib/supply/validation";
...
export async function GET() {
  try {
    // provision: 代理店なら未作成時に自動作成。代理店でなければ null。
    ...
    const { data: partner, error } = await supabase
      ...
      .eq("id", ctx.partnerId)
      .maybeSingle();
    if (error) return apiInternalError(error, "agent supply profile get");
    ...
    return apiInternalError(e, "agent supply profile get");
  }
}
...
export async function PUT(req: NextRequest) {
  try {
    ...
    if (Object.keys(updates).length > 0) {
      ...
      if (error) return apiInternalError(error, "agent supply profile update");
    }
    ...
    if (touchKey || touchSecret) {
      if (!hasEncryptionKey()) {
        return apiInternalError(new Error("SECRET_ENCRYPTION_KEY not set"), "agent supply credentials update");
      }
      ...
      if (credErr) return apiInternalError(credErr, "agent supply credentials update");
    }

    // 連携状態の自動更新: エンドポイント + 認証方式 + 鍵が揃えば connected。
    ...
    return apiInternalError(e, "agent supply profile update");
  }
}
