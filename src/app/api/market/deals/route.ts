...
export const POST = withCaller(
  ...
  { permission: "market:create", routeName: "market/deals POST" },
);
  ...
  async (req, { caller }) => {
    try {
      ...
      const withEstimates = rows.map((d) => ({
        ...
        trade_in_vehicle:
          typeof d.trade_in_vehicle_id === "string" ? (tradeInById[d.trade_in_vehicle_id] ?? null) : null,
      }));
