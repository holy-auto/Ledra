...
export const POST = withCaller(
  async (req, { caller }) => {
    ...
    // AI マスタースイッチ OFF / 月次コストキャップ超過時はスキップして手打ちにフォールバック
    ...
    // Vision 呼び出し (inner try/catch kept intentionally)
    try {
  ...
  { minRole: "staff", rateLimit: "identity_ocr", routeName: "certificates/thickness/ocr POST" },
);
