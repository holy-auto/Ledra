...
import { apiJson, apiValidationError, apiNotFound, apiInternalError, apiError } from "@/lib/api/response";
...
export const GET = withCaller(
  ...
  { routeName: "admin/reservations GET" },
);
...
export const POST = withCaller(
    ...
    try {
      ...
      after(async () => {
        const tasks = [
          ...
          maybeAutoProposeWorkflowForReservation({
            tenantId: caller.tenantId,
            reservationId: reservation.id as string,
          }),
          maybeAutoSuggestAssigneeForReservation({
            tenantId: caller.tenantId,
            reservationId: reservation.id as string,
          }),
        ];
  ...
  { permission: "reservations:edit", routeName: "admin/reservations POST" },
);
...
export const PUT = withCaller(
  async (req, { caller, supabase }) => {
    try {
      ...
      if (sentKeys.has("workflow_template_id")) {
        ...
        const changing =
          !!cur &&
          (cur as { workflow_template_id: string | null }).workflow_template_id !== updates.workflow_template_id;
        if (started && changing) {
  ...
  { permission: "reservations:edit", routeName: "admin/reservations PUT" },
);
...
export const DELETE = withCaller(
  ...
  { permission: "reservations:edit", routeName: "admin/reservations DELETE" },
);
