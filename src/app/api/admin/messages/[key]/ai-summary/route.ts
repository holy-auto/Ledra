...
import { apiOk, apiNotFound, apiValidationError, apiPlanLimit } from "@/lib/api/response";
...
export const POST = withCaller<{ key: string }>(
  async (req: NextRequest, { caller, params }) => {
    ...
    try {
      // カスタムレート制限 (usage 記録が必要なため options.rateLimit ではなく手動)
  ...
  { minRole: "staff", routeName: "messages ai-summary POST" },
);
