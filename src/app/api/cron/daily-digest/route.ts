 ...
 * 呼び出しの総数は変わらない。
 *
 * **月次コストキャップは当てにしないこと。** `settings.enabled` がキャップ超過で
 * false に倒れるのは、そのテナントがキャップを設定している場合だけ。
 * `resolveCapJpy()` の既定は 0（無効）で、`.env.example` の
 * `AI_MONTHLY_COST_CAP_JPY` も 0、本番のテナント個別上限は 0 件（2026-09-04 実測）。
 * ここで費用を抑えているのは、キャップではなく上の「1 テナント 1 日 1 回」の構造。
 *
