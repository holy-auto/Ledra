        ...
        <form onSubmit={handleCreate} className="rounded-2xl border border-border-subtle bg-surface p-4 space-y-3">
          ...
          <div className="flex gap-3">
            <input
              ...
              type="number"
              placeholder="募集上限"
              className="w-40 rounded-lg border border-border-subtle bg-surface px-3 py-2 text-sm"
            />
            <input
              name="deadline"
              type="datetime-local"
              className="w-56 rounded-lg border border-border-subtle bg-surface px-3 py-2 text-sm"
            />
          </div>
          <button
            type="submit"
          ...
          {items.map((r) => (
            ...
            >
              ...
              <div className="flex items-center gap-2">
                <span
                  className={`rounded-full px-2 py-0.5 text-[11px] font-medium ${r.is_open ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-500"}`}
                >
                  {r.is_open ? "受付中" : "締切"}
                </span>
                {isAdmin && (
