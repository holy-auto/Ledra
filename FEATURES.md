...
#### 「案件 (Job)」軸での業務フロー統合 ★最優先

予約・チェックイン・作業進捗・写真・証明書・請求・決済が別画面で分散している状態を、**案件ID を串刺しにした1画面・タブ切替** に統合します。

```
Before: /reservations → /vehicles/new → /certificates/new → /invoices/new → /pos   (5画面)
After:  /admin/jobs/[id]  タブ: 予約 → 車両 → 作業 → 証明書 → 請求 → 決済
```
