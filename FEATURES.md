...
### 2.1 ダッシュボード `/admin`
- **ログイン後のファーストビュー**: 施工店ユーザーはログイン後この `/admin`（ダッシュボード）に着地（代理店は `/agent`、本社専用ユーザーは `/admin/hq-overview`）
...
- **オンボーディングツアー** (driver.js): 初回 `/admin` 訪問時のみ自動起動。SetupChecklist→クイックアクション→Cmd+K ヒント→**AIナビ（AIに聞いて画面を開く）→サイドバー整理（すべての機能を表示・ピン留め）**→店頭/管理モード→…→サポート の順でガイド。新機能追加時はツアーキー（`ledra_tour_done_v2`）を更新し、既存ユーザーにも一度だけ再表示
...
### 2.25 共通UI機能
- **サイドバー (スリム表示 + ピン留め)**: 常時表示はコア機能（予約 / メッセージ / 顧客 / 車両 / 帳票 / 証明書 / スタッフ / 進捗）＋ ダッシュボード / 承認インボックスに絞り込み。その他は「すべての機能を表示」トグルで6グループ全体を展開 (詳細はセクション12参照)。よく使う画面は📌ピン留めで常時表示（localStorage 永続）。RBAC / プラン / 業種ゲートは従来どおり適用
- **AIナビ・アシスタント** `⌘/Ctrl+J`（各画面トップバーの「AIに聞く」ボタン / サイドバー入口からも起動）: 自由文を打つと意図を判定して動作。頻繁に使うため全画面から即起動でき、入力欄は大きめ・日本語IME確定Enterでは誤送信しない
  - **画面を開く**: 「予約を開いて」「先月の売上」等 → 該当画面へ遷移（軽量モデルで自然文→画面カタログに解決。ハルシネーション / オープンリダイレクト防止のためサーバ側で href をカタログ照合）
  - **情報を探す**: 「田中さんの情報」「ナンバー品川300」「車体番号◯◯」「証明書 LD-001」「帳票番号 INV-2024」等 → 顧客 / 車両 / 証明書 / 請求書を横断検索し、単一ヒットは即遷移・複数は候補チップ提示（テナント境界＋staff権限で担保。証明書は `public_id` で遷移）
  - **フォールバック**: AI 不通・未一致時は名前が近い画面の静的候補を提示し、ナビを止めない
  - 入力が空/「使い方」ボタンで、例文とショートカットの「検索ワード表」を表示
- **コマンドパレット** `Cmd+K`: 管理画面内のクイック検索・ナビゲーション（AIナビと併存）
